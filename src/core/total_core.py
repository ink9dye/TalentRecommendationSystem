...
if __name__ == "__main__":
    ...
    query_text = ("运动学与动力学算法研发：负责机器人运动学、动力学建模以及运动控制算法的设计与优化；"
                  "建立⾼性能、可扩展的机器⼈运动控制与状态估计模块。轨迹规划与全⾝控制算法开发："
                  "研发与优化机器人轨迹生成与全⾝控制算法，包括但不限于RRT/PRM/CHOMP/MPC/iLQR等；"
                  "针对复杂场景进行约束优化、时序规划与碰撞规避设计，确保系统的平滑性、稳定性与可执行性。"
                  "仿真平台构建与验证：利用Isaac Sim/Gazebo/MuJoCo等平台搭建仿真环境，进行算法快速验证与评估；"
                  "推动仿真到实机的一致性优化，包括动力学一致性、摩擦模型校准、控制频率匹配等。"
                  "系统集成与性能调优：参与机器人控制系统全流程开发，从底层控制到高层规划架构；"
                  "开展实时控制性能调优（延迟、抖动、稳定性分析），提升系统在复杂任务下的执行效率与鲁棒性。"
                  "技术追踪与创新：持续关注运动控制、机器人动力学建模及规划领域的前沿研究，"
    ...
    for item in results[:10]:
        ...
        # 展示学术指标 (H-Index, 总论文数, 引用量)
        m = item['metrics']
        # 重点修改：增加了 total_papers 字段的展示
