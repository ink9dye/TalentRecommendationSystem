...
# 8. 知识图谱构建 SQL 语句
SQL_QUERIES = {
    # 增加 ORDER BY 确保增量位点的单调递增
    "SYNC_AUTHORS": "SELECT author_id as id, author_id as aid, name, h_index, cited_by_count as citations, last_updated FROM authors WHERE last_updated > ? ORDER BY last_updated ASC",

    # 修正：SYNC_WORKS 之前缺少增量过滤，如果不加，每次都会全量同步
    ...
    "SYNC_INSTITUTIONS": "SELECT inst_id as id, inst_id as iid, name, cited_by_count as citations, last_updated FROM institutions WHERE last_updated > ? ORDER BY last_updated ASC",

    "SYNC_SOURCES": "SELECT source_id as id, source_id as sid, display_name as name, type, cited_by_count as citations, last_updated FROM sources WHERE last_updated > ? ORDER BY last_updated ASC",
