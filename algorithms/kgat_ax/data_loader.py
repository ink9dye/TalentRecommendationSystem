...
class DataLoaderBase(object):
    ...
    def load_auxiliary_info(self):
        """从 SQLite 或 Data 加载学者的 H-index, 引用量等 """
        # 预开发阶段：生成模拟数据 (实体总数 x 3个特征)
        self.aux_info_all = np.random.rand(self.n_users_entities, self.args.n_aux_features).astype(np.float32)
        self.aux_info_all = torch.from_numpy(self.aux_info_all)

    def load_cf(self, filename):
        # ... (保持原有的 load_cf 逻辑) ...
        return (np.array([0]), np.array([0])), {}  # 占位符

    def statistic_cf(self):
        # ... (保持原有的统计逻辑) ...
        self.n_users_entities = 0  # 占位符

    def generate_kg_batch(self, kg_dict, batch_size, highest_neg_idx):
        """采样三元组并附带辅助信息 [cite: 989, 991]"""
        # 1. 采样 ID
        ...
        for h in batch_head:
            # (简化的采样逻辑)
            rel, pos_t = kg_dict[h][0][1], kg_dict[h][0][0]
            h_ids.append(h);
            r_ids.append(rel);
            pos_t_ids.append(pos_t)
            neg_t_ids.append(random.randint(0, highest_neg_idx - 1))
