...
| Stage3 | `src/core/recall/label_pipeline/stage3_term_filtering.py` | 消费 `run_stage2` 的 dict 与 `candidate_graph`；merge/rerank 与 2.3 分层一致 |
...
- **新增参数**：`candidate_graph`（来自 Stage2 输出）、`return_coherence_report`、`paper_term_limit`。
- **返回改为 dict**：`selected_core_terms`、`selected_support_terms`、`risky_terms`、`paper_terms`、`global_coherence_report`。
- **内部阶段化**：从 `stage2_output` 读取 `all_candidates` 与 `candidate_graph`；不得假设 Stage2 顶层存在 `primary_bucket` 等终局字段。
- **迁出/弱化**：paper 细编排、Stage2 补锅逻辑迁出或变薄。
...
**P0 验收**：全链路可运行；Stage2 满足 **2.7**；下游仅依赖约定 dict 键。
