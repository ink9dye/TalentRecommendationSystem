...
└── src/
    ...
    │   │   │   ├── base.py
    │   │   │   ├── label_anchors.py
    │   │   │   ├── hierarchy_guard.py  # 层级守卫：分布/纯度/熵、hierarchical_fit（缺失 topic 用 None 不按 0）、泛词惩罚、landing/expansion 打分、should_drop_term（仅 2 条且仅对非 primary-like 生效）、score_term_record（path_topic 含 primary/similar/anchor bonus；gate=0.75+0.15×path_topic+0.10×generic_penalty）、allow_primary_to_expand（放宽）
    ...
    │   │       ├── stage3_term_filtering.py  # 词过滤与权重：按 tid 去重聚合 → 轻硬过滤（仅对非 primary-like）→ identity gate → **topic gate 仅对 support-like 严格，primary-like 跳过**（_is_primary_like）→ 唯一主分 score_term_record（primary-like path_topic 保底 0.45）→ family 角色约束 → risky/bucket → top_k；日志 [Stage3 topic_gate] bypass primary-like / drop support
...
| **核心中间产物** | **Stage1**：Stage1Result（**含 jd_profile**）、anchor_skills（含 local_context、phrase_context、**anchor_source/anchor_source_weight**）、_jd_cleaned_terms、job_ids、regex_str。**Stage2A**：primary_landings（含 domain_fit、**landing_score**、**subfield_fit**、**topic_fit** 等；有 jd_profile 时做层级 fit 与硬门槛）。**Stage2B**：raw_candidates（term_role、domain_fit、parent_anchor、parent_primary、**subfield_fit**、**topic_fit**、**landing_score**、**cluster_id**、**main_subfield_match** 等）。**Stage3**：先 **按 tid 去重聚合**，再 **轻硬过滤**（should_drop_term 仅 2 条：outside_subfield_and_no_topic_fit、weak_evidence_noise）、**唯一主分 score_term_record**（base_score、path_topic_consistency、generic_penalty、cross_anchor_factor）、**family 角色约束**、**risky/bucket**（无 cluster）；score_map、term_map、idf_map、term_role_map、term_source_map、parent_anchor_map、parent_primary_map、tag_purity_debug（含 **anchor_count**、**evidence_count**、**family_centrality**、**path_topic_consistency**、**generic_penalty**、**cross_anchor_factor**、**retrieval_role**）；label_path 据此构建 term_confidence_map、term_uniqueness_map。**Stage4**：author_papers_list。**Stage5**：paper_map、contribution、作者聚合分、最终 author_id 列表。 |
...
| **候选来源（当前版本）** | **similar_to 为主池** + **conditioned_vec 为上下文复核**：① **跨类型 SIMILAR_TO** 给出初始候选；② **conditioned_vec** 在同一学术词索引上检索，**不直接并池**，而是为每个 similar_to 候选生成 **context_sim / context_supported / context_gap** 重打分信号；③ 仅当主池候选数 ≤1 时，才从 conditioned_vec 检索结果中补最多 2 个 **context_fallback**（context_sim≥0.82）。领域过滤使用 `_term_in_active_domains_with_reason`，仅强冲突剔除。 |
| **思路** | 图中 SIMILAR_TO 提供**建图时**的工业→学术边；**conditioned_vec** 不再作为「第二路候选源」，而是作为 **similar_to 的上下文复核器**：raw_sim 与 context_sim 越一致，说明该词在「无上下文」与「有 JD 上下文」下都站得住，可压制 propulsion、simula、kinesics 等错义；准入与 primary_score 均纳入 **compute_context_consistency** 与 **check_primary_eligibility**。 |
| **逻辑流程** | ① anchor_skills 转为 **PreparedAnchor**（含 **conditioned_vec**）；② `collect_landing_candidates`：similar_to 初始池 → `_retrieve_academic_terms_by_conditioned_vec` 返回 context_neighbors + **rerank_signals**，为 similar_to 候选挂 **context_sim/context_supported/context_gap/source_role**，主池≤1 时补 1～2 个 fallback；③ flat_pool 上邻域与 conditioned_anchor_align；④ **资格赛**：`check_primary_eligibility`（identity/context 门槛、domain_conflict_strong）；⑤ **准入**：`check_primary_admission`（双空间一致、rescue、context_fallback 通道）；⑥ **打分**：`compute_primary_score`（identity + context_consistency + jd 主干，hierarchy 微调，similar_to 无上下文支持时 ×0.90）；⑦ **冲突消解** → primary_landings。 |
...
| **调用的表/知识图谱** | **Neo4j**：**Vocabulary** 节点、**(Vocabulary)-[SIMILAR_TO]->(Vocabulary)**（跨类型，边权 sim_score）；**SQLite**：`vocabulary_domain_stats`（domain_dist）、`vocabulary_topic_stats`（field_id/subfield_id/topic_id 及 *_dist）；**Faiss**：**vocab_index**（conditioned_vec 检索学术词，用于上下文纠偏与 rerank_signals）。 |
...
- **Stage2B 固定成两层门**：**Seed 门**（谁能扩）：`check_seed_eligibility` 判 retain_mode==normal、非 suppress_seed、source∈可信、**is_primary_expandable** 通过、**无 fallback 兜底**（无可扩 seed 则 diffusion_primaries=[]）；**Support 门**（扩出来的词谁能留）：dense/cooc 候选须过 **support_expandable_for_anchor**（锚点语义复核），domain_fit≥0.72、sim≥0.55、设备/对象词默认拒。**Cluster 当前全关**（`expand_from_cluster_members` 直接 return []）。
...
- **Stage3 彻底移除 cluster 依赖**：最终分只保留 4 类正交量 **base_score**、**path_topic_consistency**、**generic_penalty**、**cross_anchor_factor**；`score_term_record` 为唯一主分，gate=0.75+0.15×path_topic_consistency+0.10×generic_penalty。**cluster_cohesion、cluster_*、semantic_drift_risk、outside_* **仅 debug/explain，不参与最终分；risky 理由改为 single_anchor_or_few、weak_family_centrality、high_drift_risk、low_topic_consistency。
...
| `check_primary_admission` | **上下文稳定性优先，identity 软约束**：① 主通道：semantic≥0.80、jd_align≥0.78、**context_consistency≥0.76**、且（anchor_identity≥0.22 或 ctx_sim≥0.82）⇒ normal/weak_retain（identity≥0.30 为 normal，否则 suppress_seed）；② rescue：多锚 + path/jd/semantic 达标、**context_consistency≥0.72**、jd_align≥max(0.76, PRIMARY_RESCUE_JD_ALIGN_MIN−0.02）；③ context_fallback：ctx_sim≥0.84、jd_align≥0.80、anchor_identity≥0.32 ⇒ weak_retain；④ **hierarchy 边界**：topic_overlap≥PRIMARY_MIN_HIERARCHY_MATCH、subfield≥0.25、context_consistency≥0.70 ⇒ weak_retain（hierarchy_supported_but_context_borderline）；⑤ **通用主词弱放行**：semantic≥0.80、jd_align≥0.78、context_consistency≥0.68、anchor_identity≥0.16 ⇒ weak_retain + suppress_seed（generic_main_term_supported），用于 simulation/mechanics/route planning 等。 |
...
| 7 | **Stage3 唯一主分**：score_term_record 仅用 base_score、path_topic_consistency、generic_penalty、cross_anchor_factor；_compute_stage3_global_consensus 只写 cross_anchor_evidence、semantic_drift_risk（仅 debug），无 cluster | `hierarchy_guard.score_term_record`；`_compute_stage3_global_consensus` |
...
| **逻辑流程** | ① _merge_stage3_duplicates；② stage2_rank 写入；③ _compute_stage3_global_consensus（无 cluster）；④ _build_family_buckets；⑤ 每 term _compute_family_centrality；⑥ should_drop_term → passes_identity_gate → **若 primary_like 则跳过 topic gate，否则 passes_topic_consistency**；⑦ quality_score 仅 debug；⑧ score_term_record 唯一主分（primary-like path_topic 保底 0.45）；⑨ 排序 → _apply_family_role_constraints → 再排序；⑩ _collect_risky_reasons、_bucket_stage3_terms；⑪ select_terms_for_paper_recall；⑫ 写 map、tag_purity_debug。 |
| **输入参数（名字与含义）** | **raw_candidates**：Stage2 输出；**query_vector**；**anchor_vids**：可选。 |
| **输出参数（名字与含义）** | **score_map**、**term_map**、**idf_map**、**term_role_map**、**term_source_map**、**parent_anchor_map**、**parent_primary_map**、**paper_terms**；**tag_purity_debug** 含 tid、term、anchor_count、evidence_count、family_centrality、path_topic_consistency、generic_penalty、cross_anchor_factor、retrieval_role、stage3_explain。 |
| **主要公式** | **身份闸门**：primary/similar_to≥0.62，其余≥0.68。**topic 闸门**：primary-like 默认通过；support-like 仅当 topic_fit<0.05 且 outside>0.95 时拒绝。**唯一主分**（score_term_record）：path_topic_consistency=0.45·topic_fit+0.30·subfield_fit+0.15·primary_bonus+0.05·similar_bonus+0.05·anchor_bonus，**primary-like 保底 0.45**；gate=0.75+0.15×path_topic+0.10×generic_penalty；final=base×gate×cross_anchor_factor。**保留策略**：按 final_score 降序取前 STAGE3_TOP_K（默认 20）。 |

**Stage3 主分**：**compose_term_final_score** 已退出 Stage3 主链（deprecated），唯一主分由 **score_term_record** 承担。
...
| **5. Stage3** | 全局重排与风险分桶 | `[Stage3]` 分区头；`[Stage3 topic_gate] bypass primary-like tid=… term=…` / `[Stage3 topic_gate] drop support tid=… term=… reason=topic_consistency`；`[Stage3 Final Score Breakdown]` term \| stage2_rank \| anchor_count \| … \| final；`[Stage3 Buckets]` core_terms / support_terms / risky_terms；`[Stage3 Risky Term Reasons]` term \| reasons \| final（LEVEL≥3） |
...
| **hierarchy_guard** | 分布/纯度/熵、层级 fit、泛词惩罚；**结构信号**：build_family_key、get_retrieval_role_from_term_role；Stage3 **should_drop_term** 仅对非 primary-like；**score_term_record** 为唯一主分，path_topic_consistency 含 primary_bonus/similar_bonus/anchor_bonus，**primary-like 的 path_topic 保底 0.45**。allow_primary_to_expand 不再被 Stage2B seed 决策调用。 | Stage2A / Stage2B / Stage3 |
...
- **Stage3 全部滤空或几乎全过**：优先看 passes_identity_gate；**topic 闸门只拦 support-like**，primary-like 跳过（日志见 `[Stage3 topic_gate] bypass primary-like` / `drop support`）；当前按 final_score 排序保留 top_k（STAGE3_TOP_K）；可开 STAGE3_DETAIL_DEBUG 看每词 base_score、path_topic_consistency、各 penalty、reject_reason。
...
- **Stage3**：按 tid 去重聚合；`should_drop_term`（仅对非 primary-like）；**topic 闸门只对 support-like 严格**，primary-like 跳过（_is_primary_like）；`score_term_record`（唯一主分，primary-like 的 path_topic 保底 0.45）；family 角色约束、risky/bucket（无 cluster）、按 final_score 排序 top_k；缺失 topic/subfield 时退化为 field/domain
...
**当前已落地（防断流与轻惩罚）**：① **Stage3** 改为「按 tid 去重聚合 → 轻硬过滤（仅对非 primary-like）→ identity gate → **topic gate 仅对 support-like 严格，primary-like 跳过** → 唯一主分 score_term_record（primary-like path_topic 保底 0.45）→ family 角色约束 → risky/bucket（无 cluster）→ 按 final_score 排序 top_k（STAGE3_TOP_K=20）」；不再用 FINAL_MIN_TERM_SCORE 阈值淘汰。② **passes_topic_consistency**：primary-like 默认通过，只拦明显偏题 support。③ **score_term_record**：primary-like 的 path_topic_consistency 保底 0.45。④ **日志**：`[Stage3 topic_gate] bypass primary-like` / `[Stage3 topic_gate] drop support ... reason=topic_consistency`。⑤ **调试**：Stage3 明细含 anchor_count、evidence_count、family_centrality、path_topic_consistency、generic_penalty、cross_anchor_factor、retrieval_role（STAGE3_DETAIL_DEBUG）。
...
- **`src/core/recall/label_path.py`**  
  ...
  - **`label_pipeline/`** 子模块：**stage1_domain_anchors.py**（领域与锚点；**attach_anchor_contexts**、**build_jd_hierarchy_profile**，产出 jd_profile 与锚点 local_context/phrase_context）；**stage2_expansion.py**（学术落点与扩展；接收 jd_profile，raw_candidates 带 subfield_fit/topic_fit/landing_score/cluster_id/**main_subfield_match** 等）；**stage3_term_filtering.py**（**按 tid 去重聚合**、轻硬过滤仅对非 primary-like、**topic gate 仅对 support-like 严格**（_is_primary_like 跳过）、唯一主分 score_term_record（primary-like path_topic 保底 0.45）、family 角色约束、risky/bucket 无 cluster、按 final_score 排序 top_k；日志 `[Stage3 topic_gate] bypass primary-like` / `drop support`）；**stage4_paper_recall.py**（二层论文召回、领域软奖励、term_scores 入 paper_score、TERM_MAX_PAPERS/per-term 限流、MELT_RATIO）；**stage5_author_rank.py**（作者排序与截断；预留 CoverageBonus/HierarchyConsistency/FamilyBalancePenalty）。
