...
2. **五阶段流水线**：`stage1_domain_anchors`（领域与锚点准备）→ `stage2_expansion`（学术落点与扩展候选）→ `stage3_term_filtering`（词级重排与论文检索词选择）→ `stage4_paper_recall`（论文层召回与证据打分）→ `stage5_author_rank`（作者层聚合、排序与截断）。
...
Stage1 的职责不只是抽取岗位锚点，而是为后续扩展阶段准备一整套可复用的上下文状态。该阶段会结合领域探测结果、岗位技能抽取结果和 JD 文本本身，生成 `active_domains`、工业锚点集合、锚点局部上下文，以及供后续层级守卫使用的 `jd_profile`。在当前实现里，这一阶段还会补充锚点的 `anchor_type`、`local_context`、`phrase_context`，并构造条件化锚点表示，因此它更接近「标签路的准备阶段」，而不是单纯的关键词抽取。

##### Stage2：学术落点与扩展候选生成

Stage2 负责把岗位侧锚点落到学术词汇空间，并在此基础上生成后续可筛选的扩展候选。其核心不是简单扩词，而是先为每个锚点寻找更合适的 primary landing，再围绕主落点补充相关学术词、同簇词或共鸣词，形成带有层级适配度、落点评分和来源信息的候选集合。这个阶段决定了标签路后续是围绕「正确学术概念」继续推进，还是在一开始就被泛词或偏义项带偏。实现上仍细分为 **Stage2A / Stage2B**（候选主线组织与小半径保守扩族等），详见下文「正确分层图」与数据流表。
...
`LabelRecallPath.recall()`（约第 1586 行）依次调用：`_stage1_domain_and_anchors`（→ `stage1_domain_anchors.run_stage1`）、`_stage2_expand_academic_terms`（→ `stage2_expansion.run_stage2`）、`_stage3_word_weights`（→ `stage3_term_filtering.run_stage3`）、`_stage4_graph_search`（→ `stage4_paper_recall.run_stage4`）、`_stage5_score_and_rank_authors`（→ `stage5_author_rank.run_stage5`）。基础设施由 `LabelMeansInfra` 在 `__init__` 中统一加载（`graph`、`job_index`、`vocab_index`、`stats_conn`、`cluster_members`、`voc_to_clusters`、`cluster_centroids` 等），见 `label_means.infra`。
