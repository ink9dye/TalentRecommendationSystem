...
| **思路** | **词侧**：熔断放宽为 **MELT_RATIO**（默认 5%），(degree_w/total_w) ≥ 0.05 的泛词在 Cypher 内过滤。**论文侧**：不做 `WHERE w.domain_ids =~ $regex` 剔除，仅用 **domain_bonus**（匹配 1.2、不匹配 1.0）参与打分。**term_contrib** 不再只靠 term 命中：先在 Python 里用 paper 的 `title/domains` 做 **grounding gate**（落地主轴强度 + 偏题惩罚），再计算 `term_contrib = base_term_contrib × (0.45 + 0.55×grounding) × off_topic_penalty`；若 `grounding < 0.12` 则该 (term,paper) 直接丢弃。每个 term 再做 per-term 限流：默认 **TERM_MAX_PAPERS**，但 **paper_support** 词本地上限收紧到 **min(TERM_MAX_PAPERS, 30)**；最终 **paper_score** = Σ term_contrib，并全局 ORDER BY paper_score DESC、LIMIT 2000。 |
| **逻辑流程** | ① 输入 **vocab_ids**、**regex_str**、**term_scores**（vid→Stage3 final_score）；② **第一层 Cypher**：MATCH (v:Vocabulary) WHERE v.id IN $v_ids，WITH degree_w，WHERE (degree_w/total_w) < MELT_RATIO，WITH idf_weight；MATCH (v)<-[:HAS_TOPIC]-(w:Work)，计算 domain_bonus（CASE WHEN regex 匹配 THEN 1.2 ELSE 1.0），并 RETURN `vid, wid, idf_weight, domain_bonus, year, title, domains`；③ **Python**：对每个 (vid,wid) 取 `term_final_score` 与 `role_weight` 得 `base_term_contrib`，再调用 grounding(off_topic) 计算 `term_contrib`；若 `grounding < 0.22` 直接 continue；每 term 按 term_contrib 降序取前 `TERM_MAX_PAPERS`（或 paper_support 的 `min(TERM_MAX_PAPERS,15)`）；按 wid 聚合 paper_score = Σ term_contrib、hits = [ {vid, idf}, ... ]；按 paper_score 降序取前 GLOBAL_PAPER_LIMIT(2000)；④ **第二层 Cypher**：对上述 wids 查 (w:Work)-[:AUTHORED]-(a:Author) 之类并按 aid 聚合；⑤ 返回 **author_papers_list**。 |
...
| **主要公式** | 熔断：`(degree_w * 1.0 / total_w) < MELT_RATIO`（默认 0.05）；**idf_weight** = `log10(total_w / (degree_w + 1))`；**role_weight**：paper_primary=1.0、paper_support=0.7、其他=0.4（**不看领域词，只看 retrieval_role**）；**base_term_contrib** = term_final_score × role_weight × idf_weight × domain_bonus × recency(year)；**grounding**（0~1）来自 paper 的 `title/domains` 与 `term_meta`：包括词面命中、父锚/父主词轴命中、以及机器人/控制轴关键词命中，并对 paper_support 做轻微收紧；**off_topic_penalty** 对交通/调度等偏题模式施加惩罚（目前先降为 0.75/0.75/0.75/0.88 档）；最终 **term_contrib** = base_term_contrib × (0.45 + 0.55×grounding) × off_topic_penalty，且若 `grounding < 0.12` 则不计入；per-term 限流：默认 **TERM_MAX_PAPERS**，其中 **paper_support** 使用 `min(TERM_MAX_PAPERS, 30)`；全局 **GLOBAL_PAPER_LIMIT** = 2000。 |
