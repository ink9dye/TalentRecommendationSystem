...
└── src/
    ...
    │   │       ├── stage2_expansion.py       # run_stage2 返回 dict（all_candidates/anchor_to_candidates/candidate_graph/stage2_report）；all_candidates 即 Stage3 原 raw 列表语义
...
2. **五阶段流水线**：`stage1_domain_anchors`（领域与锚点准备；含 JD 粗分与 **`refine_stage1_anchor_layers`**）→ `stage2_expansion`（**`run_stage2` 结构化 dict**，主链取 `all_candidates`）→ `stage3_term_filtering`（词级重排与论文检索词选择）→ `stage4_paper_recall`（论文层召回与证据打分）→ `stage5_author_rank`（作者层聚合、排序与截断）。
...
**对外契约（当前仓库）**：**`stage2_expansion.run_stage2` 返回结构化 `dict`**，包含 **`all_candidates`**（与 Stage3 衔接的平铺列表，语义同原先的 `raw_candidates`）、**`anchor_to_candidates`**（按锚分组的节点视图，含 `landing_candidates` / `expansion_candidates` 槽位及 `summary`；分组键优先 `anchor_id`，缺省则回退 **`anchor_term` / `parent_anchor`**）、**`candidate_graph`**（MUV：四类边键 **`same_anchor_edges` / `cross_anchor_support_edges` / `family_edges` / `provenance_edges`**，由 **`build_stage2_candidate_graph`** 构建）、**`stage2_report`**（规模、按锚统计、`source_distribution`、`graph_edge_counts` 等）。**`LabelRecallPath.recall`** 取 **`stage2_output["all_candidates"]`** 作为传入 Stage3 的列表。

**单条候选字段分层**：**`_expanded_to_raw_candidates`** 将强 provisional 痕迹（如 **`primary_bucket`、`fallback_primary`、`reject_reason`** 等）主要收拢到 **`stage2_local_meta`**；顶层以标识、局部证据（identity/sim、层级 fit、`risk_flags`、`landing_confidence` / `expansion_confidence` 等）为主。为兼容现有 Stage3 合并逻辑，**仍保留少量顶层镜像字段**（代码中标注待 Stage3 迁走后删除）。**`risk_flags`**、**`_debug`** 等便于观测与回溯。

##### Stage3：词级重排与论文检索词选择

Stage3 不只是做一个宽泛的 filtering，而是承担词级收束与论文检索词定稿的作用。该阶段会结合 identity / quality gate、层级适配、主轴一致性以及词级全局重排结果，对 Stage2 生成的候选词进一步压缩，并通过 `S3_select` 选择真正进入论文召回的 term 子集。因此，Stage3 的职责更准确地说是「词级重排与论文检索词选择」，而不是简单删词。
