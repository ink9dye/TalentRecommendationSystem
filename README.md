...
| **Stage2B** | primary_landings（经 **check_seed_eligibility** 得 diffusion_primaries，并打 **seed_blocked/seed_block_reason**）, active_domain_set, jd_*_ids | raw_candidates（**5 个正交层级字段**；**retain_mode、topic_source、seed_blocked、seed_block_reason** 下传 Stage3/debug） | check_seed_eligibility（weak_retain/suppress_seed、topic_source cooc 严 bar、semantic_mismatch 禁扩）；expand_from_*（dense 入口审 seed+支撑过滤；cluster/cooc 仅 strong normal）；merge_primary_and_support_terms；_expanded_to_raw_candidates |
...
- **LandingCandidate 新增字段**：`context_sim`（conditioned_vec 下相似度）、`context_supported`（是否≥0.78）、`context_gap`（raw_sim − context_sim）、`source_role`（seed_candidate | context_fallback）、`primary_eligible` / `primary_eligibility_reasons`、`domain_reason`（如 domain_conflict_strong）。
...
| **Stage2B** | |
| `check_seed_eligibility` | 返回 (eligible, seed_score, block_reason)。**weak_retain 且 suppress_seed=True** 默认禁扩；**topic_source==cooc** 时仅 semantic≥0.84、jd_align≥0.82、anchor_identity≥0.35 才可扩；**is_semantic_mismatch_seed**（动力学→propulsion、抓取→retrieval/crawling、端到端→principle、强化学习→仅 q-learning）直接 block；**should_block_seed_expansion** 继续判低 identity 单支撑、过窄子项等。准入后为 primary 打 **seed_blocked / seed_block_reason**。 |
| `expand_from_vocab_dense_neighbors` | 入口对每个 primary 再跑 **check_seed_eligibility**，不通过则跳过该 primary；支撑词用 **_is_bad_support_for_anchor(anchor_term, neighbor_term)** 过滤语义偏航（与 is_semantic_mismatch_seed 同族）。 |
| `expand_from_cluster_members` | 仅处理 **retain_mode==normal**、**topic_source≠cooc**、**primary_score≥0.40** 的 primary，收紧 cluster，避免 JPEG 2000 / Compiler construction 等脏扩散。 |
| `expand_from_cooccurrence_support` | 仅处理 **retain_mode==normal**、**topic_source==direct**、**primary_score≥0.45** 的 primary，比 dense 更严。 |
...
3. **Identity Gate（锚点本义守门，软本义分）**  
   ...
   - **anchor_identity_score**：由 `compute_anchor_identity_score` 计算，**软本义分**：① 别名命中（ANCHOR_IDENTITY_ALIASES）给 **base_floor≥0.34**，中英 token 重叠/head 一致给 0.26～0.30 基础分；② score = max(base_floor, 0.40×exact_or_substring + 0.35×token_overlap + 0.25×head_consistency）；③ 泛词/歧义惩罚保留但放宽（generic_penalty 0.72、ambiguity_penalty 0.78），明显错义词（propulsion、simula、control flow、data retrieval 等）仍 0.45。  
...
| **Seed 定义** | **单一决策** `check_seed_eligibility(primary, jd_profile)` 返回 (eligible, seed_score, block_reason)。eligible = identity≥SEED_MIN_IDENTITY 且 source∈可信，且 **weak_retain 时仅 suppress_seed=False 可扩**；**topic_source==cooc** 须 semantic≥0.84、jd_align≥0.82、anchor_identity≥0.35；**is_semantic_mismatch_seed**（动力学→propulsion、抓取→retrieval/crawling 等）直接禁扩。准入后为 primary 打 **seed_blocked、seed_block_reason**。若无可选 seed 则保底取该锚点 primary 的前若干。 |
