...
└── src/
    ...
    │   │       ├── stage3_term_filtering.py  # … → select_terms_for_paper_recall（paper_select_score=final×readiness，readiness 含 Step2 锚点分/名次）→ cutoff / paper selection / near-miss audit
...
| **Stage3** | raw_candidates（含 term_role、role_in_anchor、source_types、retain_mode、**primary_bucket / fallback_primary / mainline_candidate**、**parent_anchor_final_score / parent_anchor_step2_rank** 等）, query_vector, anchor_vids | score_map, term_map, idf_map, **paper_terms** | 去重聚合 → classify_stage3_entry_groups → check_stage3_admission；**第一段**：score_term_record × identity × risk × **role_factor**；**`_assign_stage3_bucket`**（仅观测标签 core/support/risky）；**`stage3_build_score_map`**：**统一连续分**取代 bucket 乘子链 → **`_apply_family_role_constraints`**；**`select_terms_for_paper_recall`**：硬门后 **eligible 按 `paper_select_score`（= final×readiness，readiness 含锚点先验）排序** + **family 去重** + **`dynamic_floor`** + topN；仍硬挡 **单锚 conditioned_only**、**fallback_primary**；**`[Stage3 unified score breakdown]`**、**`[Stage3 paper cutoff]`**、**`[Stage3 paper selection audit]`**、**`[Stage3 paper near-miss audit]`**、**`[Stage3 support contamination]`** |
| **Stage4** | vocab_ids, regex_str, term_scores, term_retrieval_roles, term_meta, jd_text | author_papers_list（按作者聚合的论文及 hits） | 二层召回；**role_weight**（paper_primary=1.0、paper_support=0.7）；MELT_RATIO、domain 软奖励；**term-type 动态 grounding / contribution / local cap**；**canonical「robot control」在 Stage4 侧覆盖为 paper_support 并压低 paper_factor（umbrella 泛词，与 motion control 等细项区分）**；paper_scoring 在 Stage5 用；日志：**overlap survival** 优先高分存活 + cap 前 rank 靠前的淘汰项；**hierarchy consensus audit** 仅保留 `top_by_abs_score_delta` |
| **Stage5** | author_papers_list, score_map, term_map, debug_1（含 term_role_map、term_confidence_map、**term_family_keys**、**term_paper_meta**、**term_retrieval_roles**） | author_id 列表、last_debug_info（含 **stage5_term_cap_audit**、**stage5_author_structure_audit**） | **paper_scoring**；护栏 5；同词递减 → **`TERM_MAX_AUTHOR_SHARE`** → **time_weight** → **结构乘子**：强词阈 **0.35×max(term_contrib)**；**`structure_factor`** 分枝（单 term+单篇+mtp=0→0.62；st=1 且 mtp=0→0.78；st≥2 或 mtp≥1→1+0.08·min(mtp,3)；否则 0.90）再 × **(0.85+0.15·min(st,3)/3)** × **(0.80+0.20·min(paper,4)/4)** × **(1+0.12·min(mtp,3))**；AUTHOR_BEST_PAPER_MIN_RATIO；诊断：**`[Stage5 term-cap & structure audit]`**、**`[Stage5 author structure audit]`**、support dominance、term mix；CLI **合一作者榜** |
...
- **Stage3 全局复审层（非全杀门）**：去重聚合 → classify_stage3_entry_groups → check_stage3_admission → **第一段分** = score_term_record × identity_factor × risk_penalty × **role_factor**（同左 Role 因子）；**assign_stage3_bucket** 仍为 core/support/risky，**仅观测**；**第二段** **`stage3_build_score_map`**：改为 **统一连续分**（seed / anchor_identity / jd_align / family_centrality / cross_anchor / mainline / expand − drift / generic / poly / object），再 **`_apply_family_role_constraints`**；**select_terms_for_paper_recall**：**入 eligible 前**依次 **weak support 结构门**（`STAGE3_PAPER_SUPPORT_*`）→ **`risky_side_block`**（`STAGE3_PAPER_RISKY_SIDE_*`：`stage3_bucket=risky` ∧ ¬`can_expand` ∧ `mainline_hits≤0` ∧ **`term_role∈{primary_side,dense_expansion,cluster_expansion,cooc_expansion}`**），未过门分别 **`weak_support_contamination_block` / `risky_side_block`**；再过线候选计算 **`paper_select_score`**（见上）后按之排序 + **family_key** 去重 + **与 `paper_select_score` 对齐的** **`PAPER_RECALL_DYNAMIC_FLOOR_*`**；其余 **risky** 仍可入 paper；上限 **PAPER_RECALL_MAX_TERMS**（默认 12）。
...
- **Stage5 作者排序**：paper_scoring（护栏 5）→ **作者×词** → 同词递减 → **`TERM_MAX_AUTHOR_SHARE`** → **时间权重** → **高分辨率结构乘子**（避免弱 log 覆盖导致 Top 榜系数趋同）：强词 = 贡献 ≥ **0.35·max**；**structure_factor** 按 st/paper_count/mtp 四分枝；再显式连乘 **term_strength_mult**、**paper_evidence_mult**、**multi_hit_mult**（见表 3.1）；**最佳作比** → 归一化。`[Stage4 hierarchy bonus distribution]` 默认仅 **分位数摘要**；长列表仅当 **`STAGE4_HIERARCHY_BONUS_DISTRIBUTION_DETAIL`**。CLI **单块「Top 作者榜」**（含 struct× / st / mtp）。「Top term 最终贡献表」未进 paper 段 **8** 行。
...
| **`[Stage2B weak-seed audit]`** | **`check_seed_eligibility`**、`tier=weak` | **axis_consistency_seed**、**conditioned_only**、**eligible** 与 **block_reason**（降噪：同左 debug 开关） |
...
| **`[paper_term_selection gate]`** | **`_print_paper_term_selection_audit`**（`STAGE3_PAPER_GATE_DEBUG`，**默认 False 降噪**） | 逐项：`term_role`、`family_key`、`primary_bucket`、`anchor_count`、`mainline_hits`、`can_expand`、`paper_grounding`、**`risky_side_paper_block`**、`paper_support_gate` 与 cutoff 原因；**主裁决顺序**：**weak support 门** → **`risky_side_block`** → **`paper_select_score` + family + `dynamic_floor`**（全表排序传给该打印时优先带 **`paper_select_score`**） |
| **`[Stage3 unified score breakdown]`** | **`stage3_build_score_map`** 后（`STAGE3_UNIFIED_SCORE_DEBUG`） | 连续分项：seed / anchor_id / jd_align / fam_cent / cross / mainline / expand 与四类 risk → **final_score**；**bucket 仅标注观测** |
| **`[Stage3 paper cutoff]`** | **`select_terms_for_paper_recall` 末尾**（`STAGE3_PAPER_CUTOFF_AUDIT`） | **eligible 内**按 **`paper_select_score` 降序**：`dynamic_floor`（与 **`paper_select_score` 同尺度**）、`family_key`、`selected`、`cutoff_reason`（含 **`risky_side_block`** / **`weak_support_contamination_block`** / `below_dynamic_floor` / `family_dup_or_below_cut` / **`top_n_paper_select_score`** 等；列含 **final / paper_readiness / paper_select_score** |
| **`[Stage3 paper selection audit]`** | **同上**（与 cutoff 同一开关） | **窄表**：每条 eligible 的 **term、final_score、paper_readiness、paper_select_score**、结构列同上，并增 **panch（parent_anchor 缩写）、a_sc（`best_parent_anchor_final_score`）、rk（`best_parent_anchor_step2_rank`）**；用于对照「RL 的 ready 是否仍与 motion 一样」——**锚点分/名次应先拉开** |
| **`[Stage3 paper near-miss audit]`** | **同上** | **eligible 但未入选**：`paper_select_score` 贴近 **dynamic_floor** 或 **末席/top5 入选分** 的词；列 **Δ末席、a_sc、rk、reject_reason**，便于看「差一点也不进」的 term |
...
- **`select_terms_for_paper_recall`**：**eligible 构造阶段**硬挡 **单锚 conditioned_only**、**fallback_primary**；**weak support 结构门**（**`STAGE3_PAPER_SUPPORT_*`**）与 **`risky_side_block`**（**`STAGE3_PAPER_RISKY_SIDE_*`**：`risky` + 无扩 + 无 mainline + side/expansion **`term_role`**），命中则 **`paper_cutoff_reason`** 为对应枚举且不进 eligible。候选集合上 **`_apply_paper_readiness_for_recall`**：`paper_select_score = final_score * paper_readiness`。readiness 在 **bucket / mainline / 锚数 / 可扩 / conditioned_only / term_role / `_paper_grounding_score`** 之上，再乘 **锚点主线先验**：tid 合并字段 **`best_parent_anchor_final_score`**（多源 **`parent_anchor_final_score`** 的 max，来自 Step2 **`PreparedAnchor.final_anchor_score`**，经 **`stage2_expansion._expanded_to_raw_candidates`** 写入顶层）；**无分**时 **`best_parent_anchor_step2_rank`**（多源 **`parent_anchor_step2_rank`** 的 min，Step2 对 **`prepared_anchors` 枚举的 `step2_anchor_rank`**）用 **`1/(1+λ(rank−1))`** 退化加权（常量见 **`PAPER_READINESS_ANCHOR_*`**）。**无词表黑白名单**。**`_paper_recall_dynamic_floor(..., score_key="paper_select_score")`**；**`family_key` 去重**；**`PAPER_RECALL_MAX_TERMS`**。**`retrieval_role`**：bucket==core → `paper_primary`，否则 **`paper_support`**。（全局 Stage3 排序仍只用 **`final_score`**。）
...
| 优先级 | 改动 | 说明 |
|--------|------|------|
| **1** | **primary 资格 ≠ expand / seed 资格** | `judge_primary_and_expandability`：axis 初桶 + **无真实 conditioned** 时关扩。`select_primary_per_anchor`：**similar_to reconcile** + **`axis_consistency_seed` 与 Stage2B strong 同阈**（防 promote 过宽）+ **无 expandable 时 keep 只留 1 条**。 |
...
**建议日志**：`[Stage2A select]`、**`[Stage2A group rank]`**、**`[Stage2A promote audit]`**、**`[Stage2A final bucket reconcile]`**、`[Stage2A anchor bucket summary]`、`[Stage2A global bucket summary]`、`[Stage2A fallback]`、**`[Stage2] carryover_terms` / `[Stage2B input audit]`**（有 seed 时）、**无 seed：`[Stage2B no-seed reason]` + 单行 `[Stage2B] anchor=… merged=…`**、`[Stage2B seed gate]`（NOISY）/ **`[Stage2B seed tier audit]`**（仅 seed 候选）、**有 seed 时 `[Stage2B] anchor=… eligible…` 仅打一条**（已去掉与 `debug_print(2)` 的重复汇总）、**`merge_primary_and_support_terms carryover=…`**、`[Stage2B] diffusion=…`（`debug_print(1)` 保留）、`[Stage3 rerank summary]`、**`[Stage3 support subtype audit]`**、`[Stage3 bucket reason]` 中的 `bucket_reason_flags`。**`[Stage2A Commit Debug]`** 仅 **`STAGE2_NOISY_DEBUG=True`** 且 focus 命中时输出。

**相关常量**：`STAGE2A_CONDITIONED_SIM_EPS`、`STAGE2A_PRIMARY_MIN_SIM`、`STAGE2A_PRIMARY_MAX_CTX_DROP`、`STAGE2A_PRIMARY_MIN_CTX_SIM`、`STAGE2A_PRIMARY_DUAL_MIN_CTX`、`STAGE2A_PRIMARY_MULTI_MIN_FAMILY`、`STAGE2A_EXPAND_*`、`STAGE2A_EXPAND_SOLO_STRONG_CTX`、`STAGE2A_EXPAND_SOLO_MIN_FAMILY`、`STAGE2A_FALLBACK_*`、**`SEED_AXIS_CONSISTENCY_STRONG_MIN`**、**`STAGE2A_PROMOTE_MIN_AXIS_GAP`**（组内 1/2 名 axis_seed 最小间隔，防并列误抬）、`STAGE3_SUPPORT_DEMOTE_*`。
...
| **`select_primary_per_anchor`** | **`judge_*` 之后**：**组排序 + final reconcile**；**keep/sd→exp** 须 **top1 + axis 间隔 + 强轴 + 无 judge 稳定强 exp**；调试 **`[Stage2A group rank]`**、**`[Stage2A promote audit]`**。**`STAGE2_RULING_DEBUG` 开时不再重复** **`[Stage2A primary/expand split]`**。 |
...
- **select_primary_per_anchor**：返回 **五列表** + **`primary_keep_no_expand`**；**前置硬拒绝仅** `retain_mode=="reject"`；**obviously_bad_branch / drift_bad / polysemous_no_context** 仍前置 reject；**`judge_primary_and_expandability` + similar_to 主线 reconcile**；**无 expandable 时 keep 仅 top1**。
...
- **3）Stage3 第二段 + paper 选词（现行）**
  ...
  - **`select_terms_for_paper_recall`**：**weak support 结构门** + **`risky_side_block`**（side/expansion 尾词）→ **`paper_select_score` 全局排序（eligible 内）** + **`family_key` 去重** + **`dynamic_floor`（与 paper_select_score 同尺度）**；单锚 conditioned_only、**fallback_primary** 仍不进；其余 **risky** 仍可入 paper（由 **paper_select_score** 与截断决定）。
...
- **Stage3 全局复审层**：classify_stage3_entry_groups → check_stage3_admission；**第一段** = score_term_record × identity_factor × risk_penalty × **role_factor**（系数同前）；**`stage3_build_score_map`** 用 **统一连续分** 覆盖 **`final_score`**（**已移除** bucket 乘子链），再 **`_apply_family_role_constraints`**；**select_terms_for_paper_recall**：**weak support 门** + **`risky_side_block`** 后再 **`paper_select_score`（含锚点主线先验）** + **family** + **dynamic_floor**；**其余 risky** 仍可入 paper。日志含 **[Stage3 unified score breakdown]**、**[Stage3 paper cutoff]**、**[Stage3 paper selection audit]**、**[Stage3 paper near-miss audit]** 等。
...
| **函数 2：paper** | **conditioned_only** ∧ **单锚** → **`conditioned_only_single_anchor_block`**。**is_fallback_primary** → **`fallback_primary_block`**。**weak support 门** → **`weak_support_contamination_block`**。**`risky_side_block`**：`risky` ∧ ¬扩 ∧ `mainline_hits≤0` ∧ **`term_role` 为 side/expansion**。再 **`paper_select_score` 排序**，**`family_key` 去重**，**`dynamic_floor`（同尺度）** 截断，**topN**；**`retrieval_role`**：core→`paper_primary`，否则 **`paper_support`**。 |
...
| **调试审计（可选）** | **`STAGE3_AUDIT_DEBUG`**：**`[Stage3 final adjust audit]`** … **`[Stage3->Paper bridge]`**。**`STAGE3_UNIFIED_SCORE_DEBUG`**：**`[Stage3 unified score breakdown]`**。**`STAGE3_DUPLICATE_MERGE_AUDIT`**：**`[Stage3 duplicate merge audit]`**。**`STAGE3_CORE_MISS_AUDIT`**：**`[Stage3 core-miss audit]`**（**`primary_expandable` 却非 core**，对照 identity/jd/**`stage3_core_miss_reason`**）。**`STAGE3_PAPER_CUTOFF_AUDIT`** … **`DEBUG_LABEL_PATH`**：**`[Stage3 rerank summary]`**。 |
...
| **思路** | **compute_contribution** 在累加 **rank_score / term_weights** 时：对每条 hit 读 Stage4 **`role`**（`paper_primary` / `paper_support`）。**paper_primary** 或缺省 role → 系数 1；**paper_support** → 若该 wid **同时存在 paper_primary 命中**则乘 **`STAGE5_SUPPORT_WITH_PRIMARY_FACTOR`（辅证）**；若该 wid **仅有 paper_support** 则乘 **`STAGE5_SUPPORT_SOLO_FACTOR`（独狼大减）**；support 再乘 **`min(1, 0.5+0.25·min(mainline_hits,3))`**（`mainline_hits` 来自 `debug_1.term_paper_meta`）。使 *robot control / q-learning* 等合理 support 仍可进池，但**不能单凭一篇「无主线词」论文把作者抬到与 motion control 同工**。再经领域纯度、综述/覆盖/紧密度、JD 语义门得 paper score；**护栏 5** 仍生效；作者侧 term 递减聚合、time_weight、concentration/diversity、过滤归一化同前。**`[Stage5 support dominance audit]`**：Top 作者上 **`sup_share` / `sup_only_papers` / `dominant_term_role`** 一眼辨认是否 support 统治。 |
...
| **主要公式** | 先算 **`w_raw` = term_weight × term_confidence × paper_match_strength × term_uniqueness**（含义同前）；若 hit **`role==paper_support`** 则 **`w = w_raw × f_attach`**（同 wid 有 **`paper_primary`**→`STAGE5_SUPPORT_WITH_PRIMARY_FACTOR`×mainline taper；仅 support→`STAGE5_SUPPORT_SOLO_FACTOR`×mainline taper），否则 **`w=w_raw`**。**rank_score**=Σw；**论文得分**=rank_score×coverage_norm×cluster_bonus×proximity×domain×time×survey×jd_gate。**护栏 5**、作者递减聚合、**分布校正**同前。详见 **`paper_scoring.py`** 中 **`STAGE5_SUPPORT_*`**。 |
...
- **审计（wid 三级共识）**：`[Stage4 hierarchy consensus audit]`  
  两节：**按乘 bonus 前 paper_score 前 10**、**按绝对分差 abs(score_after−score_before) 前 10**；字段含 **field_cons/subfield_cons/topic_cons**、**median_consensus**、**delta_vs_median**、**hierarchy_bonus**、**paper_score_before/after**。
...
- **Stage3**：幸存词打 **stage3_bucket**（**观测**）与 **retrieval_role**（paper_primary/paper_support，由 bucket 软映射）；**select_terms_for_paper_recall**：**paper 结构门**（含 **`risky_side_block`**）后再按 **`paper_select_score`** 排序 + **family 去重** + **dynamic_floor**，上限 **PAPER_RECALL_MAX_TERMS**（默认 12）；**未命中 side/expansion risky 拦截的 risky** 仍可入选。  
...
| **STAGE3_PAPER_SUPPORT_BLOCK_ENABLED** 等（stage3_term_filtering） | 默认 True / 1 / 1 / 0.62 / 0.30 / True | **Paper bridge 弱 support 结构门**：`MAX_ANCHOR`/`MAX_MAINLINE`、**MIN_GROUNDING**（与 `_paper_grounding_score` 同尺度）、**MIN_FINAL_SCORE**（与低 grounding 联用）、**BLOCK_COND_ONLY**；关闭门将回退为仅靠统一分+floor |
