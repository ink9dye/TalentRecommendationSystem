...
- **[目录树](#目录树)**
- **[整体架构与数据流](#整体架构与数据流)**
- **[标签路、总召回与 KGATAX 同步说明](#标签路总召回与-kgatax-同步说明)**
- **[知识图谱构建详解](#知识图谱构建详解)**
...
└── src/
    ...
    │   │   ├── vector_path.py      # 向量路：query bundle → 多路 dense（Faiss）→ 轻量 hybrid → 子句补充 → 论文聚合作者
    │   │   ├── vector_query_builder.py # 向量路 Step1：JD 多视角文本 bundle（与标签路概念落点正交）
    ...
    │   │       ├── stage1_domain_anchors.py   # 领域与锚点；JD 粗分+refine 分层、Stage1Result、build_jd_hierarchy_profile（批量 SIMILAR_TO）、conditioned_vec
    │   │       ├── stage2_expansion.py       # run_stage2 返回 dict（四键）；主链将整份 stage2_output 交给 run_stage3，合并侧仍以 all_candidates 为扁平行
    │   │       ├── stage3_term_filtering.py  # run_stage3：Global Coherence Rerank；主链终点 ranked_terms；prep→paper_terms / global_coherence_report（与 Step0 边界说明一致）
...
4. **KGAT‑AX 训练数据与图索引构建**
   - `kgat_ax/generate_training_data.py` 的训练样本入口应优先基于总召回输出的候选中间层与训练池视图：  
     - `candidate_pool.candidate_records`（base candidate layer，候选中间层主表）  
     - `candidate_pool.training_pool_records`（training pool，训练用候选视图）  
     - `candidate_pool.candidate_evidence_rows`（多路证据明细）  
     在此基础上由 teacher / 弱监督规则 / 少量人工标注进一步构造分层正负样本（Strong/Weak Positive、EasyNeg/FieldNeg/HardNeg/CollabNeg），产出 `train.txt` / `test.txt` 与可选四分支侧车 `train_four_branch.json` / `test_four_branch.json`，并将全图导出为加权三元组 `kg_final.txt`。  
     说明：返回字典中的 `final_top_200` / `final_top_500` 为 **deprecated / compatibility only（legacy field）**，不再代表真实规模，不应作为 README 主体中的正式训练入口口径。
...
6. **线上召回与精排**
   - **总召回**（`core/recall/total_recall.py`）：构建可复用的**候选中间层**（candidate pool / recall pool），并在该中间层上派生两类池视图：  
     1）**base candidate layer**：三路召回并集后的候选中间层（去重 + 轻量融合 + 特征补全 + 最小必要过滤 + 分桶），**不是最终答案列表、不是训练标签、也不是最终真值**；对应 `candidate_pool.candidate_records`。  
     2）**training pool**（默认最多 **100**）：服务训练样本构造，优先保留来源多样性、边界候选与可分层样本；对应 `candidate_pool.training_pool_records`（训练池视图，不追求榜单观感，且**不等于** `base[:100]` 的机械前缀）。  
     3）**display pool**（默认最多 **50**）：服务线上展示 / 精排输入，继续沿用分桶与截断逻辑，更强调稳定性与结果质量；对应 `candidate_pool.display_pool_records`。  
     兼容字段：返回字典中的 `final_top_200` / `final_top_500` 为 **deprecated / compatibility only（legacy field）**，**不再代表真实的 200/500 规模**，当前语义与 **display pool** 的 author_id 列表一致（最多 50）。
   - **精排**（`core/total_core.py` → `RankingEngine`）：**仅作用于候选池内部**。对候选作者计算 KGAT‑AX 分，并与候选池分、`rule_stability` 做归一化加权融合（默认 `λ_pool=0.2`、`λ_kgat=0.6`、`λ_stability=0.2`，见 `ranking_engine.py`），产出最终 Top 100 及解释。即：**总召回输出候选池 → 精排在池内重排**，二者是**上下游关系**。精排阶段可理解为：**Re-ranker（候选池内排序器），不负责全库检索（not retriever）**。
...
## 标签路、总召回与 KGATAX 同步说明

**本节回答：**

- 标签路、总召回、KGAT‑AX 各自负责什么？主代码文件在哪里？
- 一次查询从编码到候选池再到精排/训练，**数据如何衔接**？
- `TotalRecallSystem` 与单独运行 `label_path.py` / `vector_path.py` 时行为有何**对齐点与差异**？

### 三者关系概览

| 模块 | 职责 | 主入口 / 核心文件 |
|------|------|---------------------|
| **标签路** | 基于图谱词汇、岗位锚点、多阶段筛选与论文层召回，产出带 `label_evidence` 的作者排序 | `src/core/recall/label_path.py`（`LabelRecallPath`）；流水线实现于 `label_pipeline/stage1`～`stage5`；资源与统计见 `label_means/`、`label_means/infra.py` |
| **总召回** | 统一编码；**并行**向量路 + 标签路；再以 V/L Top 种子跑协同路；合并为 `CandidateRecord` 列表，经 RRF 融合、特征补全、硬过滤、分桶、配额截断；**不负责全库最终排序** | `src/core/recall/total_recall.py`（`TotalRecallSystem`）；依赖 `vector_path.py`（内部用 `vector_query_builder`）、`collaboration_path.py`、`candidate_pool.py`、`candidate_features.py` |
| **KGAT‑AX** | 在**候选池内**用图注意力 + 学术特征做深度打分；训练阶段用召回侧车特征；与总召回通过 **`build_kgatax_feature_row` → 侧车行** 对齐 | 推理编排：`src/core/total_core.py` + `ranking/ranking_engine.py` + `ranking/rank_scorer.py`；训练与数据：`src/infrastructure/database/kgat_ax/`（`generate_training_data.py`、`trainer.py`、`model.py`、`data_loader.py`）；配置：`config.KGATAX_TRAIN_DATA_DIR` |

```mermaid
flowchart LR
  Q[JD 文本] --> ENC[QueryEncoder 编码]
  ENC --> DD[DomainDetector 推断领域集合]
  ENC --> V[VectorPath 召回]
  ENC --> L[LabelRecallPath 召回]
  V --> MERGE[build_candidate_records + RRF 等]
  L --> MERGE
  V --> SEEDS[V/L 各前100 author 并集]
  SEEDS --> C[CollaborationPath 召回]
  C --> MERGE
  MERGE --> ENRICH[enrich + 硬过滤 + 分桶 + 配额截断]
  ENRICH --> ROWS["kgatax_sidecar_rows = build_kgatax_feature_row(每条记录)"]
  ENRICH --> POOL[CandidatePool]
  ROWS --> TRAIN[训练: 四分支样本 / 侧车 JSON]
  POOL --> CORE[TotalCore.suggest → RankingEngine]
  CORE --> OUT[Top-K + kgat_score 等]
```

### 标签路（LabelRecallPath）

**逻辑要点（与 `label_path.py` 文档串一致）：**

1. **领域与上下文**：用查询向量做 Job/锚点侧领域探测（可与 `DomainDetector` 共享同一套 Job 索引 + Neo4j 统计）；产出 `active_domains`、锚点与扩展上下文。
2. **五阶段流水线**：`stage1_domain_anchors`（领域与锚点准备；含 JD 粗分与 **`refine_stage1_anchor_layers`**）→ `stage2_expansion`（**`run_stage2` 结构化 dict**）→ `stage3_term_filtering`（**`run_stage3` 接收整份 `stage2_output`，内部以 `all_candidates` 驱动合并/双闸门；返回结构化 dict 与 `global_coherence_report`**）→ `stage4_paper_recall`（论文层召回与证据打分）→ `stage5_author_rank`（作者层聚合、排序与截断）。
3. **依赖数据**：Neo4j（`Work`/`Author`/`Vocabulary`/`Job` 及 `SIMILAR_TO`、`REQUIRE_SKILL` 等）、Faiss（词汇/岗位索引）、`vocab_stats.db`（共现与领域统计等）。

**与总召回的同步：**

- 总召回内实例化：`LabelRecallPath(recall_limit=k_label, silent=True)`，**静默模式**下减少控制台输出，避免与向量/协同路并行时刷屏；逻辑与单独运行 `label_path.py` **同一套 `recall()`**。
- 总召回向标签路传入：`query_vector`（已与向量路一致的归一化向量）、`domain_id`（用户显式领域）、`query_text` / `semantic_query_text`（原始 JD），保证与独立调试时输入一致。
- 标签路返回的 meta 中带 `label_rank`、`label_evidence` 等，在 `build_candidate_records` 中写入 `CandidateRecord`，并供 `extract_terms_from_label_evidence` 解析 core/support/risky 词数，进入 RRF 的 `label_hint` 与硬过滤。

**独立运行：** 直接执行 `python src/core/recall/label_path.py` 便于看全阶段耗时与「必查」日志；总召回不单独打印标签子阶段，除非改 `silent` 或调 `LabelRecallPath` 的 verbose。

### 总召回（TotalRecallSystem）

**执行顺序（`execute`）：**

1. `QueryEncoder.encode` 得到 `raw_vec` 并 L2 归一化。
2. `DomainDetector.detect(raw_vec, query_text, user_domain)`：用户显式非 `0`/空时**仅用该领域**；否则自动推断 `active_domains`，并生成 `vector_domains` 字符串供向量路过滤。
3. **非训练模式**下可将 `query_text` 拼接领域 prompt 为 `final_query` 再编码为 `query_vec`；**训练模式** `is_training=True` 时跳过领域 prompt 二次编码，与 `generate_training_data` 中多进程召回对齐，避免重复编码与分布漂移。
4. **线程池并行**：`v_path.recall(query_vec, target_domains=vector_domains, query_text=query_text)` 与 `l_path.recall(...)` 同时执行（向量路用 **`query_text` 构建多视角 query bundle**，**`query_vec` 作为 raw 分支向量**；与标签路共用同一 `query_text`/`semantic_query_text`）；完成后取 **`v_meta` 与 `l_meta` 各自前 100 条**中的 `author_id` 做并集作为 **协同路种子**，调用 `c_path.recall(seeds)`（协同路依赖种子，故晚于 V/L 完成）。
5. 合并去重：`build_candidate_records` 形成 `CandidateRecord`（一人一条，保留三路来源与 evidence）。
6. 轻量融合粗排：`score_candidate_pool` 计算 **coarse prior / light fusion score**（`candidate_pool_score`），用于候选中间层的粗排序（不是最终精排分、不是训练标签）。
7. 特征补全与最小必要过滤：`_enrich_candidate_features`（SQLite 作者指标、代表作与 query 交叉特征）→ `_apply_hard_filters`（Step3：仅真硬过滤删除明显垃圾；边界候选转为 `risk_flags` 软标记保留）→ `_assign_buckets`（A～F）。
8. 形成三层输出：  
   - **base candidate layer**：`candidate_pool.candidate_records`（候选中间层主表）  
   - **training pool**：`candidate_pool.training_pool_records`（训练用视图，默认最多 100；Step5 已与 base 前缀分叉，强调来源多样与边界候选机会）  
   - **display pool**：`candidate_pool.display_pool_records`（展示/精排输入视图，默认最多 50；仍沿用分桶截断逻辑）
9. 对 base candidate layer 生成 **`kgatax_sidecar_rows`**，并封装 `CandidatePool`（含 `path_costs`、`domain_debug` 与 `pool_debug_summary`）。  
   兼容字段：返回字典中的 `final_top_200` / `final_top_500` 为 **deprecated / compatibility only（legacy field）**，当前语义与 display pool 的 author_id 列表一致（最多 50），不再代表真实规模。

**与单路测试的对比：**

| 项目 | `vector_path.py` / `label_path.py` 单跑 | `total_recall.py` |
|------|----------------------------------------|-------------------|
| 编码与领域 | 各脚本内自行选领域与打印 | 统一 `DomainDetector` + 可选领域 prompt |
| 协同路 | 无 | 有，且依赖 V/L 种子 |
| 融合分 | 单路作者分 | `candidate_pool_score`（候选中间层 coarse prior / light fusion score） |
| 下游 | 仅展示列表 | `kgatax_sidecar_rows` + `CandidatePool` 供精排/训练 |

**默认常量（与 `total_recall.py` 对齐，修改代码时请同步本文）：** `k_vector=150`、`k_label=150`、`k_collab=80`；协同路种子为 **`v_meta` / `l_meta` 各自前 100 条**的作者并集；`TRAINING_POOL_TOP_N=100`、`DISPLAY_POOL_TOP_N=50`；`RRF_K=60`；`PATH_WEIGHTS` 为 `v:l:c = 2:3:1`；`BUCKET_QUOTAS` 见源码字典。  
补充：`FINAL_POOL_TOP_N` 为历史常量/兼容保留（deprecated），当前版本以 training/display 的上限作为主要口径。

### KGAT‑AX 与总召回的衔接

**侧车特征（与代码一一对应）：`candidate_features.build_kgatax_feature_row`**

- **`recall_features`**：三路布尔、`path_count`、`bucket_type`、各路径 rank、`candidate_pool_score`。
- **`author_aux`**：h_index、works_count、cited_by_count、近期发文/引用、机构层级、`top_work_quality`。
- **`interaction_features`**：`topic_similarity`、`skill_coverage_ratio`、`domain_consistency`、`paper_hit_strength`、`recent_activity_match` 及标签词统计（含 `label_best_term_score`）。

总召回返回值中的 **`kgatax_sidecar_rows`** 即上述结构的列表，供 `KGATAXTrainingGenerator` 等导出四分支训练行；线上 **`TotalCore`** 加载 `KGAT` 权重与 `DataLoaderKGAT`，将 `CandidatePool` 交给 **`RankingEngine.execute_rank`**（候选池预排序 → `RankScorer` 计算 KGAT/四分支分数 → 与 `candidate_pool_score`、稳定项 `rule_stability` 加权融合）。

**训练侧同步要点：**

- `generate_training_data.py` 内调用 `TotalRecallSystem.execute(is_training=True)` 批量生成样本；可通过环境变量 **`KGATAX_RECALL_WORKERS`** 控制多进程并行召回。
- 权重与图数据路径统一由 **`config.KGATAX_TRAIN_DATA_DIR`**（默认 `data/kgatax_train_data/`）指向；`trainer.py`、`build_kg_index.py` 与该目录对齐。
...
本节从**核心目的**与**为达成目的所使用的方法**两方面，说明「文本转向量」以及「向量路 / 标签路 / 协同路」三路召回的详细过程。对应代码：`src/core/recall/input_to_vector.py`、`vector_query_builder.py`、`vector_path.py`、`label_path.py`、`collaboration_path.py`、`total_recall.py`。
...
**核心目的**  
走 **paper-first 的 dense retrieval 主链**：先把 JD 组织成多视角查询（query reformulation / multi-query），再在摘要向量索引上做 **双塔式** 语义检索（与 DPR 同类：query/doc 独立编码、余弦≈内积）；论文层做 **多路 dense 融合**、**轻量 dense+lexical hybrid**（术语级命中为加性修正，有上界）、可选 **子句浅层补充检索**；最后将论文级得分聚合到作者。与标签路分工：**不做**图谱式概念 grounding，**不**承担全局一致性重排的主责。

| 维度 | 说明 |
|------|------|
| **输入** | `query_vector`（上游对 JD 的一次编码，作为 **raw_query** 分支向量）、`target_domains`（可选）、`query_text`（原始 JD；**不传则仅 raw 分支参与多路检索**，其余分支无独立编码） |
| **依赖** | `vector_query_builder.build_vector_query_bundle`（Step1 文本准备）；摘要 Faiss（`ABSTRACT_INDEX_PATH`）与 id_map；`input_to_vector.QueryEncoder`（多路 query 批量编码）；主库 `works` / `authorships`；`works_to_authors.accumulate_author_scores` |
| **输出** | 作者 meta 列表：`author_id`、`vector_rank`、`vector_score_raw`、`vector_evidence`（含 `top_papers` 与 summary，反映多路命中与 hybrid 信息）；耗时 |
| **核心中间产物** | 每路 query 各检索 **search_k=500** 篇（合并去重后按 `fused_dense_score` 取前 500 篇进入后续）；`merged_records` 中 per-paper 的各分支分数字段、`paper_clause_aware_score`（hybrid + 子句 bonus 后的论文主分） |
| **主要局限** | 仍依赖摘要质量；术语级信号来自 **标题词面** 与 query 动态抽词，不是标签路的概念证据链 |

**管线（与代码 Step 编号一致）**

1. **Step1 — Query bundle（`vector_query_builder.py`）**  
   将 JD 整理为 **`raw_query` / `compressed_query` / `task_focused_query` / `method_focused_query` / `clause_queries`**：分段去噪、任务/方法视角拆分、子句列表预留。这是 **纯文本层** 的多视角组织，与标签路的 bucket/概念落点 **正交**。

2. **Step2 — Multi-query dense 检索与融合**  
   - 对 bundle 中各分支分别编码（`raw_query` **复用** 入参 `query_vector`，避免重复 encode）；内容相同的向量只 Faiss **search 一次** 再复制到各分支。  
   - 每路在摘要索引上 **`search_k=500`**，得到 `query_type → {work_id: similarity}`。  
   - 按论文合并多路命中，**加权融合** `fused_dense_score`（启用分支的权重和为分母，避免未启用分支稀释 raw）：`raw_query` **0.55**、`compressed_query` **0.20**、`task_focused_query` **0.15**、`method_focused_query` **0.10**；多分支同时命中另有 **温和 bonus**（上限 0.05）。

3. **Step4（实现上先于 Step3 hybrid）— 子句浅层补充检索**  
   从 `clause_queries` 中筛选最多 **6** 条有效子句（长度等阈值见源码 `_CLAUSE_*`）；每子句独立 Faiss 检索，深度 **`_CLAUSE_SEARCH_K=110`**，命中并入 `merged_records`；仅被子句拉入、未出现在多路 dense 并集中的论文以 **较低 synthetic fused_dense** 进入候选，用于补覆盖。**不替代** Step2 主路。  
   - 子句合并完成后，按 `fused_dense_score` 排序取 **前 500** 个 `work_id`，再查 SQLite 取标题/领域等，进入 Step3 hybrid。

4. **Step3 — Paper 层轻量 hybrid（dense 主干 + 有界 surface bump）**  
   载入 `works` 标题等元数据之后：从 query bundle **动态抽取** `lexical_terms` / `acronym_terms` / `method_tool_terms`，在论文 **标题** 上计算覆盖率等 surface 特征；在 `fused_dense_score` 上叠加 **加性修正**（lexical / acronym / method_tool / **multi-query consistency** 四项权重见 `vector_path.py` 中 `_HYBRID_W_*` 与 `_HYBRID_BUMP_CAP`），得到 `paper_hybrid_score`，再叠加 **clause_bonus** 得到 **`paper_clause_aware_score`**，并 **按该分重排** 论文序列。Hybrid 为 **工程版 dense+lexical 桥**，不替代标签路的概念对齐。

5. **领域过滤、时间/文本衰减与论文 → 作者**  
   对排序后的论文序列：`target_domains` **硬过滤**（`DomainProcessor`）；结合配置做时间衰减、文本衰减等（与 `get_decay_rate_for_domains`、`apply_text_decay`、`compute_paper_recency` 等一致）；再经 **`accumulate_author_scores`** 聚合到作者，去重截断至 **`recall_limit`**（`TotalRecallSystem` 默认 **150**）。作者顺序仍体现「靠前的高分论文」优先。

**向量路的职责边界（与标签路、纲领文档一致）**

- **Vector 路**：**高召回、宽语义覆盖、表述变体吸收**；以 **摘要 dense 检索** 为主干，辅以 **多 query + 轻量 hybrid + 子句补充**，输出 **`vector_evidence`** 供总召回与弱校准。职责是 **现代检索器边界**，不是第二条概念图谱路。  
- **Label 路**：锚点 / 学术概念落点、全局一致性、论文证据链与作者侧强解释。  
- **Collaboration 路**：基于 V/L 种子的结构扩展，不主导语义。  

向量 evidence 在总召回中：**保留来源与论文级证据**，经 `extract_vector_evidence_summary` / `apply_vector_evidence_bonus_for_candidate` 可做 **极小** `candidate_pool_score` 校准；**不可替代**标签路 `label_evidence`，也不作为训练标签或最终真值。
...
从系统定位上看，标签路结合了多种成熟信息检索与知识图谱系统的设计思想：一方面，它继承了 **Expert Finding / Expert Search（专家发现 / 专家检索）** 文献中「需求解析 → 主题或概念映射 → 文献证据聚合 → 专家排序」的基本链路；另一方面，它也借鉴了 **Candidate Generation + Rerank（候选生成 + 重排）** 的二段式 IR 架构，将「高召回的候选生成」与「强调主线一致性的全局重审」分开处理。Stage2A 的学术落点与 **Entity Linking / Wikification（实体链接 / 维基化）** 中将 **mention** 对齐到知识库 **entity** 的消歧问题同族；Stage3 则显式承担 **collective / global coherence（集体一致性、文档级全局一致性）** 重排——即候选是否正确不仅取决于对单个锚点的 **local scoring（局部打分）**，还取决于多锚点、多概念在同一 JD 下的 **joint plausibility（联合可解释性）**，这与 **collective entity linking（集体实体链接）** 文献中的思路一致（典型工作常采用局部候选 + 全局一致性特征或全局优化形式）。

##### 文献综述与术语谱系：11 篇论文与「五阶段标签路」概念模型

项目对照的内部综述文档《高端科技人才推荐系统五阶段标签路的系统性对比分析：11 篇论文综述》将相关文献归纳为 **11 篇**，其中约 **6 篇**侧重 **Wikipedia / 通用 KG 上的实体链接、相关性度量与集体推理**，约 **5 篇**补充 **短文本 EL、异构图专家发现、Learning-to-Rank 专家搜索** 等，用以共同覆盖 **JD → 学术概念 → 论文 → 作者** 的全链路。综述的核心判断是：**单独沿用 Wikipedia 式 EL** 无法覆盖「基于概念召回论文」与「论文聚合到作者」的异构图检索需求；**单独沿用 Expert Finding** 又往往假设查询已是明确主题词，难以处理 JD 中技能短语的稀疏上下文与歧义，因此需要 **互补技术栈** 拼成完整标签路（见该综述 §A.5、§F）。

下表给出综述中的 **标准五阶段** 命名、在 NLP / IR 文献中的**典型术语出处**（便于写论文「相关工作」时引用），以及 **本仓库** 的对应关系。**说明**：术语列标识的是**思想谱系与常见英文称呼**，不表示本仓库逐行复现某一篇文章的网络结构或数据集；**以 `label_path.py` + `label_pipeline` 代码为准**。

| 综述 Stage | 概念层职责（综述用语） | 典型术语与文献思想（出处类型） | 本仓库主要落点 |
|------------|------------------------|--------------------------------|----------------|
| **Stage 1** | 从 JD 得到应用侧 **anchor / 技能短语** | **Mention detection / span detection**：经典 EL 常假设 mention 已由 NER 给出；岗位场景下等价于技能短语抽取与边界确定 | `stage1_domain_anchors`：领域探测、`extract_anchor_skills`、锚点分层、`jd_profile` 等 |
| **Stage 2A** | **Local landing**：将 anchor **对齐到 KG 中学术概念** | **Entity linking / Wikification** 的消歧子问题；**短文本 EL** 强调多粒度匹配（字符 / 词 / **关系级 KG 邻居**，如 CIKM 类工作）；**神经符号 EL（LNN-EL）** 用一阶逻辑规则融合先验、字符串与上下文（AKBC 类） | `label_expansion` Stage2A：SIMILAR_TO、向量与别名、primary 桶与候选池组织 |
| **Stage 2B** | **保守扩展**：围绕已落地主概念增补邻近概念 | **Controlled query / concept expansion**；**局部嵌入、概念层次引导** 以抑制过度泛化（异构图专家查找文献中常见表述） | Stage2B：仅对合格 **seed** 做 dense / cluster / co-occurrence **小半径**扩族 |
| **Stage 3** | **Global coherence rerank**：概念级集体重排、分桶、轻过滤 | **Ranker–Linker**（先局部候选、再全局一致性特征）；**集体实体链接**（全局优化 / 线性规划 / 图传播）；全局一致性常用 **NGD、PMI、共现** 等（综述中常与 GLOW、集体标注类工作对照） | `stage3_term_filtering.run_stage3`：跨锚统一打分、**paper-term** 选择、主轴门与配额等 |
| **Stage 4** | 由入选概念 **召回论文证据** | 异构图上的 **(k,P)-core**、**PG-Index** 式高效近邻检索、SciBERT 微调等（数据库 / 专家查找合流）；亦可用 **局部嵌入** 作论文侧表示 | `stage4_paper_recall`：论文层召回与多因素打分（实现细节见下文数据流表） |
| **Stage 5** | 将论文证据 **聚合到作者** 并排序 | **Zipf 式**署名权重；**Learning-to-Rank**（如 SVMrank、LambdaMART）融合文本、档案与图特征；**Expert search in digital libraries** 问题设定 | `stage5_author_rank`、`paper_scoring`、作者贡献累积与截断 |

**综述中的知识图谱抽象**（便于与文献符号对齐）：节点类型 **Job**、**Vocabulary（学术概念）**、**Work（论文）**、**Author**；边类型 **REQUIRE_SKILL**（Job→Vocabulary）、**HAS_TOPIC**（Work→Vocabulary）、**SIMILAR_TO**（Vocabulary↔Vocabulary）、**WRITE**（Author→Work）。本仓库以 Neo4j 图谱 + `vocab_stats` 统计、簇与共现索引等实现该抽象的工程子集，边名与属性以代码及配置为准。

**工程与文献的边界**：综述 §G「工程代码职责映射」强调 **单向依赖、层内可替换**；本 README 下文「Stage1～Stage5」小节的 **字段名、闸门与审计日志** 描述的是 **当前仓库行为**。若需写「本文方法 vs 某篇论文」，建议采用「职责对齐 + 实现差异」的表述，避免将某一条 Cypher 或超参数直接说成某篇论文的原始贡献。

基于上述定位，标签路坚持如下**实现口径**：**局部语义**（SIMILAR_TO、conditioned_vec、alias / exact 等）负责**候选准入**与早期组织；**主线一致性**（Stage3 全局重排、paper-term 闸门、Stage4/5 证据闭环）负责**主词与专家序**的最终形态。最终谁成为 primary、谁保持 keep_no_expand、谁标为 risky，不由单点相似度单独决定，而由与整份 JD、多锚点及论文—作者证据链的一致性共同约束，以降低短词、泛词与跨学科同形词带来的 **semantic drift（语义漂移）**。

#### 标签路五阶段定位（与主召回流水线一致）

标签路不是单次图查询，而是一条按「锚点准备 → 学术落点与扩展 → 词级全局收束 → 论文验证 → 作者聚合」展开的多阶段流水线；**阶段编号 Stage1～Stage5 与上文综述表一致**，由主召回入口按顺序执行各阶段模块（实现集中在 `label_pipeline` 包与标签子模块中）。

以下按阶段说明**数据如何流入、中间发生什么决策、流出什么给下一阶段**。表述侧重过程与机制，不逐条罗列内部函数名；与字段级契约、参数闸门相关的硬核说明仍保留在后文表格与「正确分层图」中。

##### Stage1：领域与锚点准备

**输入**：JD 全文、查询向量（与向量路同一套编码）、可选领域提示。

**过程概要**：

1. **领域探测**  
   用岗位向量在离线建好的「岗位索引」上做近邻检索，结合图谱中岗位的领域标注，得到本次查询的 **活跃领域集合**（可多域），并生成用于后续文本匹配的 **领域正则 / 关键词模式**。其作用是缩小语义空间、降低跨领域漂移，而不是在 Stage1 就硬删 JD 内容。

2. **技能清洗与工业锚点抽取**  
   先对 JD 做技能短语级清洗（去掉明显 HR 套话、过短片段等），再在「岗位—技能」图谱约束下抽取 **工业锚点**：只保留在知识库中确有对接的技能词，按频次与覆盖率做熔断（例如全库过于常见的词不当作强锚点），必要时用 **JD 向量直接检索词汇表** 补充若干「语义上贴近但字面未写出」的补充锚点。补充锚点在分层上通常弱于图谱直出锚点，避免与主锚点同量级抢权重。

3. **早期粗分型（轻标签，非终判）**  
   在锚点进入后续落点之前，对 JD 里出现的词做一层 **粗分角色**（例如偏核心概念、偏工具框架、偏背景对象、偏纯上下文等）。这是给后续层级用的 **软参考**，不是 Stage2 的准入终判；少数启发式会修正明显矛盾（例如完整技能短语不应被标成无意义泛词）。

4. **锚点分层与分数带**  
   在粗分与来源（图谱直出 / 向量补充等）基础上，把锚点划成 **主锚 / 辅锚 / 仅上下文**：主锚参与主线竞争与扩展资格；辅锚保留证据但默认不抬到主叙事；仅上下文的短语会从主锚集合中 **侧挂** 到单独容器，仅供审计或极轻量下游读取，避免与主锚同池抢 primary。同时为每个锚点计算 **综合锚点分**（含主线重排、辅层压缩、向量补锚封顶等），使主辅在数值带上拉开，减少后文「泛词劫持主线」。

5. **上下文与条件化语义**  
   为每个锚点截取 **局部上下文窗口**（JD 中围绕该词的一小段），并构造 **条件化查询向量**（把锚点语义与整段 JD 语义做可控融合），供 Stage2 在「局部像不像」与「全局像不像」之间折中。

6. **层级与近邻画像（JD profile）**  
   对一批锚点批量查询词汇节点在图谱上的 **语义相似边（如 SIMILAR_TO）**，在每个锚点下只保留少量最高分近邻，形成 **岗位级层次画像**：它描述「从工业锚点出发，学术概念空间里大致落在哪些邻域」。该画像主要给后续 **层级守卫、偏题惩罚、主题一致性** 作证据，而不是直接等于 Stage2 的最终落点。

**输出**：活跃领域、锚点字典（含分层、分数、类型、是否具备 primary 候选资格等）、附加上下文与条件化向量、岗位层次画像等，打包成 Stage1 结果供 Stage2 消费。  
**阶段定位**：Stage1 是 **传感与建档**——把非结构 JD 变成「带领域约束、带主辅结构、带语义画像」的锚点集合；**不在此阶段做学术概念终选**。

##### Stage2：学术落点与扩展候选生成（2A + 2B）

Stage2 把 **工业语言** 推进到 **学术词汇候选空间**，并控制噪声规模。分为 **2A 局部落点与桶组织** 与 **2B 保守扩族**。

**Stage2A（每个锚点各自为政，但为全局重排准备结构标签）**  

1. **候选从哪里来**  
   对每个工业锚点，综合多条来源拉取学术概念候选：图谱 **语义相似边**、查询向量与词向量的 **近邻**、**精确 / 别名匹配**、以及少量 **家族型落点**（把锚点先映射到较稳定的「家族中心」再拉候选）。多源并集后进入本地打分与裁剪。

2. **主线组织：分桶而非一刀切**  
   候选被归入不同 **角色桶**：高置信可扩主线、弱主线种子、仅保留不扩、风险保留、拒绝等。核心思想是：**先保证「这条锚点上哪些学术词像主叙事、哪些像侧证、哪些像噪声」被结构化**，而不是在锚点内部只留一个词。部分锚点会出现 **保底主线**（组内暂时没有强 primary 时，用结构证据救回一条「先占位、不扩散、待 Stage3 再审」的候选），避免锚点整组空心化。

3. **Primary 与 Expand 解耦**  
   「算不算主线候选」与「能不能当扩族种子」分开判断：有的词可以保留进全局池但不触发 2B 扩散；有的弱主线只能当弱种子。对明显该留主线却被启发式挡住的少数情况，保留 **窄逃生** 以免误伤控制性技术词。

4. **组内收缩与合并**  
   若某锚点没有任何「可扩主线」，会对保留桶做 **极小家族收口**（每个桶只留极少量代表），避免把大量近重复 tid 塞进 Stage3；各锚点的 primary 候选再按规则合并成全局可继续处理的 landing 集合。

**Stage2B（只扩种子，不推翻 2A）**  

1. **谁能触发扩展**  
   仅 **强种子 / 弱种子** 且通过种子资格检查的概念会进入扩族；其余 2A 产物 **原样 carryover**，不参与扩散链路。

2. **扩展渠道（小半径）**  
   在受控条件下做 **语义密邻扩展**（沿向量意义相近的词）、**预计算簇内扩展**（同一主题簇的成员）、以及 **统计共现扩展**（与种子在文献共现网络中常一起出现的词）。扩展有上限与父节点强度约束，避免一步跳到泛化大词。

3. **汇总交付**  
   将 carryover 与扩展合并，输出 **全量扁平候选列表**（供 Stage3 按 tid 聚合）、**按锚分组视图**（便于追溯每个词从哪个锚点家族来）、以及一张 **候选关系草图**（同锚内支撑、跨锚支撑、家族边、来源追溯等），并附带规模与来源分布统计。

**输出**：结构化的 Stage2 总包（扁平候选 + 分组 + 关系草图 + 报告）。单条候选上同时带有 **标识、局部相似与层级贴合、风险标记、落点/扩展置信** 等；部分「临时态」元数据会收拢在局部元信息里，避免顶层字段爆炸。  
**阶段定位**：Stage2 是 **高召回、强结构**——宁可多给带标签的候选，也不在 2A/2B 做跨锚终局排序；**全局谁上论文检索词表由 Stage3 决定**。

##### Stage3：词级全局重排与论文检索词定稿

Stage2 的输出在 Stage3 被 **按词（概念 ID）跨锚点合并**：同一学术词可能来自多个锚点家族，需要把证据折成一条 **全局视图**。

**过程概要**：

1. **准入与聚合**  
   去重后，对每个词合并多来源证据，计算 **身份因子**（多锚互证程度）、**风险惩罚**（泛词、对象壳、跨域歧义等）、**角色因子**（更偏核心检索词还是支撑词）。先做一轮 **是否允许进入全局打分池** 的校验。

2. **统一连续分与观测分桶**  
   在连续分数空间中排序，同时保留 **观测用分桶**（如 core / support / risky），便于日志解释；**前序桶标签可被推翻**——Stage2 的 primary 不保证仍是 Stage3 的 paper 主词。

3. **家族与角色约束**  
   在分数之上施加 **家族级约束**（例如避免同一语义家族不合理地霸占多个 paper 名额），使最终集合在「覆盖度」与「多样性」之间折中。

4. **论文检索词选择（核心闸门）**  
   这是 Stage3 与 Stage4 的接口：**哪些词真正进入论文召回**。包含多层机制：  
   - **主轴门槛**：优先保证与 JD 主叙事一致的核心词进入；对「接近但未过线」的核心词可有 **软放行**；  
   - **主 / 支援配额与动态下限**：防止全是支援词或全是泛词；  
   - **近失配换位**：在分数边缘上，允许核心词与较弱支援词在窄窗口内互换，避免排序噪声导致主词掉队；  
   - **尾部补足**：当入选词过少或支援槽为空时，按规则从 near-miss 中再补少量词，避免论文召回「无词可用」。  
   某些结构型保底词、单锚条件词会被 **硬挡** 在论文池外，防止它们主导检索。

**输出**：全局排序词表、**最终用于 Stage4 的论文检索词集合**（含检索角色：更偏 primary 证据还是 support）、连续分数字典、全局一致性诊断报告等。主入口在进入 Stage4 时 **优先采用本阶段产出的论文检索词**；若为空再回退到备用选词逻辑。
...
1. **检索而非扩词**  
   用学术词去拉 **论文候选池**（往往分粗召回 + 精筛，多层叠加），目标是验证这些词在文献侧是否形成 **主题集中、与 JD 对齐** 的证据，而不是继续扩展新概念。

2. **词—文对齐与命中解释**  
   对每篇候选论文，检查词汇命中位置、是否与岗位语义 **grounding**（例如是否在标题/摘要关键句出现，是否被判定为「硬命中」而非偶然出现），并按词的角色（主证 / 支援）赋予不同 **证据权重**。

3. **领域与层次一致性**  
   在论文侧施加 **领域软奖励**（不是简单硬过滤），并对 **层次共识** 做审计：若某词在族内是「强主轴核心」类命中占主导，才允许更强的层次加分；否则封顶，防止「泛核心词」在组内被放大成主导证据。

4. **论文级分数**  
   综合词权重、命中质量、偏题惩罚、层次 bonus 等，得到 **论文级得分**，并按作者聚合 **每位作者名下有哪些论文、各自如何被词汇命中**。

**输出**：按作者组织的 **论文列表与命中明细**，作为 Stage5 的唯一上游证据结构（除非调试回退）。  
**阶段定位**：Stage4 是 **概念→文献的 grounding 验证**——回答「Stage3 选中的学术词，是否在论文库里真的指向对口工作」。

##### Stage5：作者层聚合、排序与截断

**输入**：Stage4 的论文与命中结构、Stage3 的词级分数与角色、时间衰减与领域配置等。

**过程概要**：

1. **论文分 → 作者分**  
   对每篇论文先形成 **论文贡献分**，再按作者维度累积。累积时考虑：同一词汇对同一作者的贡献 **递减**（避免一词刷满）、单篇论文 **作者人数过多则分摊**（削弱「超大合作论文」对多个作者线性刷榜）、以及 **时间衰减**（同样质量下新工作略优先，老工作仍保留长尾）。

2. **结构形状因子**  
   根据每位作者命中模式的 **结构特征**（例如：是否同时有强 primary 与多篇支援、是否多篇论文形成互证、是否主要只剩支援词托底等），乘以不同的 **结构乘子**，使「证据形状健康」的作者优于「只有稀薄支援」的作者。

3. **纯支援型作者的抑制**  
   若某作者命中论文几乎全是支援角色、缺乏与 primary 同框的强证据，在满足阈值时对其总分 **额外降权**，避免「广撒网式共现」排在扎实主叙事专家之前。

4. **截断与诊断**  
   按配置保留 Top-K 作者，输出最终 ID 列表；同时可附带 **细粒度审计**（每位作者由哪些词、哪些论文托起），便于解释与调参。

**输出**：排序后的作者候选列表及调试信息。  
**阶段定位**：Stage5 是 **文献→人的聚合与可解释排序**——在 Stage4 已验证的论文证据上，还原 **谁最像被岗位「主线需求」支撑的专家**。

##### 编排层与阶段间衔接（主链正文未逐条展开的部分）

上文按 Stage1～Stage5 写了**语义上的完整主干**，但**不等于**把主召回入口里的每一行调度都抄成文档：下面补全「阶段与阶段之间」还发生了哪些事，避免读者误以为只有五段黑箱、没有衔接逻辑。

1. **领域探测的两步结构（Stage1 内）**  
   先用查询向量在 **岗位向量索引** 上取一批最相近岗位，统计其 **领域字段** 的出现频次，得到 **候选领域 Top5**；再用 **查询向量与各领域名称向量**（与 JD 编码同一向量空间）算相似度，从中选出 **最终活跃领域 Top3**。同时会算一个 **领域主导度**（近邻岗位里最强领域占比），供后续 Stage5 等作软上下文。另会生成与领域相关的 **文本模式**（如正则/关键词），贯穿 Stage4 等用于辅助对齐——不是 Stage1 单独消费完就丢弃。

2. **早退与空路径**  
   若图谱不可用、Stage1 **没有任何锚点**、或 Stage2 **候选为空**，主链会直接结束并返回空作者列表（并带检查点信息），不会强行跑后续阶段。

3. **Stage2 与 Stage1 画像的衔接**  
   Stage2 会接收 Stage1 产出的 **岗位层次画像**：用于 **层级守卫、泛词抑制** 等——即扩展与落点不是「只看锚点字符串」，而是能对照「该锚点在学术邻域里大致落在哪条语义主线附近」做约束。

4. **Stage3 之后到 Stage4 之前的「精检」**  
   Stage3 先产出全局 `score_map` 与 **论文检索词行表**（每行含分数、检索角色、家族键、父锚信息、槽位层级等）。主链优先用这份 **行表** 作为 Stage4 的用词与权重；若行表为空，才 **回退** 到用分数图另选一组检索词（有数量上限）。这保证正常路径下 Stage4 吃的是 **经 paper-term 闸门润色后的集合**，而不是裸的连续分 Top-K。

5. **供 Stage5 使用的两张「乘子表」（在 Stage3 后、Stage4 前算好）**  
   - **词来源置信度**：按每个词在 Stage2/3 中的 **来源类型**（如直连相似边、密扩、簇扩、共现扩等）映射到不同置信档位，用于论文贡献里「词证据有多硬」。  
   - **词领域纯度（专属性）**：用统计库里的 **词汇—领域分布** 估算该词对当前 **目标领域集合** 是否专一；专名型词权可抬升，极泛词权可压降。二者进入 Stage5 的 **论文—词贡献** 计算，而不是只在 Stage3 装饰分数。

6. **簇与任务向量的轻量门控（贯穿打分链路）**  
   会用查询向量与 **预计算主题簇中心** 的对齐程度，给词一个 **接近 1 的簇级微调因子**（只做细调，不承担主排序）；在部分路径还会对 **同一簇内尾部** 词做衰减，避免一簇里次要词挤占前排——这是对「同簇多词」的补充，**不等同**于 Stage3 的家族配额逻辑。

7. **Stage4 的输入不仅是词表**  
   除词汇 ID 外，还传入 Stage3 给出的 **逐词检索角色**（主证/支援权重不同）、**行级元信息**（父锚、槽位、惩罚痕迹等）以及 **JD 原文**，供命中解释与打分；词分图若经精检替换，则 Stage4 用 **精检后的逐词分**，否则退回全局分图。

8. **Stage5 额外接收的全局上下文**  
   除论文列表与词信息外，还接收 **领域主导度**、上述 **置信度/纯度**、以及闭环调试所需的映射；最终作者列表会按配置的 **召回上限** 截断，对外常返回带 **标签路名次** 的元信息条目。

**结论**：就**工程完整性**而言，README 需要把本节与上文五段 **一起读**，才覆盖主召回入口所表达的 **端到端过程**；若还需细化到「每个参数闸门、每条审计日志」，则必须以代码与后文硬核表为准——那是运维/复现层，不是概念层能无损压缩进同一段文字的。

---

因此，标签路的关键不在于「为每个词找到最近邻」，而在于：为整份 JD 找到一组最能解释该岗位核心能力结构的学术概念族群，并在 **论文与作者两层** 用证据检验这组概念是否成立。一个学术词是否真正优质，不应只看它与锚点的局部相似度，还应看它能否在 Stage4 召回主题集中、方向正确的论文，能否在 Stage5 形成稳定、可解释的专家排序结果。换言之，标签路中的「学术词」不是最终目标，而是连接 JD、论文与作者的桥梁；其设计目标是最大限度地提升这座桥梁在任务语境中的稳定性、可解释性与检索效能。
...
**核心目的**  
走「**岗位技能 → 学术词 → 论文 → 作者**」的图谱路径（与内部 **11 篇**综述中的 **五阶段标签路**概念模型同构；各阶段在 IR/NLP 文献中的典型称呼见上文 **「文献综述与术语谱系：11 篇论文与『五阶段标签路』概念模型」**），用知识图谱 + 多维度打分，找出与 JD 技能要求在语义和共现上都对齐的学术词，再只保留领域垂直、论文质量高的论文与作者；强调硬技能/概念对齐，并抑制泛词、万金油论文和弱相关论文。实现上采用**五阶段流水线**（`label_pipeline`）与**标签子模块**（`label_means`），基础设施与索引（Neo4j、Faiss、vocab_stats.db、簇中心等）由 `label_means.infra` 统一加载与管理。
...
| **核心中间产物** | **Stage1**：`_jd_coarse_roles`、**`refine_stage1_anchor_layers` 后的** anchor_skills（**`coarse_jd_role` / `anchor_role` / `is_primary_eligible` / `source_kind` / `final_anchor_score`** 等）、**`_stage1_context_only_anchors`**；Stage1Result（**含 jd_profile**）、local_context、phrase_context、conditioned_vec、job_ids、regex_str、`_jd_cleaned_terms`。**Stage2（run_stage2）**：**dict** — `all_candidates`（Stage3 合并侧平铺）、`anchor_to_candidates`、`candidate_graph`、`stage2_report`；单条记录经 **`_expanded_to_raw_candidates`**：**正式契约以顶层证据 + `stage2_local_meta`（provisional）为主**，**顶层仍保留 legacy 镜像**（`term_role`、`primary_bucket` 等）。**Stage2A/2B 内部**仍产 primary_landings、dense/cluster/cooc 扩展等（见 `label_expansion`）。**Stage3（`run_stage3`）**：**入参** **`stage2_output`（四键）**；**透传**图与分组至 **`_stage3_stage2_context`**；**返回**含 **`selected_*`、`paper_terms`、`global_coherence_report`** 及 **`score_map` 等**；**按 tid 去重聚合**、**classify_stage3_entry_groups**、**check_stage3_admission**、**第一段** score_term_record×**identity_factor**×**risk_penalty**×**role_factor**、**assign_stage3_bucket**（观测）、**`stage3_build_score_map`（统一连续分）**、**select_terms_for_paper_recall**（**JD 主轴门** `STAGE3_PAPER_MAIN_AXIS_*` 仅 core + `paper_select_score`+family+floor）、_collect_risky_reasons/_bucket_stage3_terms；tag_purity_debug（含 **stage3_entry_group**、**identity_factor**、**risk_penalty**、**stage3_bucket**、anchor_count、evidence_count、family_centrality、path_topic_consistency、generic_penalty、cross_anchor_factor、retrieval_role 等）；**`debug_1["stage3_global_coherence_report"]`**；label_path 据此构建 term_confidence_map、term_uniqueness_map。**Stage4**：author_papers_list。**Stage5**：paper_map、contribution、作者聚合分、最终 author_id 列表。 |
| **主要问题** | 缩写歧义、任务词泛化、领域漂移、万金油词与弱相关论文需靠熔断与权重抑制 |

#### 3.1 标签路数据流与阶段总览

**主入口与调用链（与 `label_path.py` 对应）**  
`LabelRecallPath.recall()` 依次调用：`_stage1_domain_and_anchors`（→ `stage1_domain_anchors.run_stage1`）、`_stage2_expand_academic_terms`（→ **`stage2_expansion.run_stage2`，返回 dict**）、`_stage3_word_weights`（→ **`stage3_term_filtering.run_stage3(self, stage2_output, ...)`**）、`_stage4_graph_search`（→ `stage4_paper_recall.run_stage4`）、`_stage5_score_and_rank_authors`（→ `stage5_author_rank.run_stage5`）。其中 **`stage2_output`** 含四键；**`recall` 仍单独使用 `all_candidates` 做检查点与扩展诊断回填**。基础设施由 `LabelMeansInfra` 在 `__init__` 中统一加载（`graph`、`job_index`、`vocab_index`、`stats_conn`、`cluster_members`、`voc_to_clusters`、`cluster_centroids` 等），见 `label_means.infra`。
...
| **Stage2B** | **carryover_terms**（2A 全量保留 landing）+ **seed_candidates**（`stage2b_seed_tier∈{strong,weak}`）, active_domain_set, jd_*_ids | 内部仍产出 **ExpandedTermCandidate** 列表；**`run_stage2` 出口**打包为 **`all_candidates` + `anchor_to_candidates` + `candidate_graph` + `stage2_report`**（单条见 `_expanded_to_raw_candidates`：**`stage2_local_meta` + 兼容顶层镜像**） | **`check_seed_eligibility` 仅遍历 seed_candidates**（tier=none 的 carryover 不下门、只预置 `seed_blocked=True`/`stage2a_not_seed` 与 merge 一致）；**eligible_seeds** 再跑 dense/cluster/cooc；**`merge_primary_and_support_terms(carryover_terms, …)`** 必须把 support_keep/risky_keep 一并带进 Stage3，并从 **PrimaryLanding** 拷贝 **`admission_reason`/`reject_reason`/`mainline_candidate`/`primary_reason`** 到 **ExpandedTermCandidate**；**`seed_candidates=0` 紧凑路径** 对 **`merge_primary_and_support_terms`** 设 **`emit_merge_debug=False`**，避免与单行 `[Stage2B] anchor=…` 重复刷屏；**DENSE_PARENT_CAP=0.85**；expand_from_cluster_members；expand_from_cooccurrence_support |
| **Stage3** | **`stage2_output` dict**（须含 **`all_candidates`、anchor_to_candidates、candidate_graph、stage2_report**；合并侧字段来自 **`all_candidates`**，含 term_role、role_in_anchor、source_types、retain_mode、**primary_bucket / fallback_primary / mainline_candidate**、**parent_anchor_final_score / parent_anchor_step2_rank** 等）, query_vector, anchor_vids | **`selected_*` / `paper_terms` / `global_coherence_report`**；score_map, term_map, idf_map, **paper_terms** | 四键校验与 **`_stage3_stage2_context` 透传**；去重聚合 → classify_stage3_entry_groups → check_stage3_admission；**第一段**：score_term_record × identity × risk × **role_factor**；**`_assign_stage3_bucket`**；**`stage3_build_score_map`** → **`_apply_family_role_constraints`**；**`select_terms_for_paper_recall`**：weak support（**可 `SUPPORT_TO_PAPER_*` 软放行进 `support_pool`**）/ **`risky_side_block`** → **JD 主轴门**（core 过线 → **`paper_recall_quota_lane=primary`**；**双弱 core** → **`core_axis_near_miss_soft_admit`** 进池）；**其余未硬挡 support/risky/其它桶先入 `support_pool`** → **`eligible.extend(support_pool)`** → **`[Stage3 paper gate summary]`** 等 → **`paper_select_score`**（**`final_score` 不变**；**`base`+池惩罚不变**；**结构 bonus 正部**经 **`PAPER_SELECT_BONUS_TIER_SCALE_*`** 按 strong/bonus_core/support/other 缩放；**扫描全局序**：**强主轴 core→`support_lane`（段内近失配优先）→`other_eligible`→`bonus_core` 殿后**，**`PAPER_SELECT_STRONG_MAIN_AXIS_*`**）+ **family** + **`dynamic_floor`** + **support 槽配额**（**`paper_recall_quota_lane=support`**，**`support_quota_full`**）；**`[Stage3 paper gate reject audit]`** / soft-admit / pool / centrality / quota；**主 cutoff 后、tail 前** **`[Stage3 paper swap audit]`**（**core near-miss** 与最弱 **support soft-admit** 在 **`PAPER_RECALL_SUPPORT_SWAP_SCORE_MARGIN`** 内可一对一换位；近失手窗口 **`PAPER_RECALL_CORE_NEAR_MISS_SWAP_MAX_BELOW_FLOOR`**）→ **`PAPER_RECALL_TAIL_EXPAND_*`**（**`len(selected)≤3`** 或 **support 槽计数为 0** 时 **near-miss** 再补 1~2 词；**分层**：**`tail_expand_core_near_miss`** 先于 **`tail_expand_support_soft_admit`**，见 **`[Stage3 tail expand audit]`**）；**unified breakdown** 默认 Top **`STAGE3_UNIFIED_SCORE_DEBUG_TOP_K`（3）**；仍硬挡 **单锚 conditioned_only**、**fallback_primary** |
...
| 维度 | 说明 |
|------|------|
| **职责** | 对 Stage2A/2B 产生的**全部**候选做**跨锚点、跨来源、跨家族**的统一重审（词级收束，非简单删词）：去歧义、降噪、support/risky 下压，以及**最终 paper-term / paper recall 用词选择**。**关键词**：**重审，不是继承 Stage2A 判决**；前层 `primary_bucket`、`fallback_primary`、`mainline_candidate` 等是**参考输入**，**不是不可推翻的结论**（例如：`primary_expandable` 不保证最终进 paper，`support_keep` 也不等于一定无用，`fallback_primary` 也不等价于普通 primary）。 |
...
| 维度 | 说明 |
|------|------|
| **职责** | 在论文层验证 Stage3 定稿的 term：将词级信号落到论文候选上，结合 grounding、偏题惩罚等生成论文级分数，为作者聚合提供证据基础（目标不是继续扩概念） |
| **输出** | 论文级召回与打分结果（供 Stage5 作者聚合使用） |
| **不负责** | 学术词生成与检索词定案（由 Stage2 / Stage3 负责） |
...
| **`[Stage3 paper lane tier audit]`** | **`select_terms_for_paper_recall`**（**floor 计算之后**、**centrality 之前**） | **全 eligible** 按 **`paper_select_lane_tier`** 与**真实扫描序**输出：**term / tier / p_sel / a_sc / rk / ml / exp / lane / parent_anchor**；序为 **strong→support（near_miss≻）→other_eligible→bonus_core**（**strong** 阈值见 **`PAPER_SELECT_STRONG_MAIN_AXIS_*`**） |
| **`[Stage3 paper lane fill audit]`** | **`select_terms_for_paper_recall`**（**主 cutoff 之后**、**swap 之前**；**`STAGE3_PAPER_LANE_FILL_AUDIT`** + **`STAGE3_PAPER_CUTOFF_AUDIT`**） | **极简 5 行**：标题 + **`strong_main_axis_core=`** / **`support_lane_taken=`** / **`bonus_core_taken=`** / **`support_lane_remaining_candidates=`**（support 池未入选且 **p_sel≥floor** 的条数）。**读法**：若 **`bonus_core_taken>0`** 且 **`support_lane_remaining_candidates>0`**，主截断下属异常（殿后序下 bonus 不应先于合格 support 占坑；见图下说明）。 |
| **`[Stage3 bonus-core audit]`** | **同上**（**`STAGE3_BONUS_CORE_BLOCKED_AUDIT`**） | **与示例同形**：表头 **term / p_sel / selected / blocked_by / reason**（管道符分隔）。未入选且 **`past_paper_recall_max_terms`** 且仍有 **support p_sel≥floor** 余量 → **`support_lane_priority_not_filled`**，**blocked_by** 为当期已选 term（谁占满名额）；**family_duplicate** → **blocked_by** 为同族已选词。 |
| **`[Stage3 bonus-core readiness audit]`** | **`select_terms_for_paper_recall`**（主 cutoff 后、swap 前；**`STAGE3_BONUS_CORE_READINESS_AUDIT`** ∧ **`STAGE3_PAPER_CUTOFF_AUDIT`**） | 每条 **bonus-like primary core**：**a_sc、rk、ml、exp、fb、cond、ready 乘子、`p_raw`/`p_adj`（paper_select_score 缩放前后）、`elig_sw`/`elig_te`**（是否满足 **swap/tail** 用的 core-near-miss 结构性门槛；**bonus_core 恒否**） |
| **`[Stage3 paper swap pair audit]`** | **同上**（**swap 判定紧随** **`[Stage3 paper swap audit]`**） | **首对** swap 候选：**in/out lane、in_p_sel_raw、in_ready、in_p_sel_adj、out_p_sel、margin_ok、swap_ok、reason**（如 **`margin_fail`**） |
...
| **`[Stage3 paper swap audit]`** | **`select_terms_for_paper_recall`**（**主 cutoff 之后**、**`PAPER_RECALL_TAIL_EXPAND_*` 之前**；`STAGE3_PAPER_CUTOFF_AUDIT`） | 单行：**`swapped`**、**out/in term**、**out_p_sel / in_p_sel**、**delta**、**floor**；**swap-in 不含 `bonus_core`**（**`_is_bonus_like_core`** 不得进入 near-miss 置换）。常量：**`PAPER_RECALL_CORE_NEAR_MISS_SWAP_MAX_BELOW_FLOOR`**、**`PAPER_RECALL_SUPPORT_SWAP_SCORE_MARGIN`** |
| **`[Stage3 tail expand audit]`** | **`select_terms_for_paper_recall`**（**主** floor+family+support_cap 截断之后） | **触发**：**`len(selected)≤3`** 或 **主选阶段 `selected_support==0`**；**补位**：未入选、**`p_sel≥floor−Δ`**、禁 risky/fallback/conditioned_only；**第一层 core near-miss 不含 `bonus_core`**；**`bonus_core` ∧ `below_dynamic_floor`** → **整段 tail 不补**（含 **other_tail**，防绕层混入）。**`p_order`**：**core_near_miss(1)>support_soft_admit(2)>other(3)** |
...
- **`select_terms_for_paper_recall`**：**合并前**硬挡 **单锚 conditioned_only**、**fallback_primary**、**`weak_support_contamination`（未达软放行）**、**`risky_side_block`**。**`JD 全局主轴门`（仅 core）**：**`rk≤MAX ∨ a_sc≥MIN`** 过线 → **`paper_recall_quota_lane=primary`**；**双弱** → **`core_axis_near_miss_soft_admit`**（×**`PAPER_CORE_AXIS_NEAR_MISS_FACTOR`**）进 **`support_pool`**。**凡未硬挡的 support / risky / 其它桶先入 `support_pool`**（含 **`support_lane` / `risky_support_lane`**），再 **`eligible.extend(support_pool)`**。**`[Stage3 paper gate reject audit]`**、**`[Stage3 paper gate summary]`**（**`axis_direct_core`、`support_pool_n`、`merged_eligible_n`** 等）。**`paper_select_score`**：**`base` 与池惩罚不变**；**结构 bonus 正部**按 **`PAPER_SELECT_BONUS_TIER_SCALE_*`** 分档缩放（见上）；随后在 **本函数终审层** 对 **`_is_bonus_like_core`（即 `paper_select_lane_tier=bonus_core`）** 再乘 **`_paper_bonus_core_readiness`**（系数 **`PAPER_BONUS_CORE_READINESS_*`**），写入 **`paper_select_score_pre_bonus_core_readiness` / `paper_bonus_core_readiness` / `paper_bonus_core_readiness_debug`**，再算 **`dynamic_floor`**。合并池内对每条写入 **`paper_select_lane_tier`**：**`strong_main_axis_core`**（**`stage3_bucket=core` ∧ `paper_recall_quota_lane=primary` ∧（`a_sc≥PAPER_SELECT_STRONG_MAIN_AXIS_SCORE_MIN` ∨ `rk≤PAPER_SELECT_STRONG_MAIN_AXIS_MAX_RANK` ∨（`ml≥1`∧可扩∧`a_sc≥PAPER_SELECT_STRONG_MAIN_AXIS_ML_EXP_SCORE_MIN`∧`rk≤PAPER_SELECT_STRONG_MAIN_AXIS_ML_EXP_MAX_RANK`）））、**`bonus_core`**（其余 **primary lane** 的 **core**）、**`support_lane`** / **`other_eligible`**。**全局遍历序（主 cutoff 扫描）** = **`strong_main_axis_core` → `support_lane`（同段内 `core_axis_near_miss_soft_admit` 先于普通 support）→ `other_eligible` → `bonus_core`**：**`bonus_core` 殿后补位**，避免宽 primary / 方法学项在仍有 **support_lane** 可用名额时稳定占一席（无需 RL 黑名单；与 Stage4 **strong 正向门控**分工：**Stage3 管「谁先入选」**，Stage4 管 **wid 级 bonus 是否 >1**）。**段内排序**：**`support_lane`** 先 **`core_axis_near_miss_soft_admit`** 再其余、同级 **`paper_select_score` 降序**；**其余段**仅 **`paper_select_score` 降序**。其后 **`dynamic_floor` + `family_key` + `support_cap`** 扫描。**入选**时 **`paper_recall_quota_lane=support`** 计 **support_cap**（**`support_quota_full`**）。候选先 **`_apply_paper_readiness_for_recall`**（含 **`paper_support_factor`**），再 **`paper_select_score`**：
  ...
  - **加性注入（仅 paper 排序）**：在 **`base`** 上叠加 **`bucket_bonus`**（`primary_expandable` / `primary_support_seed` / `primary_support_keep` / `risky_keep` 等）、**`mainline_bonus`**、**`expand_bonus`**（`can_expand_from_2a` 或 `can_expand`）、**父锚名次档**（rk≤3/6/10）、**`anchor_score_bonus = PAPER_SELECT_ANCHOR_SCORE_BONUS_COEF × norm(父锚分)`**、**`role_bonus`**（`primary` / `primary_side` / `dense_expansion`）；系数见 **`PAPER_SELECT_*`**。若缺 **`parent_anchor_*`** 则 **`best_parent_anchor_*`** 回填。先得到结构 bonus 累加和 **`paper_select_score_bonus_pre_tier`**，再按与 **`paper_select_lane_tier`** 一致的分档（**`_paper_select_bonus_tier_scale`**：**strong** / **bonus_core** / **support_lane** / **other**）对 **bonus 的正部**乘以 **`PAPER_SELECT_BONUS_TIER_SCALE_*`**（负向惩罚不缩放），写入 **`paper_select_score_bonus_total`** 及 **`paper_select_bonus_tier_scale` / `paper_select_bonus_tier_key`**。用于压低「未达强主轴门槛的 primary core」（如加分项 RL）与主轴词 **`+bonus`** 梯度过平的问题。
  ...
  - **主 cutoff 后、tail 前（换位，`PAPER_RECALL_*_SWAP_*`）**：在 **不改 `dynamic_floor` 抽象（Top×REL∨ABS）** 的前提下，若存在 **未入选**、**`stage3_bucket=core`**、**非 `_is_bonus_like_core`**、主截断/拒绝原因含 **`below_dynamic_floor`**、**ml≥1**、**可扩**、且 **`p_sel≥floor−PAPER_RECALL_CORE_NEAR_MISS_SWAP_MAX_BELOW_FLOOR`** 的 **core near-miss**，同时 **已入选** 中最弱的一条为 **`stage3_bucket=support`** 且 **`paper_recall_quota_lane=support`** 的 **weak support soft-admit**，且 **`in_psel ≥ out_psel − PAPER_RECALL_SUPPORT_SWAP_SCORE_MARGIN`**，则 **一对一替换**。**`bonus_core` 不得 swap-in**。详见 **`[Stage3 paper swap pair audit]`**。
  - **尾部补位（`PAPER_RECALL_TAIL_EXPAND_*`）**：**`bonus_core` ∧ 主截断 `below_dynamic_floor`**：**整条 tail 跳过**。**第一层** core near-miss **同 swap，排除 `bonus_core`**。其余分层与记因不变（**support soft-admit** → **other**）。**`p_sel` 已含 bonus_core ready 乘子**。
  - **主 cutoff 后、swap/tail 前（殿后序验收，极简日志）**：
    - **`[Stage3 paper lane fill audit]`** 示例：
      ```text
      [Stage3 paper lane fill audit]
      strong_main_axis_core=2
      support_lane_taken=2
      bonus_core_taken=0
      support_lane_remaining_candidates=3
      ```
      **逻辑**：`support_lane_remaining_candidates` = 仍为 support_lane tier、未进栏、且 **p_sel≥dynamic_floor** 的条数。与 **`bonus_core_taken`** 对照：若 bonus 已进栏而该项 **>0**，说明「合格 support 还在池里、bonus 却占了一席」——主扫描下应极少出现（否则检查实现或 swap/tail 后续改写）。
    - **`[Stage3 bonus-core audit]`** 示例：
      ```text
      [Stage3 bonus-core audit]
      term | p_sel | selected | blocked_by | reason
      reinforcement learning | 0.348 | False | route planning, robotic arm | support_lane_priority_not_filled
      ```
      **逻辑**：**blocked_by** = 当期已入选 term（comma）；**support_lane_priority_not_filled** = 名额被前排占满且仍有 **support p_sel≥floor** 未入选，bonus 殿后仍无空位（规则生效时的典型挡因之一）。
  - 其余审计：**`[Stage3 paper gate reject audit]`**、**`[Stage3 support soft-admit audit]`**、**`[Stage3 paper lane tier audit]`**、**`[Stage3 bonus-core readiness audit]`**、**`[Stage3 paper centrality audit]`**、**`[Stage3 paper pool penalty audit]`**（**eligible core close-call** 由 **`STAGE3_ELIGIBLE_CORE_CLOSE_CALL_AUDIT`** 控制，默认关）、**`[Stage3 paper swap audit]`**、**`[Stage3 paper swap pair audit]`**、**`[Stage3 tail expand audit]`**、**`[Stage3 paper selected composition]`**、**`[Stage3 paper quota audit]`**（**primary/support lane**）、 **`[Stage3 paper main-axis gate audit]`**、**`[Stage3 paper gate summary]`**、**`paper_anchor_*`** 等。**`[Stage3 paper selection audit]`**：**仅 `STAGE3_DEBUG_FOCUS_TERMS` 非空** 时打印焦点词；增列 **`psf` / `sup_reason`**。**主轴门**：**OR**；双弱 **降级进池**，非整词黑名单。**`_paper_recall_dynamic_floor(..., score_key="paper_select_score")`**（**在 bonus_core ready 乘子之后**）；**`family_key` 去重**；**`PAPER_RECALL_MAX_TERMS`**；总长可至 **`max_terms + PAPER_RECALL_TAIL_EXPAND_MAX_EXTRA`**。**`retrieval_role`**：**`paper_recall_quota_lane=primary`** → `paper_primary`，否则 **`paper_support`**（**含 bucket 仍为 core 但 near-miss 降级者**）。
...
**相关常量**：`STAGE2A_CONDITIONED_SIM_EPS`、…、`STAGE3_SUPPORT_DEMOTE_*`；**paper 最后一跳**：**`PAPER_SELECT_*`** 加性 bonus（与 **`PAPER_READINESS_*`** 分工：后者只进 **`paper_readiness`**）+ **`PAPER_SELECT_BONUS_TIER_SCALE_*`**（对 **bonus 正部**按 strong / bonus_core / support_lane / other 缩放）+ **`PAPER_SELECT_POOL_PENALTY_*`**（**`paper_select_score`** 减项）+ **`PAPER_SELECT_STRONG_MAIN_AXIS_*`**（**强主轴判定**与**四段扫描序**，与 **`STAGE3_ELIGIBLE_CORE_CLOSE_CALL_AUDIT`**、**`STAGE3_DEBUG_FOCUS_TERMS`** 配合降噪）。
...
**Stage3：词级重排与论文检索词选择（去重聚合 + 分层准入 + 软 identity + 分桶 + `S3_select`）**
...
**Stage4：论文层召回与证据打分（二层召回 + 领域软奖励 + grounding）**
...
| 项 | 说明 |
|----|------|
| **作用** | 展开 **paper_map**、论文贡献度→作者，累计 evidence 并截断；**`TERM_MAX_AUTHOR_SHARE`** 后由 **`_compute_author_structure_shape`** 用 **分段 `structure_factor`** 压 **单词单篇、无 mtp** 的作者、抬 **mtp≥1 或 st≥2**，**无词表特判**；×时间权重；再对 **纯 support 线、无 primary 论文托底** 的作者施加 **`support_only` 乘子**（**不改词分**）；× **AUTHOR_BEST_PAPER_MIN_RATIO** 与归一化，输出排序与 **last_debug_info**（部分结构一致性/平衡性项可为预留位，见上文五阶段定位）。 |
...
| **调用的表/知识图谱** | **内存**：score_map、term_map、term_role_map、term_confidence_map、term_uniqueness_map、paper_map、voc_to_clusters、all_vocab_vectors、vocab_to_idx（proximity）；**无直接 DB**。**works_to_authors**：accumulate_author_scores 用 papers 中的 wid、score、authors（含 pos_weight）。 |
...
| **simple_factors** | survey_decay_factor、coverage_norm_factor、paper_cluster_bonus 等论文侧因子 | paper_scoring 与词/论文打分 |
| **advanced_metrics** | 共现/共鸣相关（term_resonance、cooc_span_penalty、cooc_purity_bonus 等，部分当前为占位 1.0） | Stage2/Stage3 词权重 |
| **base** | 公共基类或工具 | 按需 |
| **label_debug_cli** | 调试 CLI，与 RecallDebugInfo 配合；**【Top term 最终贡献表】**按 **`final_term_ids_for_paper`** 拆分；**`paper_count_hit` / `top_paper_contrib` 两列**仅当环境变量 **`STAGE5_TERM_CONTRIB_DEBUG=1`** 时打印（默认可为 0、降噪） | 开发与排查 |
...
**核心目的**  
对 JD **只编码一次**得到统一 query_vec；**并行**跑向量路与标签路，再用两路各 **Top 100** `author_id` 并集做种子跑协同路；将三路结果构建为统一**候选中间层**（CandidateRecord + CandidatePool），并在该中间层上派生 **training pool（≤100）** 与 **display pool（≤50）** 两个视图出口，供**精排与训练**复用。  
...
| **输出** | **candidate_pool**（base 主表 + training/display 视图 + 证据明细 + pool_debug_summary）、**training_pool_top_100**、**display_pool_top_50**、**kgatax_sidecar_rows**、rank_map、总耗时（`final_top_200 / final_top_500` 为 deprecated/compatibility only） |
...
- **候选池辅助标记**：bucket_type（**A～F / Z**，见总召回 `_assign_buckets`）、passed_hard_filter、dominant_recall_path、hard_filter_reasons、bucket_reasons；vector_evidence、label_evidence、collab_evidence。

**CandidatePool**（显式三块）

- **candidate_records**：候选主表，一人一条 CandidateRecord。
- **training_pool_records**：训练池视图（≤100），服务训练样本构造（来源多样、边界候选机会，不追求榜单观感）。
- **display_pool_records**：展示池视图（≤50），服务展示/精排输入（仍沿用分桶截断，更强调稳定）。
- **candidate_evidence_rows**：候选证据明细表，一人多条（召回路径、skill/term/paper/collab 等）。
- **pool_debug_summary**：三路原始召回数、去重前/去重后数量、真硬过滤人数（hard delete）、软标记人数（soft_flagged_count）、各桶人数、base/training/display 三层池规模等；调参与排查用。
...
4. **score_candidate_pool(records)**：算 RRF 主体与少量校准项，形成候选中间层 **coarse prior / light fusion score**（`candidate_pool_score`），用于粗排序（非最终精排分、非训练标签）。
...
6. **_apply_hard_filters(records, ...)**：Step3 口径：仅删除明显垃圾/空壳（真硬过滤），其余边界候选保留在 base，并写入 `risk_flags` / `sampleability_flags`（soft flag only）。
7. **_assign_buckets(records)**：**A**=vector+label，**B**=vector+collab，**C**=vector_only，**D**=label+collab，**E**=label_only，**F**=collab_only，**Z**=异常。写 bucket_type、bucket_reasons、各桶计数。
8. **三层输出**：形成 **base candidate layer**（`candidate_pool.candidate_records`），从 base 派生：  
   - **training pool（≤100）**：`candidate_pool.training_pool_records`（Step5 轻量分叉策略，不等于 base 的机械前缀）  
   - **display pool（≤50）**：`candidate_pool.display_pool_records`（沿用分桶截断逻辑）  
   并返回 `training_pool_top_100`、`display_pool_top_50`；`final_top_200` / `final_top_500` 为 deprecated / compatibility only。
...
- `execute`：在 enrich、最小必要过滤（硬过滤仅删明显垃圾；边界候选写 `risk_flags` 软标记保留）与分桶后，形成 **base candidate layer**（`candidate_pool.candidate_records`）；从 base 派生 **training pool（≤100）** 与 **display pool（≤50，分桶截断）**；并对 base 构造 `kgatax_sidecar_rows = [build_kgatax_feature_row(r) for r in candidate_pool.candidate_records]`。
...
本节说明离线索引的**建立方式**、**在系统中的作用**与**设计考量**，对应代码：`src/infrastructure/database/build_index/` 下的 `build_collaborative_index.py`、`build_feature_index.py`、`build_vector_index.py`、`build_vocab_stats_index.py`（及 QueryEncoder 共振词表/领域向量快照等）；路径与开关由 `config.py` 统一配置。
...
历史版本曾提供 `build_work_title_embeddings.py` 与 `data/build_index/work_title_embeddings.db`，用于 Stage5「论文标题 ↔ JD」余弦门控的查表加速。**该门控与离线索引已删除**，`paper_scoring` 不再对论文分乘标题–JD gate；无需再构建标题向量库。

**常驻服务（避免冷启动）**  
- `python -m uvicorn src.core.recall.label_recall_service:app --host 127.0.0.1 --port 8765`  
- `POST /recall`：`query_text`、`domain_id`（默认 `"0"`）、可选 `verbose`。  
- 细粒度累计 `paper_scoring` 耗时：环境变量 `LABEL_PROFILE_STAGE5=1`。
...
- **耗时预期**：快照命中时，冷启动一般可少掉 **约 0.5～3s（词表）+ 约 1～5s（17 条领域 encode，视 CPU 而定）**；**单次召回总耗时**仍主要由 S1 `anchor_ctx`、S5 `paper_contribution`、Neo4j 等决定，快照不直接缩短这些阶段。
...
模块级说明与代码保持一致：`build_vocab_stats_index.py` 负责构建并写入 **`vocab_stats.db`**，包含下列**六类**与 Vocabulary 相关的统计型索引（与脚本文件头 docstring 同步）：

| # | 索引 / 表 | 用途摘要 |
|---|-----------|----------|
| 1 | **领域分布** `vocabulary_domain_stats` | 每词关联论文的领域分布与跨度；标签路降噪 / 排序 / IDF 等 |
| 2 | **领域占比** `vocabulary_domain_ratio` | 按 `(voc_id, domain_id)` 存 `ratio`，便于「目标领域占比 ≥ 阈值」快速筛选 |
| 3 | **共现** `vocabulary_cooccurrence` | 同 Work 下词对频次；与图谱侧共现语义一致；标签路学术/锚点共鸣、`cooc_span` / `cooc_purity` |
| 4 | **共现领域占比** `vocabulary_cooc_domain_ratio` | 共现伙伴领域占比的 freq 加权均值；标签路直接查 `cooc_purity` |
| 5 | **三级领域** `vocabulary_topic_stats` | 优先用 `vocabulary_topic_index.json` 填 field/subfield/topic；缺省词用共现补全 `*_dist` |
| 6 | **概念簇** `vocabulary_cluster`、`cluster_members` | 词→簇、簇→成员；产出 **`cluster_centroids.npy`**（及 `INDEX_DIR` 下相关产物），供簇扩展 |

**数据来源（与代码注释一致）**：领域分布与共现均从主库 `works` 的 `concepts_text` / `keywords_text` 解析，与 vocabulary 映射后聚合，**不依赖 Neo4j**；概念簇依赖 `build_vector_index` 产出的词汇向量与 `vocabulary.entity_type`。

**`VocabStatsIndexer.build_index()` 执行顺序（与 `build_vocab_stats_index.py` 源码一致）**  

类 `VocabStatsIndexer` 的 `build_index()` 按固定顺序调用（各步可依赖 `build_progress` 断点续跑，**不会自动清空表**，重复运行多为增量/覆盖写入）：

1. **`_build_domain_stats()`** → `vocabulary_domain_stats`  
2. **`_build_domain_ratio()`** → `vocabulary_domain_ratio`  
3. **`_build_cooccurrence_index()`** → `vocabulary_cooccurrence`  
4. **`_build_cooc_domain_ratio()`** → `vocabulary_cooc_domain_ratio`  
5. **`_build_topic_stats()`** → `vocabulary_topic_stats`（可选 `vocabulary_topic_index.json`）  
6. **`_build_concept_clusters()`** → `vocabulary_cluster`、`cluster_members`，并 **`np.save`** → `INDEX_DIR/cluster_centroids.npy`  

入口：`if __name__ == "__main__"` 中实例化 `VocabStatsIndexer()` 后调用 `build_index()`。
...
- **触发**：在项目根执行  
  ...
  或进入 `build_index` 目录后 `python build_vocab_stats_index.py`（与源码 `__doc__` 一致）。亦可脚本内按步骤调用 `VocabStatsIndexer`。  
...
- **输出**：SQLite 库 `VOCAB_STATS_DB_PATH`（如 `data/build_index/vocab_stats.db`），内多张表（见上表）；另可选 `cluster_centroids.npy` 等。
...
### 与代码同步：训练流水线 `pipeline.py`

`src/infrastructure/database/kgat_ax/pipeline.py` 中 `main()` 用 `subprocess` **依次**启动同目录下的脚本（源码注释中的阶段名如下）：

| 顺序 | 脚本 | 与 `pipeline.py` 注释一致的职责说明 |
|------|------|----------------------------------|
| 1 | `generate_training_data.py` | 调用 `TotalRecallSystem.execute(is_training=True)` 生成候选中间层（base candidate layer）与 training/display 两类池视图（`TRAINING_POOL_TOP_N=100`、`DISPLAY_POOL_TOP_N=50`）→ 基于候选池来源/证据构造分层样本 → 四级梯度持久化（阶段描述见源码注释为准） |
| 2 | `build_kg_index.py` | 为大规模三元组构建 SQLite 覆盖索引，支持秒级子图采样（源码第 45–46 行；边规模以当前 `kg_final.txt` 为准） |
| 3 | `trainer.py --n_epoch 50 --evaluate_every 5` | CF + KG 联合训练，阶梯采样与评估周期见参数（源码第 49–52 行） |

**注意**：`run_command` 传入的是 `generate_training_data.py` 等**相对文件名**，通常需在 **`kgat_ax` 目录为当前工作目录**下执行 `python pipeline.py`，或改为 `python -m src.infrastructure.database.kgat_ax.generate_training_data` 等形式；生产环境更常见的是**分步单独运行**各脚本并自行传参、监控日志。

训练数据与权重根目录由 **`config.KGATAX_TRAIN_DATA_DIR`** 指定（默认 `data/kgatax_train_data/`），与 `generate_training_data.py`、`trainer.py` 内对 `args.save_dir` / `data_dir` 的赋值一致。
...
- KGAT 分、候选池分、`rule_stability` 如何归一化与加权融合？（默认 **λ_pool=0.2、λ_kgat=0.6、λ_stability=0.2**）
- 推荐理由（代表作、匹配类型）是怎么生成的？主路径、Fuzzy、Fallback 分别何时触发？

本节说明**精排阶段**的细节、流程与设计考虑，对应代码：`src/core/ranking/ranking_engine.py`、`rank_scorer.py`、`rank_explainer.py`；精排在 `TotalCore.suggest()` 中位于「语义导航 → 多路召回」之后，对总召回输出的候选中间层（base candidate layer，`candidate_pool.candidate_records`）在池内做预排序（若候选过多可压缩至 `PRE_RANK_MAX`，默认 200）、KGAT 打分与稳定融合，输出 Top 100 及可解释字段。
...
2. **多路召回**：对同一 query 做向量路 + 标签路 + 协同路，构建候选中间层（base candidate layer，含来源与证据），并派生 training/display 两类池视图；精排阶段在池内进行预排序与（必要时）压缩，再进入 KGAT-AX 深度打分与融合。
3. **精排过滤策略**：若用户指定了领域则用该领域；否则用三个锚点岗位的 `domain_ids` 并集得到 `filter_pattern`。
4. **精排引擎**：`RankingEngine.execute_rank(real_job_ids, candidates, filter_domain=filter_pattern, candidate_pool=...)`，得到最终 Top 100 及解释。

即：**精排输入** = 若干岗位锚点 + **最多约 200 名**召回候选人（`candidate_pool` 完整记录）+ 可选领域过滤；**输出** = 排序后的 100 人及每人一条推荐理由、代表作等结构化信息。

| 维度 | 说明 |
|------|------|
| **输入** | real_job_ids（语义 Top-3 岗位）、candidates（作者 ID 列表，与候选池一致）、`candidate_pool`（含 `candidate_records` 时启用池分与 `rule_stability`）、filter_domain（可选） |
...
| **核心中间产物** | 领域过滤后候选人、kgat_scores、**candidate_pool_score 归一化**、**rule_stability** 归一化、**λ_pool/λ_kgat/λ_stability 加权融合分**、解释链 |
...
**2.3 候选池基础分（pool_scores）与无池回退**

- **目的**：在存在 `candidate_records` 时，用 **`candidate_pool_score`（RRF 融合后的候选池质量分）** 作为精排的一路信号，而不是仅依赖列表下标。
- **实现**：对每个 `active_candidates` 对应位置填入 `candidate_pool_score`；若**未**传入候选池记录，则回退为 **线性名次分**：`torch.linspace(1.0, 0.0, steps=n)`（与旧逻辑兼容）。
- **考虑**：有候选池时，精排融合的是「结构化召回分」而非纯顺序。

**2.4 KGAT 分与三路归一化融合（与 `ranking_engine.py` 一致）**

- **目的**：将 **KGAT 原始分**、**候选池分**、**rule_stability** 归一化到 [0,1] 后加权求和，控制模型与稳健性。
- **实现**：对 `kgat_scores`、`pool_scores`、`rule_stability_scores` 分别做 Min-Max 归一化（必要时加 `1e-8` 防除零）；  
  `final_fusion_scores = λ_pool * pool_norm + λ_kgat * kgat_norm + λ_stability * stab_norm`，  
  默认 **`LAMBDA_POOL=0.2`、`LAMBDA_KGAT=0.6`、`LAMBDA_STABILITY=0.2`**（见 `ranking_engine.py` 顶部常量）。
- **考虑**：**0.6 权重给 KGAT** 表示仍以模型细排为主；**0.2 候选池分**保留多路召回一致性；**0.2 稳定项**抑制异常候选。旧文档中的「0.4 kgat + 0.6 线性召回序」已**被上述三通道替代**（仅在无 `candidate_records` 时，`pool_scores` 才退化为线性序）。
...
整体上，精排阶段在「多路召回 + 领域过滤」之后，用 **RankScorer** 做一次「岗位–候选人」的 KGAT+AX 打分，与 **候选池分**、**rule_stability** 做归一化加权融合，再用 **RankExplainer** 从图谱和注意力生成可解释的代表作与推荐理由，最终输出 Top 100 及每条的结构化详情。  
...
- **`src/core/recall/total_recall.py`**  
  - 定义 `TotalRecallSystem`：
    ...
    - 将 JD 文本扩展为带领域 bias 的查询，并行调用 V/L（向量路额外传入 **`query_text`** 以构建多视角 bundle），再以 V/L 种子跑协同路；
    ...
    - **_assign_buckets**：A～F（vector/label/collab 组合），见 `total_recall.py`；
    - 从 **base candidate layer**（`candidate_pool.candidate_records`）派生 **training pool（≤100）** 与 **display pool（≤50，分桶截断）**，并返回 **CandidatePool**、**kgatax_sidecar_rows**、`training_pool_top_100`、`display_pool_top_50`、`rank_map`。`final_top_200` / `final_top_500` 为 deprecated / compatibility only。
  - 同时提供命令行模式，用于单独测试召回效果。

- **`src/core/recall/vector_query_builder.py`**  
  - 向量路 Step1：将 JD 拆成 **`raw_query` / `compressed_query` / `task_focused_query` / `method_focused_query` / `clause_queries`**（结构规则与招聘套话过滤，不做概念落点）。
- **`src/core/recall/vector_path.py`**  
  - `VectorPath`：加载摘要 Faiss 与 id_map；`recall(query_vector, target_domains, query_text=...)` 内串联 **query bundle → 多路编码与每路 `search_k=500` → fused_dense 融合 → 子句浅层检索 → paper 层 hybrid（标题词面 + 有界 bump）→ `paper_clause_aware_score` 重排 → 领域/衰减 → `accumulate_author_scores` 聚合作者**，并填充 `vector_evidence`。
...
- **`generate_training_data.py`**（已对齐 README 5.5 与四分支导出）  
  - `KGATAXTrainingGenerator`：
    - **训练样本入口**：优先基于 `candidate_pool.candidate_records`（base candidate layer）与 `candidate_pool.training_pool_records`（training pool 视图）构造样本；`final_top_200` / `final_top_500` 为 **deprecated / compatibility only**，不应作为正式训练入口口径。
...
| **KGAT-AX** | `KGATAX_TRAIN_DATA_DIR` | 训练数据根目录，如 `data/kgatax_train_data`（含 `weights/*.pth`、`train.txt`、`kg_final.txt` 等）；`generate_training_data`、`trainer`、`build_kg_index` 均由此读取/写入 |
...
后续不推翻这一设计，并以当前实现为准：`TotalRecallSystem` 已升级为**统一候选中间层构建器**，返回结构化的 `candidate_pool`（`CandidatePool`），显式包含：

- **base candidate layer**：`candidate_pool.candidate_records`（候选中间层主表，一人一条）；
- **training pool**：`candidate_pool.training_pool_records`（训练用视图，≤100；强调来源多样与边界候选机会，不追求榜单观感）；
- **display pool**：`candidate_pool.display_pool_records`（展示/精排输入视图，≤50；仍沿用分桶截断，更强调稳定）；
- **候选证据明细**：`candidate_pool.candidate_evidence_rows`；
- **池级统计**：`candidate_pool.pool_debug_summary`（含 base/training/display 池规模与软标记计数等）。

说明：返回字典中的 `final_top_200` / `final_top_500` 为 **deprecated / compatibility only（legacy field）**，当前语义与 display pool 的 author_id 列表一致（最多 50），不再代表真实规模。
...
- **阶段六：分桶**  
  **当前实现（`total_recall._assign_buckets`）**：按多路命中组合分为 **A～F**（如 A=vector+label，C=vector_only 等），并配合 `bucket_quota_truncate`。下述「A/B/C/D 主题/指标」为早期规划口径，**与代码桶语义不一致时以代码为准**。
...
**已实现**：`generate_training_data.py` 优先基于 `candidate_pool.candidate_records`（base candidate layer）与 `candidate_pool.training_pool_records`（training pool 视图）构造训练样本；`final_top_200` / `final_top_500` 为 **deprecated / compatibility only（legacy field）**，仅用于旧调用兜底，不再代表真实规模，也不应作为正式训练入口口径。
...
- **输出**：当 `len(active_records) > PRE_RANK_MAX`（默认 **200**）时压缩至 **PRE_RANK_MAX**；当前总召回以 **base candidate layer** 作为候选中间层主表（规模随 query 而变），并另有 display pool（≤50）供展示/精排输入；因此该步是否为 no-op 取决于输入使用的是 base 还是 display 视图。保留候选来源、分桶和证据明细。
...
- **训练样本入口与 CandidatePool 对齐（已实现）**：`generate_training_data.py` 优先基于 `candidate_pool.candidate_records`（base candidate layer）与 `candidate_pool.training_pool_records`（training pool 视图）构造样本；`final_top_200` / `final_top_500` 为 **deprecated / compatibility only（legacy field）**，仅保留用于旧调用兜底，不再代表真实规模。
