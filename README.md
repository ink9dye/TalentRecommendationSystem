...
| `check_primary_eligibility` | **资格赛**（在 admission 前）：identity<0.30、similar_to 且 context_sim<0.74、context_fallback 且 ctx_sim<0.82 或 identity<0.50、domain_reason==domain_conflict_strong 时 ⇒ 不争 primary；结果写入 **primary_eligible / primary_eligibility_reasons**。 |
| `check_primary_admission` | 准入纳入**上下文一致性**：① 先 `check_primary_eligibility`，不通过直接 reject；② **主通道**：semantic≥0.80、jd_align≥0.80、anchor_identity≥0.35、**context_consistency≥0.78** ⇒ normal（dual_space_supported）；③ **rescue**：多锚支持 + path/jd/semantic 达标且 **context_consistency≥0.74** ⇒ weak_retain（cross_anchor_context_rescue）；④ **context_fallback 通道**：ctx_sim≥0.84、jd_align≥0.82、anchor_identity≥0.55 ⇒ weak_retain（context_fallback_supported）；⑤ 否则 dual_space_not_stable ⇒ reject。 |
