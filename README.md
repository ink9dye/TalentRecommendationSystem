...
2. **五阶段流水线**：`stage1_domain_anchors`（领域、锚点、`jd_profile`）→ `stage2_expansion`（学术落点与扩展候选）→ `stage3_term_filtering`（词级门控与可选主轴过滤）→ `stage4_paper_recall`（论文二层召回与 grounding）→ `stage5_author_rank`（作者级累计与截断）。
