...
| **Stage2A** | prepared_anchors, active_domain_set, query_vector, query_text, 可选 jd_*_ids | primary_landings（含 **can_expand_from_2a**、**fallback_primary**、**can_expand**（与前者同步）、**bucket**（含 primary_fallback_keep_no_expand）、primary_score 等；_debug 透传至 Stage3） | **最小收尾三点**：① `judge_primary_and_expandability` 将 **primary 资格（宽）** 与 **expand 资格（严）** 解耦 → `select_primary_per_anchor`；无 expandable 时 keep **最多 1 条**；② 仍 0-primary 且锚点允许时 **`pick_fallback_primary_for_anchor`** 保线（不扩、交 Stage3）；③ `merge_stage2a_primary` |
...
| **Stage3** | raw_candidates（含 term_role、role_in_anchor、source_types、retain_mode、**primary_bucket / fallback_primary / mainline_candidate** 等）, query_vector, anchor_vids | score_map, term_map, idf_map, **paper_terms** | 去重聚合 → classify_stage3_entry_groups → check_stage3_admission；**唯一主分** × role_factor；**`_assign_stage3_bucket`**（conditioned_only **永不得 core**；**fallback vs locked mainline**）；风险循环；**`select_terms_for_paper_recall`**（单锚 conditioned_only、**fallback_primary** 不进 paper；**locked mainline** 计入主线条目）；**`_print_paper_term_selection_audit`** |
...
| 函数 | 目的与要点 |
|------|------------|
| **`judge_primary_and_expandability`** | 在原有 expand 四门（`drop_ok_exp`、`branch_blocked`、`dual/multi/solo ctx`）之前插入 **强主线直通**：`source_type` ∈ `similar_to` / `conditioned_vec`，且 **真实** `conditioned_sim`、`static_sim≥0.83`、`ctx_sim≥0.82`、`ctx_drop≤0.08`、`family_match≥0.50`，并满足 **dual_support** 或 **多锚** 或 **family_match≥0.72**；仍受 **branch_blocked** 约束 ⇒ 返回 **`primary_expandable`**、`reason=strong_mainline_direct`，便于 **motion control / robot control** 等拿到 **`can_expand_from_2a=True`** 进入 Stage2B。 |
...
**验收日志**：`[Stage2A expand factors]` 中 **`strong_mainline_direct`**；`[Stage2A fallback]` 是否仍出现 mechanics/kinesiology/simulation；`[Stage2B] Dense expansion funnel` 中 **after_mainline_filter / final_kept** 是否大于 0。
...
- **judge_primary_and_expandability**：**primary** 须 **真实 `conditioned_sim`** 路径或 **多锚高 family** 或 **dual+conditioned**；**expand** 在 **`drop_ok_exp` ∧ 非 branch_blocked** 下走 **dual / multi / solo ctx** 阈值；另设 **`strong_mainline_direct`** 直通 **`primary_expandable`**（双路高 sim、低 ctx_drop、family 支撑，且 **similar_to / conditioned_vec** 来源）；返回桶 + reason。
...
| **逻辑流程** | ① _merge_stage3_duplicates（最佳 seed 记录整表字段保留，含 Stage2A 透传）；② stage2_rank；③ _compute_stage3_global_consensus；④ _build_family_buckets、_compute_family_centrality；⑤ **_classify_stage3_entry_groups**；⑥ should_drop_term → **_check_stage3_admission**；⑦ score_term_record × identity × risk_penalty × **role_factor**；⑧ **`_assign_stage3_bucket`**（conditioned_only + **fallback / locked mainline** 规则，见下表）；⑨ _apply_family_role_constraints；⑩ _collect_risky_reasons、**_collect_stage3_bucket_reason_flags**；⑪ **mainline 风险修正**；⑫ _bucket_stage3_terms；⑬ **`select_terms_for_paper_recall`**（conditioned 单锚、**fallback_primary**、support  grounding、family 去重）；⑭ **`_print_paper_term_selection_audit`**；⑮ 写 map、**_debug_print_stage3_tables**。 |
...
**Stage3 主分**：**compose_term_final_score** 已退出 Stage3 主链（deprecated）。当前：**score_term_record** 产出 raw_final，再乘 **identity_factor**（term_scoring.compute_identity_factor，软因子不硬杀）与 **_compute_stage3_risk_penalty**；final = raw_final × identity_factor × risk_penalty。score_term_record 内部仍含 base_score×gate×cross_anchor_factor×backbone_boost×object_like_penalty×bonus_term_penalty。
...
| **全局分（保留）** | **mainline 风险循环**仍含 conditioned 分层罚分与 **`cross_anchor_score_bonus`**（跨锚排序用），与上列两函数独立。 |
