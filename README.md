...
└── src/
    ...
    │   │       ├── stage3_term_filtering.py  # run_stage3(stage2_output)：校验四键、透传图/分组；→ select_terms_for_paper_recall（JD 主轴门等）→ paper_terms / global_coherence_report；…
...
Stage3 不只是做一个宽泛的 filtering，而是承担词级收束与论文检索词定稿的作用。**`run_stage3`** 校验 **`stage2_output`** 四键并将 **`anchor_to_candidates` / `candidate_graph` / `stage2_report`** 透传到 **`recall._stage3_stage2_context`**（及 debug）；**合并与双闸门仍以 `all_candidates` 扁平行为主**，**候选图尚未参与全局边表重排**（与模块内「壳层契约迁移」说明一致）。该阶段会结合 identity / quality gate、层级适配、主轴一致性以及词级全局重排结果压缩候选，并产出 **`selected_core_terms` / `selected_support_terms` / `risky_terms` / `paper_terms`** 与 **`global_coherence_report`**，主链用 **`paper_terms`**（非空时）经 **`S3_select`** 进入 Stage4，否则回退 **`_select_terms_for_paper`**。
