...
| **Stage2A** | prepared_anchors, active_domain_set, query_vector, query_text, 可选 jd_*_ids | primary_landings（含 **mainline_preference**、**mainline_rank**、**anchor_internal_rank**、**survive_primary**、**can_expand**、**role_in_anchor**、domain_fit、expandable、primary_score 等；_debug 透传至 Stage3） | **主线优先组内选主（无固定阈值）**：build_stage2a_mainline_profile → collect_landing_candidates → build_cross_anchor_index → enrich_stage2a_candidates（证据+build_mainline_preference+mainline_rank+build_stage2a_sort_key 排序）→ select_primary_per_anchor（mainline+can_expand=True，side+can_expand=False）→ merge_stage2a_primary；Stage2B 仅 can_expand=True 扩展 |
...
- **Stage2A 学术落点**：锚点 → 跨类型 SIMILAR_TO + conditioned_vec + alias/exact → **主线优先组内选主**（build_stage2a_mainline_profile、build_mainline_preference、build_stage2a_sort_key 以 mainline_rank 为首位；无 mainline/off 一票否决）→ primary_landings（每锚点 1～2 个主落点，**primary 与 can_expand 分离**：mainline 可扩、side 保留不扩）；Stage2B 仅 can_expand=True 扩展。
...
| **逻辑流程** | ① `build_stage2a_mainline_profile(recall, anchors, query_vector, query_text)`；② 每锚 `collect_landing_candidates` → `landing_candidates_to_stage2a`；③ `build_cross_anchor_index(per_anchor_candidates)`；④ 每锚 `enrich_stage2a_candidates`（证据 + **build_mainline_preference** + assign_relative_scores_within_anchor（含 **mainline_rank 由复合主线偏好**）+ **build_stage2a_sort_key** 排序 + anchor_internal_rank/sort_key_snapshot）；⑤ `select_primary_per_anchor`（主线优先排序后 top1 必 mainline+can_expand=True，top2 若 is_competitive_runner_up 则 side+can_expand=False）；⑥ `merge_stage2a_primary`（mainline_hits、support_count、best_rank、can_expand）；⑦ Stage2B 仅 `can_expand=True` 进 diffusion_primaries。 |
...
- **select_primary_per_anchor**：返回 `{ primary_expandable, primary_keep_no_expand, rejected }`；仅 normal 且非 suppress_seed 的 top1 为 primary_expandable，runner-up 为 primary_keep_no_expand。
- **merge_stage2a_primary**：按 (mainline_hits, support_count, -best_rank, can_expand) 排序；合并字段含 anchors、support_roles、retain_modes；Stage2B 仅对 can_expand=True 的 primary 做扩展。
...
| **score_stage2a_candidate**（新增） | — | 主线资格分 mainline_pref_score 与保留资格分 retain_score 分开 | `mainline_pref_score`、`retain_score`、`mainline_admissible` |
...
| **select_primary_per_anchor** | 组内第一名自动 expandable、候选二号默认保留 | 三桶：按 mainline_pref_score 排序；mainline_admissible→primary_expandable；否则 retain_score≥RETAIN_MIN→primary_keep_no_expand；否则 reject | `bucket`、`role_in_anchor`、`can_expand`、`admission_reason` |
| **merge_stage2a_primary** | support_count 高就更可信、多锚洗白 generic/shifted | 跨锚支持仅弱加分；family_type 为 generic/shifted 时 effective_bonus = raw_bonus×0.3 | `cross_anchor_support_raw`、`cross_anchor_support_effective`、`cross_anchor_bonus`、`family_type` |

常量：`CTX_SUPPORT_MIN`、`CTX_GAP_SHIFT_MIN`、`HIER_WEAK_MIN`、`JD_ALIGN_WEAK_MIN`、`GENERIC_RISK_MIN`、`POLY_RISK_MIN`、`MAINLINE_IDENTITY_MIN`、`RETAIN_MIN`。  
...
| **select_primary_per_anchor** | rank1=mainline/rank2=side 旧二元逻辑、无 normal 即整锚归零、weak_retain 等同 reject | 明确输出：primary_expandable（normal 且非 suppress 的 top1）、primary_keep_no_expand（其余 kept 的 top1）、rejected | role_in_anchor, can_expand, anchor_internal_rank, primary_bucket |
...
**主流程**：`collect_landing_candidates`（召回 + **分型**：family_type / generic_like / scene_shifted / 四段式 identity）→ `landing_candidates_to_stage2a`（复制定性字段与分型）→ `build_cross_anchor_index` → `enrich_stage2a_candidates`（证据 + **score_stage2a_candidate** → mainline_pref_score / retain_score / mainline_admissible + admission + mainline_preference）→ `select_primary_per_anchor`（按 mainline_pref_score 排序，**三桶**：mainline_admissible→primary_expandable，否则 retain_score≥RETAIN_MIN→primary_keep_no_expand，否则 reject）→ 为选中候选挂 `parent_anchor` / `parent_anchor_obj` → `merge_stage2a_primary`（跨锚仅弱加分，generic/shifted 不洗白）；Stage2B 仅对 **primary_expandable**（can_expand=True）做扩散。

**调试日志**（`LABEL_EXPANSION_DEBUG` + `STAGE2_VERBOSE_DEBUG` 时）：  
① **Stage2A 候选定性表**：anchor \| term \| semantic \| jd_align \| ctx_cont \| anchor_id \| hier \| poly \| object \| generic \| retain_mode \| suppress_seed \| mainline_pref；  
② **Stage2A 选主结果表**：anchor \| primary_expandable \| primary_keep_no_expand \| rejected_top_reasons；  
...
- **Stage2A 主线优先组内选主**：主流程为 `build_stage2a_mainline_profile → collect_landing_candidates → landing_candidates_to_stage2a → build_cross_anchor_index → enrich_stage2a_candidates`（证据 + **build_mainline_preference** + mainline_rank 由复合主线偏好 + **build_stage2a_sort_key** 排序）→ `select_primary_per_anchor`（mainline+can_expand=True，side+can_expand=False）→ `merge_stage2a_primary`。**不再**使用 mainline_sim/role 固定阈值一票否决；**primary 与 can_expand 分离**，生死由主线优先排序与 `is_competitive_runner_up` 决定。
- **Stage2B 固定成两层门**：**Seed 门**（谁能扩）：**仅 can_expand=True 的 primary** 进入 diffusion_primaries（由 Stage2A 直接赋值，无 fallback）；**Support 门**（扩出来的词谁能留）：dense/cooc 候选须过 **support_expandable_for_anchor**，domain_fit≥0.72、sim≥0.55，设备/对象词默认拒。**Cluster 当前全关**（`expand_from_cluster_members` 直接 return []）。
