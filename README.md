...
| `expand_from_vocab_dense_neighbors` | 入口对每个 primary 再跑 **check_seed_eligibility**，不通过则跳过。**Dense 最小修复补丁**：候选须过 **support_expandable_for_anchor** 四道门（primary_consistency≥0.72、anchor_consistency≥0.70、context_stability≥0.72、family_support≥0.68），不靠硬编码；identity_score 用 keep_score（四门加权）。sim≥0.55、domain_fit≥0.72；**设备/对象词**默认拒（除非 anchor_allows_device_expansion）；每 seed 最多 **2 条**，强 seed 最多 **3 条**。锚点 context（conditioned_vec、_context_neighbors、_context_score_map）由 collect_landing_candidates 写 anchor、stage2 写 diffusion_primaries 供 gate 使用。 |
...
| **Dense 最小修复补丁** | **目标**：拦住 robotic arm→robotic hand/robot hand、motion control→motion controller、robot control→autonomous robot 等脏扩散，且不靠硬编码。**思路**：候选不仅“像 parent primary”，还要“仍像当前 anchor 主线”。**support_expandable_for_anchor(label, anchor, parent_primary, candidate_vid, candidate_term)** 返回 (keep, meta)。四道门：① primary_consistency = cosine(primary_vec, candidate_vec) ≥ 0.72；② anchor_consistency = cosine(anchor.conditioned_vec, candidate_vec) ≥ 0.70（无 conditioned_vec 时跳过）；③ context_stability = estimate_context_support（缓存的 _context_score_map 或与 _context_neighbors 相似度）≥ 0.72（无 context 数据时跳过）；④ family_support = estimate_anchor_family_support（同锚 surviving primary 支撑，含 drift_penalty）≥ 0.68（无同锚 primary 时跳过）。keep_score = 0.30×primary + 0.30×anchor + 0.25×context + 0.15×family，作为 dense 的 identity_score。辅助：_get_vocab_vec、_estimate_context_support、_estimate_anchor_family_support。 |
