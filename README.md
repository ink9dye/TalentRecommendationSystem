...
6. **线上召回与精排**
   - **总召回**（`core/recall/total_recall.py`）：构建**候选池**。三路召回合并去重后，经硬过滤、分桶、`bucket_quota_truncate` 截断，**默认最多保留 `FINAL_POOL_TOP_N`（200）条** `CandidateRecord`（去重后人数可大于 200，截断前统计见 `pool_debug_summary`）。返回值中 **`final_top_200` 与 `final_top_500` 指向同一列表**，`final_top_500` 仅为历史/兼容键名。向量/标签/协同单路 `recall_limit` 默认分别为 **150 / 150 / 80**，与「约 500」无关。**总召回不负责最终排序**，只决定谁进入候选池。
...
| **总召回** | 统一编码；**并行**向量路 + 标签路；再以 V/L Top 种子跑协同路；合并为 `CandidateRecord` 列表，经 RRF 融合、特征补全、硬过滤、分桶、配额截断；**不负责全库最终排序** | `src/core/recall/total_recall.py`（`TotalRecallSystem`）；依赖 `vector_path.py`、`collaboration_path.py`、`candidate_pool.py`、`candidate_features.py` |
...
4. **线程池并行**：`v_path.recall(query_vec, target_domains=vector_domains)` 与 `l_path.recall(...)` 同时执行；完成后取 **`v_meta` 与 `l_meta` 各自前 100 条**中的 `author_id` 做并集作为 **协同路种子**，调用 `c_path.recall(seeds)`（协同路依赖种子，故晚于 V/L 完成）。
5. `build_candidate_records` → `score_candidate_pool`（RRF，`PATH_WEIGHTS` v/l/c；`pair_path_bonus`；`label_hint`）→ `_enrich_candidate_features`（SQLite 作者指标、代表作与 query 交叉特征）→ `_apply_hard_filters` → `_assign_buckets`（A～F）→ `bucket_quota_truncate`（`BUCKET_QUOTAS` + **`FINAL_POOL_TOP_N=200`**）。
6. 对截断后的每条记录生成 **`kgatax_sidecar_rows`**，并封装 `CandidatePool`（含 `path_costs`、`domain_debug`）。返回的 **`final_top_200` / `final_top_500` 均为截断后的作者 ID 列表（内容相同，`final_top_500` 为兼容键）**。
...
| 协同路 | 无 | 有，且依赖 V/L 种子 |
| 融合分 | 单路作者分 | `candidate_pool_score`（RRF+奖励） |
| 下游 | 仅展示列表 | `kgatax_sidecar_rows` + `CandidatePool` 供精排/训练 |

**默认常量（与 `total_recall.py` 对齐，修改代码时请同步本文）：** `k_vector=150`、`k_label=150`、`k_collab=80`；协同路种子为 **`v_meta` / `l_meta` 各自前 100 条**的作者并集；`FINAL_POOL_TOP_N=200`；`RRF_K=60`；`PATH_WEIGHTS` 为 `v:l:c = 2:3:1`；`BUCKET_QUOTAS` 见源码字典。
...
| **核心中间产物** | Faiss 阶段检索 **search_k=500** 篇候选 work（与作者截断 `recall_limit` 不同）、领域过滤后的 work 列表、work→author 映射与排序 |
...
4. **去重与截断**  
   同一作者可能在多篇论文中出现，只保留第一次出现的位置（GROUP BY author_id + 上述 ORDER BY）；返回前 **`recall_limit` 个** author_id 及耗时（`TotalRecallSystem` 默认 **150**，独立测试脚本可能使用其它上限；**勿与 Faiss search_k=500 混淆**）。
...
**核心目的**  
对 JD **只编码一次**得到统一 query_vec；**并行**跑向量路与标签路，再用两路各 **Top 100** `author_id` 并集做种子跑协同路；将三路结果**构建为统一候选池**（CandidateRecord + CandidatePool），经**合并 → 打分 → 特征补全（enrich）→ 硬过滤 → 分桶 → 分桶配额截断（默认最多 200 人）**后导出，供**精排与训练**直接使用。  
...
| **输出** | **candidate_pool**（候选主表 + 证据明细 + pool_debug_summary）、**final_top_200 / final_top_500**（同列表）、**kgatax_sidecar_rows**、rank_map、总耗时 |
...
6. **_apply_hard_filters(records, ...)**：仅协同命中、无论文无指标等；**label_only** 时按 `domain_consistency`、`paper_hit_strength`、`recent_activity_match`、risky/core 词规则过滤（见 `total_recall.py`）。写 hard_filter_reasons、pool_debug_summary.hard_filtered_count。
...
8. **bucket_quota_truncate** → 截断至 `FINAL_POOL_TOP_N`；**组装 CandidatePool 并返回**：candidate_records、candidate_evidence_rows、pool_debug_summary；**final_top_200** 与 **final_top_500**（同内容，兼容键）、rank_map。
...
|------|------|----------------------------------|
| 1 | `generate_training_data.py` | 调用 `TotalRecallSystem.execute(is_training=True)` 生成候选池与样本（规模以 `FINAL_POOL_TOP_N`、配额为准，非固定 500）→ RRF 融合与分层标签 → 四级梯度持久化（阶段描述见源码第 41–42 行） |
...
本节说明**精排阶段**的细节、流程与设计考虑，对应代码：`src/core/ranking/ranking_engine.py`、`rank_scorer.py`、`rank_explainer.py`；精排在 `TotalCore.suggest()` 中位于「语义导航 → 多路召回」之后，对**总召回截断后的候选池**（默认最多 **200** 人，与 `FINAL_POOL_TOP_N` 一致）做预排序（可选压缩至 `PRE_RANK_MAX`，默认亦为 **200**）、KGAT 打分与稳定融合，输出 Top 100 及可解释字段。
...
2. **多路召回**：对同一 query 做向量路 + 标签路 + 协同路，RRF 融合与分桶截断后，默认最多 **200** 名进入 `candidate_pool.candidate_records`（与总召回 `FINAL_POOL_TOP_N` 一致）。
...
- **`src/core/recall/total_recall.py`**  
  - 定义 `TotalRecallSystem`：
    ...
    - **bucket_quota_truncate** 后返回 **CandidatePool**、**kgatax_sidecar_rows**、**final_top_200**（与 **final_top_500** 同内容）、**rank_map**。
...
- **`generate_training_data.py`**（已对齐 README 5.5 与四分支导出）  
  - `KGATAXTrainingGenerator`：
    - **训练样本入口**：优先基于 `candidate_pool.candidate_records` 构造样本；不足时回退到 **`final_top_500`（与 `final_top_200` 同内容）**。
...
**已实现**：`generate_training_data.py` 优先基于 `candidate_pool.candidate_records` 构造训练样本；当候选池不足或不可用时回退到返回字典中的 **`final_top_500`（与 `final_top_200` 为同一列表）**，训练数据主入口与线上候选池保持一致。
...
- **输出**：当 `len(active_records) > PRE_RANK_MAX`（默认 **200**）时压缩至 **PRE_RANK_MAX**；总召回默认已截断至 **200**，多数请求下此步为 no-op；保留候选来源、分桶和证据明细。
...
- **训练样本入口与 CandidatePool 对齐（已实现）**：`generate_training_data.py` 优先基于 `candidate_pool.candidate_records` 构造样本；若不可用则回退到返回字典中的 **`final_top_500`（与 `final_top_200` 相同列表，仅键名兼容）**。
