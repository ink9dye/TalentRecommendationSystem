...
标签路不是单次图查询，而是一条按「锚点准备 → 学术落点 → 词级收束 → 论文验证 → 作者聚合」展开的多阶段流水线。对应实现位于 `label_pipeline/stage1_domain_anchors.py` 至 `stage5_author_rank.py`，整体由 `LabelRecallPath.recall(...)` 串联调用。

##### Stage1：领域与锚点准备

Stage1 的职责不只是抽取岗位锚点，而是为后续扩展阶段准备一整套可复用的上下文状态。该阶段会结合领域探测结果、岗位技能抽取结果和 JD 文本本身，生成 `active_domains`、工业锚点集合、锚点局部上下文，以及供后续层级守卫使用的 `jd_profile`。在当前实现里，还会在 **技能短语进入锚点抽取之前** 做 **JD 早期粗分型**（`build_jd_coarse_roles_map`，结果挂到 `recall._jd_coarse_roles`），锚点生成后由 **`refine_stage1_anchor_layers`** 统一写回 **`coarse_jd_role` / `anchor_role` / `is_primary_eligible`**、**`final_anchor_score`**（含向量补充锚封顶、上下文-only 帽等）与流程化 **`anchor_type`**；**`context_only`** 类锚点会从主 `anchor_skills` 中剥离，侧挂到 **`recall._stage1_context_only_anchors`** 供审计。随后 **`attach_anchor_contexts`**、条件化向量（`conditioned_vec` 等），以及 **`build_jd_hierarchy_profile`**（多锚点一次性 `SIMILAR_TO` 批量查询后在 Python 侧各锚取 top-k，等价于原 per-anchor Cypher）产出 **`jd_profile`**。结果壳 **`Stage1Result`**（含 `jd_profile`）写入 **`recall._last_stage1_result`**。因此它更接近「标签路的准备阶段」，而不是单纯的关键词抽取。

##### Stage2：学术落点与扩展候选生成

Stage2 负责把岗位侧锚点落到学术词汇空间，并在此基础上生成后续可筛选的扩展候选。其核心不是简单扩词，而是先为每个锚点寻找更合适的 primary landing，再围绕主落点补充相关学术词、同簇词或共鸣词，形成带有层级适配度、落点评分和来源信息的候选集合。实现上仍细分为 **Stage2A / Stage2B**（`label_expansion.stage2_generate_academic_terms`），详见下文「正确分层图」与数据流表。

**对外契约（当前仓库）**：**`stage2_expansion.run_stage2` 返回结构化 `dict`**，包含 **`all_candidates`**（与 Stage3 合并逻辑衔接的平铺列表，语义同原先的 `raw_candidates`）、**`anchor_to_candidates`**（按锚分组的节点视图，含 `landing_candidates` / `expansion_candidates` 槽位及 `summary`；分组键优先 `anchor_id`，缺省则回退 **`anchor_term` / `parent_anchor`**）、**`candidate_graph`**（MUV：四类边键 **`same_anchor_edges` / `cross_anchor_support_edges` / `family_edges` / `provenance_edges`**，由 **`build_stage2_candidate_graph`** 构建）、**`stage2_report`**（规模、按锚统计、`source_distribution`、`graph_edge_counts` 等）。**`LabelRecallPath.recall`** 将 **整份 `stage2_output`** 传入 **`stage3_term_filtering.run_stage3`**（检查点与诊断仍可用 **`stage2_output["all_candidates"]`** 计数与回填）。

**单条候选字段分层**：**`_expanded_to_raw_candidates`** 将强 provisional 痕迹（如 **`primary_bucket`、`fallback_primary`、`reject_reason`** 等）主要收拢到 **`stage2_local_meta`**；顶层以标识、局部证据（identity/sim、层级 fit、`risk_flags`、`landing_confidence` / `expansion_confidence` 等）为主。为兼容 Stage3 合并逻辑，**仍保留少量顶层镜像字段**（待下游完全以 meta 为准后可删）。**`risk_flags`**、**`_debug`** 等便于观测与回溯。

##### Stage3：Global Coherence Rerank（词级聚合与全局重排）

**正式职责（Step0 冻结）**：**term-level aggregation**、**cross-anchor coherence**、**global rerank**、**light guardrail**。**`run_stage3`** 校验 **`stage2_output`** 四键并将 **`anchor_to_candidates` / `candidate_graph` / `stage2_report`** 透传到 **`recall._stage3_stage2_context`**（及 debug）。**主链自然终点**为 **`stage3_result["ranked_terms"]`**；**Stage4 prep** 仅后置消费 `ranked_terms`（如 **`_prepare_stage4_terms_from_stage3`**），再产出供下游使用的 **`paper_terms`** 等。**paper lane / dynamic floor / tail expand / swap / quota** 在边界上**不属于** Stage3 主路径职责。实现上仍结合 identity / quality gate、层级适配与词级全局重排，并保留 **`selected_*` / `paper_terms` / `global_coherence_report`** 等兼容字段；**合并与双闸门仍以 `all_candidates` 扁平行为主**，**候选图尚未参与全局边表重排**（与 `note/stage2_3_4_refactor_plan.md` 0.3 一致）。**`recall`** 在 Stage3 之后仍按既有逻辑优先使用 **`paper_terms`**（非空时经 **`S3_select`** 进入 Stage4，否则回退 **`_select_terms_for_paper`**）。

##### Stage4：论文层召回与证据打分

Stage4 以上一阶段保留下来的 term 集合为输入，回到论文层进行二次验证。这里的目标不是继续扩概念，而是检验这些词是否真的能在论文侧形成较强的主题贴合与 grounding 证据。实现上，这一阶段会把词级信号落到论文候选集合上，并结合 grounding、偏题惩罚等因素生成论文级分数，为最终作者聚合提供更扎实的证据基础。

##### Stage5：作者层聚合、排序与截断

Stage5 负责把论文级证据重新汇总到作者级，形成标签路输出的最终作者排序结果。该阶段会累计论文贡献、整理作者侧 evidence，并执行最终排序与截断。当前实现中，Stage5 的核心定位是「作者层聚合与排序输出」；部分结构一致性或平衡性项在代码中仍保留为预留位，因此本文档对这一阶段的表述保持相对保守，不写成已经实现了完整复杂的作者层一致性优化框架。
