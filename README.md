...
| **Stage2A** | prepared_anchors（含 **source_type/source_weight**）, active_domain_set, query_vector, 可选 jd_field/subfield/topic_ids | primary_landings（含 domain_fit、**hierarchy_reason/hierarchy_score/hierarchy_level**） | retrieve_academic_term_by_similar_to；**_term_in_active_domains_with_reason**（主领域守卫；topic_hierarchy_no_match 不再剔除）；collect_landing_candidates 为每候选写 hierarchy_*；**_hierarchy_norm** 保守映射（HIERARCHY_NORM_*）+ identity 联动上限；primary 最后 **× anchor.source_weight**（补充锚点 0.65）；select_primary_academic_landings（domain_fit≥0.45 才可做 primary） |
...
| **逻辑流程** | ① anchor_skills（含 **anchor_source/anchor_source_weight**）转为 **PreparedAnchor**（含 **conditioned_vec**、**source_type**、**source_weight**）；② **数据驱动两阶段选主**：对每个 anchor 调用 `collect_landing_candidates`（…）；每锚保留 **top-m** 候选；③ flat_pool 上 `_compute_neighborhood_and_isolation` 与 **`_compute_conditioned_anchor_align_and_multi_anchor_support`**；④ **primary_score** = (edge_affinity + conditioned_anchor_align + jd_align + **hierarchy_consistency**（保守映射 + identity 联动）+ neighborhood + multi_anchor − isolation)×identity_gate×… **× anchor.source_weight**（补充锚点 0.65）；⑤ 每锚按 primary_score 排序，满足 min_identity/domain_fit_min 的取前 PRIMARY_MAX_PER_ANCHOR 个为 primary；⑥ 产出 **stage2_anchor_evidence_table**。 |
...
2. **两阶段选主**  
   ...
   - 第二阶段：对所有锚点的候选组成 **flat_pool**，统一算 `_compute_neighborhood_and_isolation` 与 **`_compute_conditioned_anchor_align_and_multi_anchor_support`**，再算 **base_primary**（同上式），最后乘 **Identity Gate** 得 **primary_score**：  
     `base_primary = w1·edge_affinity + w2·conditioned_anchor_align + ... - w7·semantic_isolation_penalty`  
     `primary_score = base_primary × identity_gate × (1 + β·anchor_identity_score)`  
     其中 **identity_gate** 由 **anchor_identity_score**（候选与锚点“本义”一致性）软映射：≥0.75→1.0，≥0.55→0.9，≥0.35→0.72，否则 0.45；β=IDENTITY_GATE_BETA（默认 0.08）。  
   - 每锚点先按 **primary_cap**（conditioned_vec 且 anchor_identity_score<0.30 的标为 secondary_only）再按 primary_score 排序，在满足 `min_identity`、`domain_fit_min` 的前提下取前 **PRIMARY_MAX_PER_ANCHOR** 个作为 primary，再进入 Stage2B 扩散。
...
| 9 | **Identity Gate（锚点本义守门）**：`normalize_identity_surface` + `compute_anchor_identity_score` 算候选与锚点“本义”一致性；`collect_landing_candidates` 内为每候选设 anchor_identity_score、identity_gate、primary_cap（conditioned_vec 且 identity<0.30→secondary_only）；primary_score = base_primary × identity_gate × (1+β·anchor_identity_score)；选 primary 时先非 secondary_only 再按 primary_score；压制 动力学→propulsion、运动学→kinesics、仿真→simula、抓取→Data retrieval 等错义 | `label_expansion` 中 normalize_identity_surface、compute_anchor_identity_score、_identity_gate_from_score；collect_landing_candidates；primary 打分循环与 pool_for_anchor 排序；`[Stage2A Identity Gate]` 打印 |
...
| **topic_hierarchy_no_match 不再一票否决** | `_term_in_active_domains_with_reason`、`_term_in_active_domains` | 当三级层级均未命中（原 `topic_hierarchy_no_match`）时**不再剔除**候选：仍返回通过、仅保留 reason 供日志；候选进池后在 **primary 打分**中通过 **hierarchy_level** 惩罚。`collect_landing_candidates` 为每个候选写入 **hierarchy_reason**、**hierarchy_score**、**hierarchy_level**（topic/subfield/field/none/missing）。**_hierarchy_norm** 使用**保守映射**（config：HIERARCHY_NORM_TOPIC=0.75、SUBFIELD=0.45、FIELD=0.20、NONE=0.05）；**identity 联动上限**：当 `anchor_identity_score < HIERARCHY_IDENTITY_THRESHOLD` 时 hierarchy 再乘 `HIERARCHY_IDENTITY_DISCOUNT`，避免 propulsion/simula/control flow 等 topic 命中但本义不对的词拿满 hier。 |
...
| **输出参数（名字与含义）** | **raw_candidates**：每项含 tid、term、term_role、identity_score、source、degree_w、domain_span、topic_align、domain_fit、parent_anchor、parent_primary，以及**层级守卫相关**：**subfield_fit**、**topic_fit**、**outside_subfield_mass**、**topic_entropy**、**landing_score**、**cluster_id**、**main_subfield_match**；Stage2A 候选上还挂 **hierarchy_reason**（如 `topic_hierarchy_no_match` 或空）、**hierarchy_score**、**hierarchy_level**（topic/subfield/field/none/missing），供 primary 打分中的 _hierarchy_norm 使用。供 Stage3 做 should_drop_term 与 score_term_record。 |
...
| **label_expansion** | 学术落点（跨类型 SIMILAR_TO）与学术侧补充（dense、簇、共现）；**get_vocab_hierarchy_snapshot**、**collect_landing_candidates(jd_profile)** 做层级 fit 与 landing 打分，并为每候选写 **hierarchy_reason**、**hierarchy_score**、**hierarchy_level**；**_hierarchy_norm** 保守映射（HIERARCHY_NORM_*）+ **identity 联动上限**（identity&lt;阈值时 hierarchy×折扣）；**_anchor_skills_to_prepared_anchors** 带出 **source_type**、**source_weight**；Stage2A primary 最后 **× anchor.source_weight**（补充锚点 0.65）；**_term_in_active_domains_with_reason** 对 topic_hierarchy_no_match 不再剔除；**allow_primary_to_expand** 参与扩散门控、merge 时写 **cluster_id** 与层级字段 | Stage2A / Stage2B |
