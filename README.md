...
| **核心中间产物** | **Stage1**：Stage1Result（**含 jd_profile**）、anchor_skills（含 local_context、phrase_context、**anchor_source/anchor_source_weight**）、_jd_cleaned_terms、job_ids、regex_str。**Stage2A**：primary_landings（含 domain_fit、**landing_score**、**subfield_fit**、**topic_fit** 等；有 jd_profile 时做层级 fit 与硬门槛）。**Stage2B**：raw_candidates（term_role、domain_fit、parent_anchor、parent_primary、**subfield_fit**、**topic_fit**、**landing_score**、**cluster_id**、**main_subfield_match**；**`run_stage2`→`_expanded_to_raw_candidates` 顶层透传** **`primary_bucket`、`can_expand_from_2a`、`fallback_primary`、`admission_reason`、`reject_reason`、`survive_primary`、`stage2b_seed_tier`、`mainline_candidate`、`primary_reason`**，供 Stage3 聚合与 `_support_grounded_enough` 消费）。**Stage3**：**按 tid 去重聚合**（保留多来源与 best_*）、**classify_stage3_entry_groups**（trusted_primary/secondary_primary/support_expansion）、**check_stage3_admission**（trusted 不硬杀）、**唯一主分** score_term_record×**identity_factor**×**risk_penalty**、**assign_stage3_bucket**（core/support/risky）、**select 按 core→support→risky**、_collect_risky_reasons/_bucket_stage3_terms；score_map、term_map、idf_map、term_role_map、term_source_map、parent_anchor_map、parent_primary_map、tag_purity_debug（含 **stage3_entry_group**、**identity_factor**、**risk_penalty**、**stage3_bucket**、anchor_count、evidence_count、family_centrality、path_topic_consistency、generic_penalty、cross_anchor_factor、retrieval_role 等）；label_path 据此构建 term_confidence_map、term_uniqueness_map。**Stage4**：author_papers_list。**Stage5**：paper_map、contribution、作者聚合分、最终 author_id 列表。 |
...
| 维度 | 说明 |
|------|------|
| **职责** | 把每个工业锚点，落到「候选学术主词集合」上。**关键词**：定性优先，排序其次。 |
| **输出** | `per-anchor candidate pool`、`primary_expandable`、`primary_keep_no_expand`、`dropped` |
| **不负责** | 大规模扩散；全局 family 竞争；paper recall 适格判定 |
...
| 维度 | 说明 |
|------|------|
| **职责** | 围绕可信 primary，补同家族近邻词。**关键词**：主词扩族，不负责重新翻译锚点。 |
| **输出** | `dense_expansion`、`cooc_expansion`、`cluster_expansion`、`merged anchor-family terms` |
| **不负责** | 推翻 Stage2A 的主词身份；全局 term 最终胜负 |
...
| 维度 | 说明 |
|------|------|
| **职责** | 跨锚点、跨来源、跨家族，做统一去歧义与 paper-term 选择。**关键词**：重审，不是继承 Stage2A 判决。 |
| **输出** | `final_term_ids_for_paper`、`risky_terms`、`support_terms`、`core_terms` |
| **不负责** | 重新生成候选池；重新做 anchor 翻译 |
...
- **Stage2A 学术落点**：锚点 → 跨类型 SIMILAR_TO + conditioned_vec + alias/exact → **组内选主**：`judge_primary_and_expandability` + `_classify_keepish_primary_bucket` 将候选分为 **五层**（见下节）：`primary_expandable`（强 seed）、`primary_support_seed`（弱 seed）、`primary_support_keep`（支线保留）、`risky_keep`（高风险弱保留）、`reject`。**兼容字段** `primary_keep_no_expand` = 后三者与弱 seed 的并集（旧代码路径）。无 **强 expandable** 时，非 reject 组 **合计仅 top1**；仍 **0-primary** 时对合格 **canonical** 锚点 **`pick_fallback_primary_for_anchor`**（`primary_fallback_keep_no_expand`，不扩）。**`stage2b_seed_tier`**：`strong` | `weak` | `none`。
- **Stage2B 学术侧补充**：**`can_expand_from_2a=True`**（强档 + **弱 seed**）经 `check_seed_eligibility`；排序裁剪 **强 seed：`SEED_SCORE_MIN`；弱 seed：`SEED_SCORE_MIN_WEAK`**。弱 seed 在 `expand_from_vocab_dense_neighbors` 中 **收紧 top_k / max_keep**。**fallback / 非 seed 桶不扩散**。**cluster 全关**；**support_expandable_for_anchor**；**DENSE_PARENT_CAP=0.85**；输出 List[ExpandedTermCandidate]（primary / dense_expansion / cooc_expansion）。
- **Stage3 全局复审层（非全杀门）**：去重聚合 → classify_stage3_entry_groups → check_stage3_admission → **唯一主分** = score_term_record × identity_factor × risk_penalty × **role_factor**。**Role 因子**：mainline primary × **STAGE3_MAINLINE_BOOST(1.06)**、dense_expansion × **STAGE3_DENSE_PENALTY(0.90)**、conditioned_vec × **STAGE3_CONDITIONED_PENALTY(0.94)**、side × **STAGE3_SIDE_PENALTY(0.95)**；**assign_stage3_bucket** 消费 Stage2A **primary_bucket / fallback_primary / mainline_candidate / can_expand_local**；**select_terms_for_paper_recall** 仅 **core→support**（**risky** 不进 paper），上限 12；日志三张表：Entry Group、Admission、Scoring。
...
**判定顺序（实现）**：① 坏分支 → `reject`；② `primary_ok` 不成立 → `best_sim≥STAGE2A_RISKY_KEEP_MIN_BEST_SIM` 则 **`risky_keep`**，否则 **`reject`**；③ 无真实 `conditioned_sim` → `_classify_keepish_primary_bucket(..., no_real_ctx=True)` → 支线 **`primary_support_keep`** 或 **`risky_keep`**；④ **强主线 / 窄逃生口 / good_mainline** → **`primary_expandable`**；⑤ 其余在 **`_classify_keepish_primary_bucket`** 中按 **generic/poly/object** 与 **`mainline_admissible`** 分流为 **弱 seed**（`primary_support_seed`）、**支线 keep** 或 **`risky_keep`**。常量：`STAGE2A_WEAK_SEED_*`、`STAGE2A_RISKY_KEEP_HIGH_*`、`SEED_SCORE_MIN_WEAK`、`DENSE_MAX_PER_PRIMARY_WEAK`。
...
| **`[paper_term_selection gate]`** | **`_print_paper_term_selection_audit`**（`STAGE3_PAPER_GATE_DEBUG`） | 每条打印 **`paper_final_score_need`**（与 **`primary_bucket` 对齐的 support `final_score` 门槛：seed **0.42** / keep 等 **0.46**），与 **`paper_grounding` / `gate`** 对照 |

**Stage3 `stage3_build_score_map` 与 paper 门槛（与五层对齐）**

- **risky 桶**：统一再乘 **0.72**（在原有 reason_flags 压制之外）。
- **support 桶**：在桶内再分档——**primary_support_seed ×0.92**、**primary_support_keep**（含旧 **`primary_keep_no_expand` 别名**）**×0.82**；**conditioned_only** 仍走原有 B 段链式压制。
- **Paper support**：`select_terms_for_paper_recall` → `_support_grounded_enough`：在 cross-anchor / 窄豁免与 **`paper_grounding`** 之前，先有 **`final_score` 前置门**，且按 **`primary_bucket` 分层**：**`primary_support_seed` → `paper_final_score_need=0.42`**；**`primary_support_keep` / `primary_keep_no_expand` 及其它** → **0.46**（不把 keep、risky、conditioned_only 一并放宽）。默认需 **`(anchor_count≥2 ∧ mainline_hits≥1) ∨ mainline_hits≥2`** 才进入 `paper_grounding`；**窄豁免**：**`primary_support_seed`** + **单锚** + **`mainline_hits≥1`** + **`can_expand`（含 `can_expand_from_2a`）** 亦可进入 **`paper_grounding`**。**primary_support_seed** 的 **`paper_grounding`≥0.18**，**primary_support_keep**（含 `primary_keep_no_expand`）**≥0.26**。support 落选时写入 **`paper_final_score_need`**（本条适用的总分门槛）、**`paper_grounding` / `paper_grounding_need` / `paper_support_gate`**（如 `grounded_support_ok`、`paper_grounding`、`final_score_floor`、`risk_caps`）；**`[paper_term_selection gate]`** 在 **`paper_final_score_need` 非空时**打印该门槛，便于区分「死在 0.42/0.46」与「grounding 不够」。
...
| 优先级 | 改动 | 说明 |
|--------|------|------|
| **1** | **primary 资格 ≠ expand 资格** | `judge_primary_and_expandability`：**禁止**「仅 `similar_to` 高相似、无 **`conditioned_sim`**」靠 `ctx_drop=0` 进主线；primary 仅当 **① 真实 `conditioned_sim` 达标 + static-ctx 不离谱**，或 **② 多锚且无 ctx 时 `family_match≥STAGE2A_PRIMARY_MULTI_MIN_FAMILY`**，或 **③ `dual_support` + conditioned 达标**。**expand** 必须存在 **`conditioned_sim`**，且 `ctx` 更高阈值 + **object/generic/poly** 结构上限；另设 **solo 强 ctx**（`STAGE2A_EXPAND_SOLO_*`）防 dual 标志异常时误杀真主线。`select_primary_per_anchor`：**无 expandable 时 keep 只留 1 条**。 |
...
| **3** | **Stage2B：tier 分流** | `check_seed_eligibility`：**`stage2b_seed_tier=none`** → `stage2a_not_seed`；**strong / weak** 分别走 **SEED_SCORE_MIN / SEED_SCORE_MIN_WEAK**（弱 seed 另拦 **conditioned-only** 等）；`fallback_primary` 不扩；**semantic_mismatch_seed** 硬挡。扩散父节点列表 = **`eligible=True`** 的项（分数门槛已在门内统一，不再对 diffusion 二次 `_min_seed`）。 |
...
**建议日志**：`[Stage2A select]`、**`[Stage2A final bucket reconcile]`**、`[Stage2A anchor bucket summary]`、`[Stage2A global bucket summary]`、`[Stage2A fallback]`、**`[Stage2] carryover_terms` / `[Stage2B input audit]`**（有 seed 时）、**无 seed：`[Stage2B no-seed reason]` + 单行 `[Stage2B] anchor=… merged=…`**、`[Stage2B seed gate]`（NOISY）/ **`[Stage2B seed tier audit]`**（仅 seed 候选）、**有 seed 时 `[Stage2B] anchor=… eligible…` 仅打一条**（已去掉与 `debug_print(2)` 的重复汇总）、**`merge_primary_and_support_terms carryover=…`**、`[Stage2B] diffusion=…`（`debug_print(1)` 保留）、`[Stage3 rerank summary]`、**`[Stage3 support subtype audit]`**、`[Stage3 bucket reason]` 中的 `bucket_reason_flags`。

**相关常量**：`STAGE2A_CONDITIONED_SIM_EPS`、`STAGE2A_PRIMARY_MIN_SIM`、`STAGE2A_PRIMARY_MAX_CTX_DROP`、`STAGE2A_PRIMARY_MIN_CTX_SIM`、`STAGE2A_PRIMARY_DUAL_MIN_CTX`、`STAGE2A_PRIMARY_MULTI_MIN_FAMILY`、`STAGE2A_EXPAND_*`、`STAGE2A_EXPAND_SOLO_STRONG_CTX`、`STAGE2A_EXPAND_SOLO_MIN_FAMILY`、`STAGE2A_FALLBACK_*`、`STAGE3_SUPPORT_DEMOTE_*`。

**Stage2A/2B 三函数最小收尾（仅 `label_expansion.py`，不改 `check_seed_eligibility`）**

| 函数 | 目的与要点 |
|------|------------|
| **`judge_primary_and_expandability`** | ① **强主线直通**：高 static/ctx、低 ctx_drop、family 支撑，且 **`source_type` ∈ similar_to / conditioned_vec**；若 **非** `branch_blocked` 则直接 **`primary_expandable` / strong_mainline_direct**。② **控制主线窄逃生口**（`expand_escape=control_mainline_escape`）：条件同左（**`generic_risk≥0.50`、`polysemy_risk≤0.22`、`object_like_risk≤0.05`** 等）。③ **五层分流**：未达强 expandable 时由 **`_classify_keepish_primary_bucket`** 产生 **`primary_support_seed`（主线近邻弱 seed）** / **`primary_support_keep`** / **`risky_keep`**；其中 **robot control** 类 **branch_blocked** 但 **generic/poly 未达「高风险」阈** 的可升为 **弱 seed**（**`can_expand_from_2a=True`**, **`stage2b_seed_tier=weak`**），由 **`check_seed_eligibility`** 的 **`weak_seed_ok`** 与 **`method_like` 逃逸** 约束，**dense** 侧 **弱配置**。 |
| **`pick_fallback_primary_for_anchor`** | 0-primary 保线收紧：先挡 **object/poly/drift** 坏分支；要求 **`lexical_shape_match(anchor, cand)≥0.35` 或 `family_match≥0.58`**；**obj/gen/poly** 过高直接不入选；**fallback_score** 改为 **0.38·best_sim + 0.18·ctx + 0.16·(1−ctx_drop) + 0.14·lexical + 0.14·family**，减少 **动力学→mechanics / 运动学→kinesiology / 仿真→simulation** 等泛词被捞成 fallback。 |
| **`expand_from_vocab_dense_neighbors`** | **strong_seed**：`seed_score≥0.68` 或（`semantic_score≥0.83` 且 `conditioned_sim≥0.80`）。强 seed：**domain_fit 下限 0.58**（否则仍为 `max(SUPPORT_MIN_DOMAIN_FIT,0.72)`）、**max_keep=3**、Faiss **k=min(top_k+8,24)**。若 **support_expandable_for_anchor** 未过但为 strong_seed 且锚点有 **conditioned_vec**，在满足 **sim≥0.72、domain_fit≥0.58、anchor_consistency≥0.52、family_support≥0.45** 且非设备词、非 **`_is_bad_support_for_anchor`** 时，**弱放行** `reason=weak_support_release_for_strong_seed`，缓解 **after_domain_fit=0 / final_kept=0** 的全灭。 |

**验收日志**：`[Stage2A expand factors]` 中 **`strong_mainline_direct`**；被 **branch_blocked** 误杀后由窄口救回的词条看 **`expand_escape=control_mainline_escape`** 与快照里的 **`control_mainline_escape`**；`[Stage2A fallback]` 是否仍出现 mechanics/kinesiology/simulation；`[Stage2B] Dense expansion funnel` 中 **after_mainline_filter / final_kept** 是否大于 0。
...
- **judge_primary_and_expandability**：**primary** 须 **真实 `conditioned_sim`** 路径或 **多锚高 family** 或 **dual+conditioned**；**expand** 在 **`drop_ok_exp` ∧ 非 branch_blocked** 下走 **dual / multi / solo ctx** 阈值；另设 **`strong_mainline_direct`** 直通 **`primary_expandable`**（且非 **branch_blocked**）；若 **branch_blocked** 但命中 **控制主线窄逃生口**（高 ctx、**dual_support**、**generic≥0.50** 且 **poly≤0.22**、**object_like≤0.05** 等），仍可得 **`primary_expandable`** 与 **`expand_escape=control_mainline_escape`**；返回桶 + reason。
- **select_primary_per_anchor**：返回 `{ primary_expandable, primary_keep_no_expand, rejected }`；**前置硬拒绝仅** `retain_mode=="reject"`；**obviously_bad_branch / drift_bad / polysemous_no_context** 仍前置 reject；再跑 **`judge_primary_and_expandability`**（低 `primary_score` 在此阶段才判 keep/reject）；**无 expandable 时 keep 仅 top1**。
...
- **3）Stage3 最终分数修正 + paper 选词**  
  ...
  - **select_terms_for_paper_recall**（当前实现）：core 直接放行；单锚 conditioned_only 不进；**fallback_primary**（含 Stage2A fallback 语义）不进；support：**`final_score≥paper_final_score_need`**（**`primary_support_seed`：0.42**；**keep 类及其它：0.46**）+ 风险阈值 + **`paper_grounding` 分层**；**cross-anchor 主线托底**为 `(anchor_count>=2 ∧ mainline_hits>=1) ∨ mainline_hits≥2`，另加 **`primary_support_seed` + 单锚 + mainline_hits≥1 + can_expand** 的窄豁免后仍须过 **`paper_grounding`**；**risky 桶一律不进** paper。
...
- **Stage3 彻底移除 cluster 依赖，并改为全局复审层**：classify_stage3_entry_groups → check_stage3_admission；最终分 = score_term_record × identity_factor × risk_penalty × **role_factor**（mainline primary×**STAGE3_MAINLINE_BOOST(1.06)**、dense×**STAGE3_DENSE_PENALTY(0.90)**、conditioned_vec×**STAGE3_CONDITIONED_PENALTY(0.94)**、side×**STAGE3_SIDE_PENALTY(0.95)**）；score_term_record 内部为 base_score×gate×cross_anchor_factor×backbone_boost×object_like_penalty×bonus_term_penalty。assign_stage3_bucket（core/support/risky）后，再由 **`stage3_build_score_map`** 对 **`final_score`** 做**第二次乘法修正**（**support 跨锚正奖励排除 conditioned_only**；单锚 conditioned_only 在 support 上额外压制；其余 weak keep 规则见 Stage3 表）；select_terms_for_paper_recall 按 core→support→risky。日志三张表 + **[Stage3 Bucket Details]**。
...
| 项 | 说明 |
|----|------|
| **作用** | 对 raw_candidates **按 tid 去重聚合**（保留 mainline_hits、best_anchor_identity、best_jd_align、best_context_continuity、risk 及 Stage2A **primary_bucket** 等）后：① **classify_stage3_entry_groups**；② **check_stage3_admission**；③ **should_drop_term**；④ **topic 闸门**；⑤ **唯一主分** × identity × risk_penalty × **role_factor**；⑥ **assign_stage3_bucket**；⑦ **select_terms_for_paper_recall**（仅 **core/support** 进论文召回，**risky** 不进）；⑧ 写 map、tag_purity_debug。**目标**：主线保留词不因缺少 mainline_hits/可扩计数而被误杀；fallback 漂移词仍压在 **risky** 且不进 paper。 |
| **思路** | Stage3 从「全杀门」改为**全局复审层**：mainline primary 先进入评分，identity 只打噪声不杀 2A 主词，dense/support 才是重点审查；**降分/分桶**为主，只有明显噪声才 drop。 |
| **逻辑流程** | ① _merge_stage3_duplicates（最佳 seed 记录整表字段保留，含 Stage2A 透传）；② stage2_rank；③ _compute_stage3_global_consensus；④ _build_family_buckets、_compute_family_centrality；⑤ **_classify_stage3_entry_groups**；⑥ should_drop_term → **_check_stage3_admission**；⑦ score_term_record × identity × risk_penalty × **role_factor**；⑧ **`_assign_stage3_bucket`**（conditioned_only + **fallback / locked mainline** 规则，见下表）；⑨ _apply_family_role_constraints；⑩ _collect_risky_reasons、**_collect_stage3_bucket_reason_flags**；⑪ **`stage3_build_score_map`**（**score_mult**×**cross_bonus** 第二段修正；**support 跨锚排除 conditioned_only**；写 **`stage3_explain.conditioned_only`**；重排后**重刷** `bucket_reason_flags`）；⑫ _bucket_stage3_terms；⑬ **`select_terms_for_paper_recall`**（conditioned 单锚、**fallback_primary**、support  grounding、family 去重）；⑭ **`_print_paper_term_selection_audit`**；⑮ 写 map、**_debug_print_stage3_tables**。 |
...
| **主要公式** | **准入** + **identity** + **risk_penalty** + **role_factor** 同前。**Stage2A 透传字段**（随 `_merge_stage3_duplicates` 保留在最佳 seed 记录上）：`primary_bucket`、`primary_reason`、`fallback_primary`、`mainline_candidate`、`can_expand_local`。**语义判定**：`is_fallback_primary` = fallback_primary 或 primary_bucket=`primary_fallback_keep_no_expand` 或 primary_reason=`anchor_core_fallback`；`is_locked_mainline` = primary_bucket=`primary_keep_no_expand` 或 primary_reason=`usable_mainline_no_expand` 或（mainline_candidate ∧ ¬can_expand_local）；`has_mainline_support` = mainline_hits≥1 ∨ can_expand ∨（分桶侧另含 is_locked_mainline）。**bucket（`_assign_stage3_bucket`）**：① **is_fallback_primary** → **risky**（与正常 primary 区分）；② **conditioned_only**（定义同前：有 `conditioned_vec` 且无 `similar_to`、无 `family_landing`）单锚 → **support** 若 has_mainline_support 否则 **risky**；多锚 conditioned_only → **support**；③ **is_locked_mainline**（且非 fallback）→ **support**；④ **core** 仅当 mainline_hits≥1 ∧ can_expand ∧ identity_factor≥0.95，且 **非**（conditioned_only∧单锚）— 否则 **压为 support**（**`conditioned_only_core_cap`**）；⑤ 否则 **support** 若 has_mainline_support 否则 **risky**；**support** 仍可被 **`_stage3_single_anchor_expand_branch_demote`**→risky。**选词（`select_terms_for_paper_recall`）**：单锚 conditioned_only → **`conditioned_only_single_anchor_block`**；**is_fallback_primary** → **`fallback_primary_block`**；**core** 直接进；**support** 须 **has_mainline_support**（mainline_hits≥1 ∨ can_expand ∨ is_locked_mainline）或 anchor_count≥2，否则 `support_but_not_grounded`；**risky** → `risky_bucket_block`；**family_key** 去重；顺序 core→support→risky、同桶按分，上限 **PAPER_RECALL_MAX_TERMS**。**`_collect_stage3_bucket_reason_flags`**：`no_mainline_support` / `only_weak_keep_sources` 改为基于 has_mainline_support 与 is_fallback_primary；locked / fallback 分别打 **`locked_mainline_no_expand`**、**`fallback_primary`**。 |

**Stage3 主分**：**compose_term_final_score** 已退出 Stage3 主链（deprecated）。当前：**score_term_record** 产出 raw_final，再乘 **identity_factor**（term_scoring.compute_identity_factor，软因子不硬杀）与 **_compute_stage3_risk_penalty**；再乘 **role_factor** 得初值 **`final_score`**；随后 **`_assign_stage3_bucket`** 与 family 约束、初排完成后，由 **`stage3_build_score_map`** 将 **`final_score`** 再乘 **`mainline_risk_penalty`（score_mult）** 与 **`cross_anchor_score_bonus`**（**support 跨锚 1.03 不发给 conditioned_only**），并写入 **`stage3_explain.conditioned_only`**，重排、重刷 **`bucket_reason_flags`**。score_term_record 内部仍含 base_score×gate×cross_anchor_factor×backbone_boost×object_like_penalty×bonus_term_penalty。
...
| **函数 1：分桶** | **is_fallback_primary** → **risky**。**单锚** conditioned_only：**support** 若 has_mainline_support（强主线 ∨ locked mainline），否则 **risky**；**永不为 core**。**多锚** conditioned_only：**support**。**is_locked_mainline**（非 fallback）→ **support**。其余路径：**core** 仅当 `mainline_hits≥1` ∧ `can_expand` ∧ `identity_factor≥0.95`（含 **`primary_expandable`** / 回退 **core_ok**），且 **立即经 `_cap_core_if_conditioned_single_anchor`**：**conditioned_only∧anchor_count≤1** 时强制 **support**（`stage3_core_cap_reason=conditioned_only_single_anchor`，**`conditioned_only_core_cap`**），与 paper **单锚 conditioned** 口径一致。**未达 core** 时：**support** 若 has_mainline_support 否则 **risky**；**support** 仍可能被 **`_stage3_single_anchor_expand_branch_demote`** → **risky**。 |
| **函数 2：paper** | **conditioned_only** ∧ **单锚** → **`conditioned_only_single_anchor_block`**。**is_fallback_primary** → **`fallback_primary_block`**。**core** → `paper_primary`。**support** → `has_mainline_support`（mainline_hits≥1 ∨ can_expand ∨ is_locked_mainline）或 **anchor_count≥2**，否则 `support_but_not_grounded`。**risky** → **`risky_bucket_block`**。同 **`family_key`** 去重；顺序 **core→support→risky**，桶内 **`final_score` 降序**，直至 **`PAPER_RECALL_MAX_TERMS`**。 |
...
| **全局分：`stage3_build_score_map`** | 仅 **`stage3_term_filtering.stage3_build_score_map`**：在 bucket 已赋值之后，用 **`bucket_reason_flags`** 与桶名统一第二段乘子。**score_mult**：`no_mainline_support`×0.72，同 flag 且 **¬can_expand** 再×0.82；**only_weak_keep_sources**×0.72；**locked_mainline_no_expand**×0.86；**risky**×0.82；**secondary_primary** 且非 core×0.88；**conditioned_only**：单锚基础×0.68，单锚且 **mainline_hits≤1** 再×0.86，单锚且桶为 **support** 再×0.88；多锚且 **mainline_hits=0∧¬can_expand**×0.80，否则×0.90；**C** 段：单锚无主线且不可扩再×0.74（与 weak 侧翼一致）。**cross_bonus**：**core** 且 **anchor_count≥2** 才×1.08；**support** 仅当 **(¬conditioned_only)∧anchor_count≥2∧(mainline_hits≥1∨can_expand)** 时×1.03（**conditioned_only 不再拿 support 跨锚正奖励**）；**risky** 与 **locked_mainline_no_expand** 不吃跨锚正奖励；**anchor_count≥2∧mainline_hits=0∧¬can_expand** 再×0.92 回收虚高。最后按 **`final_score`** 重排、重写 **`stage3_rank`** 与 **`bucket_reason_flags`**；**`stage3_explain.conditioned_only`** 记录布尔便于审计。 |
| **调试审计（可选）** | **`STAGE3_AUDIT_DEBUG`**：在 **`stage3_build_score_map`** 之后打印 **`[Stage3 final adjust audit]`**、**`[Stage3 cross-anchor audit]`**、**`[Stage3 support/risky concise]`**；在 **`select_terms_for_paper_recall`** 末尾（已写 **`selected_for_paper` / `paper_reject_reason`**）打印 **`[Stage3->Paper bridge]`**。**`STAGE3_DEBUG_FOCUS_TERMS`** 非空时上述表仅保留 **term 字面（strip）命中** 的行，避免刷屏；置 **`set()`** 则按各表 **top_k** 输出。审计字段对齐本仓库 **`dict` 候选**：桶为 **`stage3_bucket`**，乘子读 **`stage3_explain.mainline_risk_penalty` / `cross_anchor_score_bonus` / `conditioned_only`**，**pre_adjust** 用 **`_stage3_pre_adjust_score`**（由 `stage3_build_score_map` 写入）。**`DEBUG_LABEL_PATH`**（`stage3_term_filtering.py`）：在 **`stage3_build_score_map`** 内额外打印 **`[Stage3 score adjust]`**、**`[Stage3 watch]`**、**`[Stage3 rerank summary]`**（含 **`primary_bucket`** 列）；**`_run_stage3_dual_gate`** 内 **`[Stage3 topic_gate]`**：**`STAGE3_DEBUG_FOCUS_TERMS` 为空**时仅 **`bypass primary-like count=`** 一行。默认 **`True`**，关日志时置 **`False`**。 |
...
| 项 | 说明 |
|----|------|
| **作用** | 将 author_papers_list 展开为 **paper_map**（按 wid 去重时**合并 hits 并集**，不丢跨 term 命中），对每篇论文算**贡献度**（含 **term_confidence**、护栏 5），再按作者聚合（accumulate_author_scores）、时序与活跃度加权、**family coverage bonus** 与 **family balance penalty**（不依赖具体领域词），最佳论文比过滤，得到最终作者排序与 **last_debug_info**。 |
| **思路** | 论文贡献度中，**单 term 贡献** = **term_weight × term_confidence × paper_match_strength × term_uniqueness**，再经领域纯度、综述/覆盖/紧密度/时序/簇奖励、JD 语义门得到 paper score；**护栏 5**：primary_count=0 且 supporting_count<2 的论文 score *= 0.05；作者分分两层：第一层由 `papers_for_agg` 全量 retained 论文直接构造 author-term 命中（不再仅看 top3）；第二层在 Stage5 内对 **author×term** 做“先收集逐篇贡献、再同词递减聚合”（`TERM_REPEAT_DECAY=0.60`），并以递减后的 term 贡献和重建 author 总分，抑制单个高频 term 线性灌榜；随后再乘 time_weight、recency；最后使用通用 **term concentration penalty + diversity bonus**（不依赖 family/黑白名单）做分布校正，再按 AUTHOR_BEST_PAPER_MIN_RATIO 过滤后归一化排序。 |
| **逻辑流程** | ① 从 debug_1 读取 **term_role_map**、**term_confidence_map**、**term_uniqueness_map**（由 label_path._build_term_uniqueness_map 从 vocabulary_domain_stats 按目标领域占比构建），放入 context；② 展开 author_papers_list 为 paper_map（wid→{ hits, title, year, domains, authors }），其中同 wid 重复出现时执行 `hits = old_hits ∪ new_hits`；③ 输出审计日志 **[Stage5 merged paper multi-hit audit]**，检查 merged paper 中 `hits>=2` 的论文数；④ 对每篇 paper 调用 **paper_scoring.compute_contribution**，得到 (score, hit_terms, rank_score, term_weights, primary_count, supporting_count)；⑤ 输出审计日志 **[Stage5 paper term_weights audit]**，检查 `term_weights` 非零项是否保留多 term 分摊；⑥ 护栏 5：primary_count==0 且 supporting_count<2 则 score *= 0.05；⑦ 论文得分经 tanh 压缩（95 分位 tau）；⑧ `accumulate_author_scores` 仍用于基础聚合与兼容结构；⑨ 直接从 `papers_for_agg` 全量构建 `author_term_hit_lists` 与 `author_all_retained_works`，并将 author works 口径切到 retained 全量；⑩ 按 `TERM_REPEAT_DECAY=0.60` 做同词递减聚合，得到 `author_term_contrib`；⑪ 用 `sum(author_term_contrib[aid].values())` 重建 author 总分并乘时间特征；⑫ 施加通用 `term concentration penalty` 与 `diversity_bonus`；⑬ AUTHOR_BEST_PAPER_MIN_RATIO 过滤、归一化排序，构建 scored_authors；⑭ 输出 **[Stage5 top-author paper provenance]**（Top20 作者各前 3 篇：`wid`、`paper_score`、`author_piece`、`hit_terms`）；⑮ **aggregate_author_evidence_by_term_role** 写入 last_debug_info。 |
...
- **断点 2（交集生存性）**：`[Stage4 overlap survival audit]`  
  对交集 pid 输出 `rank_before_cap/score/survive_after_cap`；用于判断是否被 per-term local cap 提前砍掉。
- **断点 3（wid 写入行为）**：`[Stage4 paper_map write]`  
  默认仅当 **合并后 new_hit_count≥2** 或 **old/new hit 条数变化** 时打印，避免淹没主链路日志；全量逐条排查时设 **`STAGE4_PAPER_MAP_WRITE_VERBOSE=True`**（`stage4_paper_recall.py`）。字段：`incoming_term/old_hit_count/new_hit_count/old_terms/new_terms`。
- **断点 4（paper->author 传递）**：`[Stage4 merged wid multi-hit detail]` 与 `[Stage4 author payload multi-hit detail]`  
...
- **Stage3**：幸存词打 **stage3_bucket**（core/support/risky）与 **retrieval_role**（paper_primary/paper_support）；**select_terms_for_paper_recall** 按 **core → support** 选取（**risky 不进** final_term_ids_for_paper），上限 **PAPER_RECALL_MAX_TERMS**（默认 12）。  
...
| **PAPER_RECALL_MAX_TERMS**（stage3_term_filtering） | 12 | 参与论文检索的最大词数（按 core→support→risky 优先选取） |
