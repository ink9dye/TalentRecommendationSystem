...
| **逻辑流程** | ① _merge_stage3_duplicates（最佳 seed 记录整表字段保留，含 Stage2A 透传）；② stage2_rank；③ _compute_stage3_global_consensus；④ _build_family_buckets、_compute_family_centrality；⑤ **_classify_stage3_entry_groups**；⑥ should_drop_term → **_check_stage3_admission**；⑦ score_term_record × identity × risk_penalty × **role_factor**；⑧ **`_assign_stage3_bucket`**（conditioned_only + **fallback / locked mainline** 规则，见下表）；⑨ _apply_family_role_constraints；⑩ _collect_risky_reasons、**_collect_stage3_bucket_reason_flags**；⑪ **`stage3_build_score_map`**（**mainline_risk_penalty**×**cross_anchor_score_bonus** 第二段修正，重排后**重刷** `bucket_reason_flags`）；⑫ _bucket_stage3_terms；⑬ **`select_terms_for_paper_recall`**（conditioned 单锚、**fallback_primary**、support  grounding、family 去重）；⑭ **`_print_paper_term_selection_audit`**；⑮ 写 map、**_debug_print_stage3_tables**。 |
...
| **全局分：`stage3_build_score_map`** | 仅 **`stage3_term_filtering.stage3_build_score_map`**：在 bucket 已赋值之后，用 **`bucket_reason_flags`** 与桶名统一第二段乘子。**score_mult**：`no_mainline_support`×0.72，同 flag 且 **¬can_expand** 再×0.82；**only_weak_keep_sources**×0.72；**locked_mainline_no_expand**×0.86；**risky**×0.82；**secondary_primary** 且非 core×0.88；**conditioned_only**：单锚×0.68，多锚且 **mainline_hits=0∧¬can_expand**×0.80，否则×0.90；单锚无主线且不可扩再×0.74。**cross_bonus**：**core** 且 **anchor_count≥2** 才×1.08；**support** 仅当 **anchor_count≥2∧(mainline_hits≥1∨can_expand)** 时×1.03；**risky** 与 **locked_mainline_no_expand** 不吃跨锚正奖励；**anchor_count≥2∧mainline_hits=0∧¬can_expand** 再×0.92 回收虚高。最后按 **`final_score`** 重排、重写 **`stage3_rank`** 与 **`bucket_reason_flags`**。 |
