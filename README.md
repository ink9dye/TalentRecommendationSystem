...
| **Stage2B** | primary_landings（经 **`can_expand_from_2a=True`** 且 primary_score≥SEED_SCORE_MIN 得 diffusion_primaries）, active_domain_set, jd_*_ids | raw_candidates（retain_mode、topic_source、seed_blocked 等下传 Stage3） | **`check_seed_eligibility` 仅认 `can_expand_from_2a`**；`fallback_primary` / `primary_keep_no_expand` 不扩；diffusion_primaries = 上述 eligible ∧ primary_score≥SEED_SCORE_MIN；**expand_from_vocab_dense_neighbors**（support_expandable_for_anchor…）；**DENSE_PARENT_CAP=0.85**；expand_from_cluster_members 全关；expand_from_cooccurrence_support；merge_primary_and_support_terms |
| **Stage3** | raw_candidates（含 term_role、role_in_anchor、source_types、retain_mode 等）, query_vector, anchor_vids | score_map, term_map, idf_map, **paper_terms** | 去重聚合 → classify_stage3_entry_groups → check_stage3_admission；**唯一主分** × role_factor；**`_assign_stage3_bucket`**（两函数硬门之一：conditioned_only **永不得 core**）；风险循环（conditioned 分层罚分、**cross_anchor_score_bonus**）；**`select_terms_for_paper_recall`**（两函数硬门之二：**单锚 conditioned_only 永不进 paper**）；审计日志 **`_print_paper_term_selection_audit`** |
...
- **Stage3 全局复审层（非全杀门）**：去重聚合 → classify_stage3_entry_groups → check_stage3_admission → **唯一主分** = score_term_record × identity_factor × risk_penalty × **role_factor**。**Role 因子（最小可行修复）**：mainline primary × **STAGE3_MAINLINE_BOOST(1.06)**、dense_expansion × **STAGE3_DENSE_PENALTY(0.90)**、conditioned_vec × **STAGE3_CONDITIONED_PENALTY(0.94)**、side × **STAGE3_SIDE_PENALTY(0.95)**，使排序更符合直觉（motion control 稳于 Motion controller）；assign_stage3_bucket（core/support/risky）；select_terms_for_paper_recall 优先 core→support→risky，上限 12；日志三张表：Entry Group、Admission、Scoring。
...
- **judge_primary_and_expandability**：**primary** 须 **真实 `conditioned_sim`** 路径或 **多锚高 family** 或 **dual+conditioned**；**expand** 必须带 **`conditioned_sim`** + 更高 ctx 阈 + **object/generic/poly** 上限 +（dual / multi / solo 强 ctx）；返回桶 + reason。
...
- **pick_fallback_primary_for_anchor** / **anchor_allows_fallback_primary**：0-primary 时的 **保线 fallback**（不扩）。
...
- **3）Stage3 最终分数修正 + paper 选词**  
  ...
  - **select_terms_for_paper_recall**：core 直接放行；support 需 mainline_hits≥1 或 can_expand，且 conditioned_only 且单锚且无 mainline 的 support 不进；risky 默认不进，仅当 final_score≥0.48 且（mainline_hits≥1 或 can_expand 或 anchor_count≥2）且非 only_weak_keep_sources 时放行。  
...
| `expand_from_vocab_dense_neighbors` | 入口对每个 primary 再跑 **check_seed_eligibility**；候选须过 **support_expandable_for_anchor**（四道门 + anchor_identity_for_support≥0.16 + family_drift_penalty；keep_score≥0.76）。**Dense 不压过 parent**：通过后 **keep_score = min(keep_score, parent.primary_score × DENSE_PARENT_CAP(0.85))**，避免 Motion controller 等在 Stage3 反超 motion control。sim≥0.55、domain_fit≥0.72；每 seed 最多 2/3 条。 |
...
| **逻辑流程** | ① _merge_stage3_duplicates；② stage2_rank；③ _compute_stage3_global_consensus；④ _build_family_buckets、_compute_family_centrality；⑤ **_classify_stage3_entry_groups**；⑥ should_drop_term → **_check_stage3_admission**；⑦ score_term_record × identity × risk_penalty × **role_factor**；⑧ **`_assign_stage3_bucket`**（conditioned_only 分桶硬规则，见下表）；⑨ _apply_family_role_constraints；⑩ _collect_risky_reasons、**_collect_stage3_bucket_reason_flags**；⑪ **mainline 风险修正**（conditioned 分层、**cross_anchor_score_bonus**）；⑫ _bucket_stage3_terms；⑬ **`select_terms_for_paper_recall`**（单锚 conditioned_only 硬挡 + family 去重）；⑭ **`_print_paper_term_selection_audit`**；⑮ 写 map、**_debug_print_stage3_tables**。 |
...
| **主要公式** | **准入** + **identity** + **risk_penalty** + **role_factor** 同前。**bucket（`_assign_stage3_bucket`）**：**conditioned_only** = 有 `conditioned_vec` 且无 `similar_to`、无 `family_landing`（`_stage3_normalized_source_types`）；单锚 conditioned_only → 仅 **support**（mainline_hits≥1 或 can_expand）或 **risky**；多锚 conditioned_only → **support**（不得 **core**）；非 conditioned → **core** 当且仅当 mainline_hits≥1 ∧ can_expand ∧ identity_factor≥0.95；否则 **support** 若 mainline/can_expand/跨锚（且非纯 cross_side_only）成立，否则 **risky**；单锚可扩旁枝仍可走 **`_stage3_single_anchor_expand_branch_demote`**→risky。**选词**：单锚 conditioned_only → **`conditioned_only_single_anchor_block`**；**risky 桶不进 paper**；**family_key** 去重；顺序 core→support、同桶按分，上限 **PAPER_RECALL_MAX_TERMS**。 |
...
| 项 | 说明 |
|----|------|
| **目标** | **仅改两个函数**：**`_assign_stage3_bucket`**、**`select_terms_for_paper_recall`**。彻底关死 **conditioned_only 且单锚** 进入 **core** 与 **final_term_ids_for_paper**（典型：Telerobotics、Robot manipulator）。**conditioned_only** 严格定义为：`_stage3_normalized_source_types` 中含 **`conditioned_vec`**，且**无** **`similar_to`**、**无** **`family_landing`**。 |
| **函数 1：分桶** | **单锚** conditioned_only：`mainline_hits≥1` 或 `can_expand` → **support**，否则 **risky**；**永不为 core**。**多锚** conditioned_only：**support**，不为 **core**。非 conditioned：**core** 当且仅当 `mainline_hits≥1` ∧ `can_expand` ∧ `identity_factor≥0.95`；否则若 `mainline`/`can_expand`/跨锚（且非「仅 cross_side：anchor≥2 且 mainline_hits=0」）→ **support**，否则 **risky**；**support** 仍可能被 **`_stage3_single_anchor_expand_branch_demote`** 打成 **risky**。 |
| **函数 2：paper** | 若 **conditioned_only** ∧ **`anchor_count≤1`** → 直接 **`paper_reject_reason=conditioned_only_single_anchor_block`**，**不入选**（无词面豁免）。否则：**core** → 可入选 `paper_primary`；**support** 须 `mainline_hits≥1` 或 `can_expand` 或 `anchor_count≥2`；**risky** → `risky_bucket_block` **一律不入选**。同 **`family_key`** 去重；遍历顺序 **core→support→risky**，桶内 **`final_score` 降序**，直至 **`PAPER_RECALL_MAX_TERMS`**。每条写入 **`selected_for_paper` / `select_reason` / `paper_reject_reason`**；调试由 **`_print_paper_term_selection_audit`** 打出（含未入选）。 |
