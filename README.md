...
| **核心中间产物** | **Stage1**：Stage1Result（**含 jd_profile**）、anchor_skills（含 local_context、phrase_context、**anchor_source/anchor_source_weight**）、_jd_cleaned_terms、job_ids、regex_str。**Stage2A**：primary_landings（含 domain_fit、**landing_score**、**subfield_fit**、**topic_fit** 等；有 jd_profile 时做层级 fit 与硬门槛）。**Stage2B**：raw_candidates（term_role、domain_fit、parent_anchor、parent_primary、**subfield_fit**、**topic_fit**、**landing_score**、**cluster_id**、**main_subfield_match** 等）。**Stage3**：**按 tid 去重聚合**（保留多来源与 best_*）、**classify_stage3_entry_groups**（trusted_primary/secondary_primary/support_expansion）、**check_stage3_admission**（trusted 不硬杀）、**唯一主分** score_term_record×**identity_factor**×**risk_penalty**、**assign_stage3_bucket**（core/support/risky）、**select 按 core→support→risky**、_collect_risky_reasons/_bucket_stage3_terms；score_map、term_map、idf_map、term_role_map、term_source_map、parent_anchor_map、parent_primary_map、tag_purity_debug（含 **stage3_entry_group**、**identity_factor**、**risk_penalty**、**stage3_bucket**、anchor_count、evidence_count、family_centrality、path_topic_consistency、generic_penalty、cross_anchor_factor、retrieval_role 等）；label_path 据此构建 term_confidence_map、term_uniqueness_map。**Stage4**：author_papers_list。**Stage5**：paper_map、contribution、作者聚合分、最终 author_id 列表。 |
...
- **Stage2A 学术落点**：锚点 → 跨类型 SIMILAR_TO + conditioned_vec + alias/exact → **组内选主（最小收尾）**：`judge_primary_and_expandability` 将 **primary 资格（宽）** 与 **expand 资格（严）** 解耦 → `primary_expandable` / `primary_keep_no_expand`；无 expandable 时 keep **仅 top1**；仍 **0-primary** 时对合格 **canonical** 锚点 **`pick_fallback_primary_for_anchor`**（`primary_fallback_keep_no_expand`，不扩）。primary_landings 上 **`can_expand_from_2a`** 标识强主线；**`can_expand` 与之同步**（兼容旧字段）。
- **Stage2B 学术侧补充**：仅 **`can_expand_from_2a=True` 且 primary_score≥SEED_SCORE_MIN** 经 `check_seed_eligibility` 进 diffusion_primaries；**fallback / keep_no_expand 不 seed**。**cluster 全关**；**support_expandable_for_anchor**；**DENSE_PARENT_CAP=0.85**；输出 List[ExpandedTermCandidate]（primary / dense_expansion / cooc_expansion）。
...
| **3** | **Stage2B 只认强主线 seed** | `check_seed_eligibility`：**仅 `can_expand_from_2a=True`** 可过 seed 门；`fallback_primary` → `fallback_primary_no_expand`；否则 `stage2a_not_expandable`。仍用 **SEED_SCORE_MIN** 做 eligible 后的排序裁剪；保留 **semantic_mismatch_seed** 硬挡。 |
...
**建议日志**：`[Stage2A select]`、`[Stage2A fallback]`、`[Stage2B seed gate]`（含 eligible / block_reason）、`[Stage3 bucket reason]` 中的 `bucket_reason_flags`。
...
| 函数 | 目的与要点 |
|------|------------|
| **`judge_primary_and_expandability`** | ① **强主线直通**（同左列旧述）：高 static/ctx、低 ctx_drop、family 支撑，且 **`source_type` ∈ similar_to / conditioned_vec**；若 **非** `branch_blocked` 则直接 **`primary_expandable` / strong_mainline_direct**。② **控制主线窄逃生口**（`expand_escape=control_mainline_escape`）：当 **`branch_blocked`** 仍成立（多为 **generic_risk** 触顶）但候选实为控制主线上可用词时，避免与真泛词一锅端——仅在同时满足 **`drop_ok_exp`、semantic_ok、`ctx_path_ok`、`base_ctx_ok`、真实 ctx、`conditioned_sim≥0.84`、`dual_support`、主线 tier 为 **strong_mainline_direct 或 good_mainline**（实现为 **`strong_mainline_direct or can_expand`**）、**`generic_risk≥0.50`、`polysemy_risk≤0.22`、`object_like_risk≤0.05`** 时，升格为 **`primary_expandable`** 并 **`can_expand_from_2a=True`**。**第一版故意不救** poly 常 **>0.22** 的 **robot control** 等，以压低对 **digital / machine / servo control** 的连带误放行。 |
...
| **函数 1：分桶** | **is_fallback_primary** → **risky**。**单锚** conditioned_only：**support** 若 has_mainline_support（强主线 ∨ locked mainline），否则 **risky**；**永不为 core**。**多锚** conditioned_only：**support**。**is_locked_mainline**（非 fallback）→ **support**。非上述：**core** 当且仅当 `mainline_hits≥1` ∧ `can_expand` ∧ `identity_factor≥0.95`；否则 **support** 若 has_mainline_support 否则 **risky**；**support** 仍可能被 **`_stage3_single_anchor_expand_branch_demote`** → **risky**。 |
...
| **函数 3（日志，建议同步）** | **`_collect_stage3_bucket_reason_flags`**：`no_mainline_support` / `only_weak_keep_sources` 与上述 **has_mainline_support**、**is_fallback_primary** 对齐；增补 **`locked_mainline_no_expand`**、**`fallback_primary`**。 |
...
| **思路** | **词侧**：熔断放宽为 **MELT_RATIO**（默认 5%），(degree_w/total_w) ≥ 0.05 的泛词在 Cypher 内过滤。**论文侧**：不做 `WHERE w.domain_ids =~ $regex` 剔除，仅用 **domain_bonus**（匹配 1.2、不匹配 1.0）参与打分。在原有 term-grounding 基础上，新增 **JD↔论文摘要向量一致性（jd_align）**，但只做弱辅助：`jd_boost = 0.85 + 0.15*jd_align`（当 `term_grounding < 0.20` 时改为 `0.92 + 0.08*jd_align`），`hybrid_grounding = term_grounding * jd_boost`。这保证“先像 term，再由 JD 精排”，避免 JD 相似度把 term 不够像的论文抬上来。再叠加 **term-type 因子**（来自 `term_meta`：`can_expand`、`stage3_bucket`、`retrieval_role`、`object_like_penalty`、`bonus_term_penalty`、`generic_penalty`）：方法骨架词略放宽、对象词进一步收紧；最后 `term_contrib = base_term_contrib × (0.10 + 0.90*hybrid_grounding) × off_topic_penalty × term_paper_role_factor`。入池门控采用“双阈值”：`primary_gate: term_grounding>=0.12`；`secondary_gate: term_grounding>=0.06 且 jd_align>=0.78`（secondary 命中保留但按 0.65 降权后入池），`hybrid_grounding` 仅参与排序。每个 term 再做 **动态 local cap**（方法骨架词更宽、对象词更紧；对象型词当前收紧到 **3**），最终 **paper_score** = Σ term_contrib，并全局 ORDER BY paper_score DESC、LIMIT 2000。 |
...
| **主要公式** | 熔断：`(degree_w * 1.0 / total_w) < MELT_RATIO`（默认 0.05）；**idf_weight** = `log10(total_w / (degree_w + 1))`；**role_weight**：paper_primary=1.0、paper_support=0.7、其他=0.4（**不看领域词，只看 retrieval_role**）；**base_term_contrib** = term_final_score × role_weight × idf_weight × domain_bonus × recency(year)；**term_grounding_raw**（0~1）来自 paper 的 `title/domains` 与 `term_meta`：`0.55*lexical_hit + 0.20*anchor_axis_hit + 0.25*jd_axis_match`，再乘 `term_grounding_factor`（方法骨架词 1.10~1.20、对象词当前收紧到约 **0.72**）；**jd_align**：`query_vec` 与摘要向量余弦，映射到 [0,1]；**jd_boost**：`0.85 + 0.15*jd_align`（若 `term_grounding < 0.20`，改为 `0.92 + 0.08*jd_align`）；**hybrid_grounding**：`term_grounding * jd_boost`（仅用于排序，不用于硬门）；**off_topic_penalty** 采用更强偏题惩罚（泛交通/物流约 **0.55**，RL 非机器人主轴约 **0.45**，route planning 交通化约 **0.40**，robotic arm 非控制/操作学语境约 **0.60**）；最终 **term_contrib** = base_term_contrib × (0.35 + 0.65*hybrid_grounding) × off_topic_penalty × `term_paper_role_factor`（对象词再打折，当前约 **0.60/0.70**）；**入池硬门仅看 `term_grounding`**（`term_grounding < 0.12` 则不计入）；per-term 使用 **动态 local cap**（方法骨架词更宽、对象词当前收紧到 **3**）；全局 **GLOBAL_PAPER_LIMIT** = 2000。 |
...
- **断点 3（wid 写入行为）**：`[Stage4 paper_map write]`  
  每次写 wid 输出 `incoming_term/old_hit_count/new_hit_count/old_terms/new_terms`；用于区分“没进来”还是“进来后 merge 失败”。
