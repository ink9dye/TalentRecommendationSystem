...
| **逻辑流程** | ① `build_stage2a_mainline_profile`；② 每锚 `collect_landing_candidates` → `landing_candidates_to_stage2a`；③ `build_cross_anchor_index`；④ 每锚 `enrich_stage2a_candidates`；⑤ **pre-primary 分桶**（① 明显坏分支→hard_reject ② 弱相关技术词→keep_no_expand ③ low_identity/low_mainline_no_ctx→hard_reject ④ 高歧义无上下文→hard_reject）；⑥ `select_primary_per_anchor`；⑦ `merge_stage2a_primary`；⑧ Stage2B 仅 `can_expand=True` 进 diffusion_primaries。 |
...
- **check_seed_eligibility**：已改为**不再**用 context_gain≤0 硬拦截；context_gain 仅作 seed_score 软加减分；准入由 strong_static_seed / dual_support_seed / cross_anchor_seed 三条之一决定。详见「三函数补丁」。
...
- 在项目根执行：  
  ...
  可选：`--limit N` 试跑前 N 条；`--out 路径` 覆盖输出库。  
- **换 SBERT 目录或模型后须全量重建**。
