...
本节说明离线索引的**建立方式**、**在系统中的作用**与**设计考量**，对应代码：`src/infrastructure/database/build_index/` 下的 `build_collaborative_index.py`、`build_feature_index.py`、`build_vector_index.py`、`build_vocab_stats_index.py`（及可选 `build_work_title_embeddings.py`、QueryEncoder 快照等）；路径与开关由 `config.py` 统一配置。
...
| | `WORK_TITLE_EMB_DB_PATH` | 论文标题向量库（标签路 Stage5），如 `data/build_index/work_title_embeddings.db` |
