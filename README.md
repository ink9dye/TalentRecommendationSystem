...
| **Stage2A** | prepared_anchors, active_domain_set, query_vector, query_text, 可选 jd_*_ids | primary_landings（含 **can_expand**、**bucket_reason**、**is_good_mainline**、mainline_preference、expandable、primary_score 等；_debug 透传至 Stage3） | **主线优先组内选主（终稿三档分桶）**：… → select_primary_per_anchor（obvious_bad→reject；good_mainline→primary_expandable；否则→primary_keep_no_expand）→ merge_stage2a_primary；Stage2B 仅 can_expand=True 且 primary_score≥SEED_SCORE_MIN 扩展 |
| **Stage2B** | primary_landings（经 Stage2A **can_expand + primary_score≥SEED_SCORE_MIN** 得 diffusion_primaries，**不做第二轮主线审批**；打 seed_blocked/seed_block_reason）, active_domain_set, jd_*_ids | raw_candidates（**5 个正交层级字段**；retain_mode、topic_source、seed_blocked、seed_block_reason 下传 Stage3/debug） | diffusion_primaries = can_expand ∧ primary_score≥SEED_SCORE_MIN；expand_from_vocab_dense_neighbors（support_expandable_for_anchor、sim≥0.55、domain_fit≥0.72）；**expand_from_cluster_members 全关**；expand_from_cooccurrence_support；merge_primary_and_support_terms；_expanded_to_raw_candidates |
| **Stage3** | raw_candidates（含 term_role、role_in_anchor、can_expand、retain_mode、风险/对齐字段）, query_vector, anchor_vids | score_map, term_map, idf_map, term_role_map, term_source_map, parent_anchor_map, parent_primary_map, **paper_terms** | **按 tid 去重聚合**（保留 mainline_hits、best_*、risk 等）；**classify_stage3_entry_groups**（trusted_primary/secondary_primary/support_expansion）；**check_stage3_admission**（trusted 不 hard_drop，identity 仅软因子）；**唯一主分** score_term_record×**identity_factor**×**risk_penalty**；**assign_stage3_bucket**（core/support/risky）；**select_terms_for_paper_recall** 优先 core→support→risky，上限 PAPER_RECALL_MAX_TERMS(12)；三张调试表（entry group / admission / scoring） |
...
- **Stage2A 学术落点**：锚点 → 跨类型 SIMILAR_TO + conditioned_vec + alias/exact → **主线优先组内选主（终稿三档分桶）**（obvious_bad→reject；good_mainline→primary_expandable；否则→primary_keep_no_expand）→ primary_landings（**can_expand** 由 Stage2A 直接赋值）；Stage2B 仅 **can_expand=True 且 primary_score≥SEED_SCORE_MIN** 进 diffusion_primaries，不做第二轮主线审批。
- **Stage2B 学术侧补充**：仅对 **Stage2A 传下的 can_expand + primary_score≥SEED_SCORE_MIN** 的 diffusion_primaries 做 dense/cooc 扩展（**cluster 当前全关**）；dense/cooc 候选须过 **support_expandable_for_anchor**；输出 List[ExpandedTermCandidate]（term_role：primary / dense_expansion / cooc_expansion）。
- **Stage3 全局复审层（非全杀门）**：**按 tid 去重聚合**（保留 anchor_count、evidence_count、mainline_hits、best_anchor_identity、best_jd_align、best_context_continuity、risk 等）→ **classify_stage3_entry_groups**（trusted_primary / secondary_primary / support_expansion）→ **check_stage3_admission**（trusted_primary 不 hard_drop，仅打 risk_flags；support_expansion 才严格准入）→ **唯一主分** score_term_record × **identity_factor**（软因子，不硬杀）× **risk_penalty**；**assign_stage3_bucket**（core/support/risky）；**topic 闸门只对 support-like 严格**，primary-like 跳过；**select_terms_for_paper_recall** 优先 core、再补 support、最后 risky，上限 12；日志三张表：Entry Group、Admission、Scoring。
...
Stage2A **不再做「达线考试」**，改为**主线偏好第一优先级 + 组内相对选主**。核心：不因 `mainline_sim < x`、`role == off` 或「不能扩散」而整组全灭；**primary 生存** 与 **can_expand** 分离，Stage2B 仅围绕 `can_expand=True` 的 primary 扩展。

| 项 | 说明 |
|----|------|
| **理念** | **主线第一优先级、无固定阈值**：不做 mainline_sim/role 一票否决；证据只参与组内百分位排序；**排序键**以 mainline_rank 为首位（字典序），生死由「锚点内相对竞赛」决定。 |
| **职责** | ① 构建全局 **主线画像** `build_stage2a_mainline_profile`；② 每锚点召回候选（similar_to + conditioned_vec + alias/exact），**不判死**；③ 建 **cross_anchor 证据** `build_cross_anchor_index`；④ 每锚 **enrich**：证据 + **mainline_preference**（mainline_sim、anchor_continuity、jd_continuity、cross_support、anti_polysemy）→ **mainline_rank**（复合主线偏好百分位）→ **build_stage2a_sort_key**（mainline_rank 第一关键字）排序；⑤ **选 primary**：第一名 mainline + can_expand=True，第二名若 `is_competitive_runner_up` 则 side + can_expand=False；⑥ **merge_stage2a_primary** 按 mainline_hits、support_count、best_rank、can_expand 排序；⑦ **Stage2B 仅对 can_expand=True 的 primary 做扩展**。 |
...
| **逻辑流程** | ① `build_stage2a_mainline_profile`；② 每锚 `collect_landing_candidates` → `landing_candidates_to_stage2a`；③ `build_cross_anchor_index`；④ 每锚 `enrich_stage2a_candidates`（证据 + mainline_preference + 排序）；⑤ **pre-primary 明显错落点拦截**（`_should_hard_reject_stage2a_candidate`：只拦大错词，不拦次优词）；⑥ `select_primary_per_anchor`（三档分桶）；⑦ `merge_stage2a_primary`；⑧ Stage2B 仅 `can_expand=True` 进 diffusion_primaries。 |
| **输入/输出** | 输入：prepared_anchors、active_domain_set、query_vector、query_text、jd_*_ids、label。输出：每锚 primary_landings（含 expandable=can_expand、mainline_preference、mainline_rank、anchor_internal_rank、role_in_anchor 等）；Stage2B 仅取 expandable=True；raw_candidates 的 `_debug` 透传上述字段。 |
...
- 最终标签：`survive_primary, can_expand, reject_reason, role`，**role_in_anchor**（mainline / side / dropped）；**primary 与 can_expand 分离**：mainline 才可扩散，side 保留但禁扩散。
...
- **select_primary_per_anchor**：返回 `{ primary_expandable, primary_keep_no_expand, rejected }`；**三档分桶**：obvious_bad→reject；good_mainline（normal + mainline_pref≥0.50 + anchor_identity≥0.52）→ primary_expandable；否则→ primary_keep_no_expand（含 bucket_reason、is_good_mainline）。
- **merge_stage2a_primary**：按 (mainline_hits, support_count, -best_rank, can_expand) 排序；合并时 can_expand 沿用候选已算好的值；Stage2B 仅对 can_expand=True 且 primary_score≥SEED_SCORE_MIN 的 primary 做扩展。
...
Stage2A 从「候选平均打分器」改为「主线优先的组内选主器」：主线偏好第一排序关键字，primary 与 can_expand 分离，Stage2B 只扩 can_expand=True。
...
| **select_primary_per_anchor** | mainline_admissible/suppress_seed 一票否决主线词 | **三档分桶**：obvious_bad（retain_mode∉{normal,weak_retain} 或 primary_score&lt;WEAK_KEEP_MIN）→ reject；good_mainline（normal 且 mainline_pref≥MAINLINE_PREF_MIN 且 anchor_identity≥EXPANDABLE_IDENTITY_MIN）→ primary_expandable；否则 → primary_keep_no_expand | `bucket`、`can_expand`、`bucket_reason`、`is_good_mainline`、`role_in_anchor`、`admission_reason` |
...
**主流程**：`collect_landing_candidates`（召回 + **分型**）→ `landing_candidates_to_stage2a` → `build_cross_anchor_index` → `enrich_stage2a_candidates`（证据 + **score_stage2a_candidate**）→ **pre-primary 明显错落点拦截**（`_should_hard_reject_stage2a_candidate`，仅拦明显错义落点）→ `select_primary_per_anchor`（三档分桶）→ 挂 `parent_anchor` → `merge_stage2a_primary`；Stage2B 仅对 **can_expand=True 且 primary_score≥SEED_SCORE_MIN** 的 primary 做扩散。

**Stage2A pre-primary 明显错落点拦截**（只动 2A、少改行数）：在 `enrich_stage2a_candidates` 之后、`select_primary_per_anchor` 之前增加一层 **reject gate**，不推翻现有框架、不引入硬编码词表，只用已有特征（ctx_supported、anchor_identity、mainline_pref、poly_risk）。**常量**：`STAGE2A_REJECT_IDENTITY_FLOOR=0.33`、`STAGE2A_REJECT_MAINLINE_FLOOR=0.40`、`STAGE2A_POLY_HARD_RISK=0.55`。**三条规则**：① 上下文不支持且 anchor_identity &lt; 0.33 → hard_reject（low_identity_no_context）；② 上下文不支持且 mainline_pref &lt; 0.40 → hard_reject（low_mainline_no_context）；③ poly_risk≥0.55 且无上下文且 anchor_identity&lt;0.40 → hard_reject（polysemous_no_context）。**只拦大错词**（如 动力学→dynamism、控制→control(management)、运动学→sports science、仿真→simula），**不拦次优词**（如 robotic hand、q-learning、medical robotics 保留为 primary_keep_no_expand）。被拦候选写入 `stage2a_hard_rejected`，日志输出 `[Stage2A hard reject] anchor=... term=... reason=...`。

**调试日志**（`LABEL_EXPANSION_DEBUG` + `STAGE2_VERBOSE_DEBUG` 时）：  
① **Stage2A 候选定性表**：anchor \| term \| semantic \| jd_align \| ctx_cont \| anchor_id \| hier \| poly \| object \| generic \| retain_mode \| suppress_seed \| mainline_pref \| **hard_reject_reason**；  
...
- **Stage2A 主线优先组内选主（终稿三档分桶）**：主流程为 `build_stage2a_mainline_profile → collect_landing_candidates → … → enrich_stage2a_candidates` → **pre-primary 明显错落点拦截**（`_should_hard_reject_stage2a_candidate`，只拦大错词）→ `select_primary_per_anchor`（三档分桶）→ `merge_stage2a_primary`。**不再**用 mainline_admissible/suppress_seed 一票否决主线词；真正主线（motion control、reinforcement learning、robotic arm 等）按阈值回 primary_expandable；错义落点（dynamism、control(management)、sports science、simula 等）在入 primary 前被 2A hard_reject 拦截。
- **Stage2B 固定成两层门**：**Seed 门**（谁能扩）：**仅 can_expand=True 且 primary_score≥SEED_SCORE_MIN(0.50) 的 primary** 进入 diffusion_primaries（只消费 Stage2A 传下的 can_expand，不做第二轮主线审批）；**Support 门**：dense/cooc 候选须过 **support_expandable_for_anchor**。**Cluster 当前全关**（`expand_from_cluster_members` 直接 return []）。
...
- **Stage3 彻底移除 cluster 依赖，并改为全局复审层**：**classify_stage3_entry_groups**（trusted_primary/secondary_primary/support_expansion）→ **check_stage3_admission**（trusted_primary 不 hard_drop；identity 仅作软因子 **compute_identity_factor** 参与 final，不再硬闸门）。最终分 = score_term_record 的 raw_final × **identity_factor** × **risk_penalty**；score_term_record 内部仍为 base_score×gate×cross_anchor_factor×**backbone_boost**×**object_like_penalty**×**bonus_term_penalty**（base 权重 0.36·semantic+0.18·context+…；gate=0.75+0.15×path_topic+0.10×generic_penalty）。**assign_stage3_bucket**：trusted_primary 且 score≥0.55 且 poly≤0.55→core；score≥0.42 且 obj/generic 不过高→support；其余 risky。**select_terms_for_paper_recall** 按 core→support→risky 优先选词。日志 **[_debug_print_stage3_tables]** 三张表：Entry Group、Admission、Scoring；**[Stage3 Bucket Details]** 打印 term | bucket | final | ptc | cross | reasons。
...
| `expand_from_vocab_dense_neighbors` | 入口对每个 primary 再跑 **check_seed_eligibility**，不通过则跳过。**Dense 补丁（再收一刀）**：候选须过 **support_expandable_for_anchor**：① 四道门（primary_consistency≥0.72、anchor_consistency≥0.70、context_stability≥0.72、family_support≥0.68）；② **anchor_identity_for_support** = compute_anchor_identity_score(anchor_term, candidate_term) ≥ 0.16，抑制 robotic arm→robotic hand/robot hand；③ **family_drift_penalty** = max(0, primary_consistency − family_support)，keep_score 减去 0.18×drift_penalty；keep_score = 0.26×primary + 0.26×anchor + 0.22×context + 0.16×family + 0.10×anchor_identity − 0.18×drift_penalty，≥0.76 才保留。sim≥0.55、domain_fit≥0.72；每 seed 最多 2/3 条。 |
...
| 项 | 说明 |
|----|------|
| **作用** | 对 raw_candidates **按 tid 去重聚合**（保留 mainline_hits、best_anchor_identity、best_jd_align、best_context_continuity、risk 等）后：① **classify_stage3_entry_groups**（trusted_primary / secondary_primary / support_expansion）；② **check_stage3_admission**（trusted_primary **不 hard_drop**，仅打 risk_flags；secondary 弱时才拒；support_expansion 才严格）；③ **轻硬过滤** should_drop_term（仅对非 primary-like）；④ **topic 闸门** 仅对 support-like 严格；⑤ **唯一主分** score_term_record × **identity_factor**（软因子）× **risk_penalty**；⑥ **assign_stage3_bucket**（core/support/risky）；⑦ **select_terms_for_paper_recall** 优先 core、再 support、再 risky；⑧ 写 map、tag_purity_debug。**目标**：2A 已扶正的主词（如 motion control、reinforcement learning、robotic arm）允许进入 final scoring 并落入 core/support，而非被 identity_gate 全杀。 |
| **思路** | Stage3 从「全杀门」改为**全局复审层**：mainline primary 先进入评分，identity 只打噪声不杀 2A 主词，dense/support 才是重点审查；**降分/分桶**为主，只有明显噪声才 drop。 |
| **逻辑流程** | ① _merge_stage3_duplicates（保留多来源与 best_*）；② stage2_rank；③ _compute_stage3_global_consensus；④ _build_family_buckets、_compute_family_centrality；⑤ **_classify_stage3_entry_groups**；⑥ should_drop_term → **_check_stage3_admission**（替掉原 passes_identity_gate 硬杀）；⑦ 通过则 quality_score、score_term_record，再 × **compute_identity_factor** × **_compute_stage3_risk_penalty**；⑧ **_assign_stage3_bucket**；⑨ _collect_risky_reasons、_bucket_stage3_terms；⑩ **select_terms_for_paper_recall**（core→support→risky）；⑪ 写 map、**_debug_print_stage3_tables**（Entry Group / Admission / Scoring）。 |
...
| **主要公式** | **准入**（check_stage3_admission）：trusted_primary 不 hard_drop；secondary_primary 仅当 anchor_id<0.16 且 jd_align<0.72 且 ctx<0.48 时拒；support_expansion 弱证据或 object+generic 双高时拒。**identity 软因子**（compute_identity_factor）：≥0.50→1.0，≥0.35→0.94，≥0.22→0.88，否则 0.78。**risk_penalty**：trusted 轻罚、support 重罚（polysemy/object_like/generic）。**唯一主分**：score_term_record 的 final × identity_factor × risk_penalty。**bucket**：trusted_primary 且 score≥0.55 且 poly≤0.55→core；score≥0.42 且 obj/generic 不过高→support；其余 risky。**选词**：core 先取满，再 support，再 risky，上限 PAPER_RECALL_MAX_TERMS(12)。 |
...
- **Stage3**：幸存词打 **stage3_bucket**（core/support/risky）与 **retrieval_role**（paper_primary/paper_support/paper_risky）；**select_terms_for_paper_recall** 优先选 **core**、再补 **support**、不足再补 **risky**，上限 **PAPER_RECALL_MAX_TERMS**（默认 12）。  
