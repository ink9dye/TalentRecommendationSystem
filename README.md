...
└── src/
    ...
    │   │       ├── stage3_term_filtering.py  # 词过滤与权重：按 tid 去重聚合 → 轻硬过滤 → identity/topic gate → score_term_record → family 角色约束 → _collect_risky_reasons（weak_topic_fit_tail 等）→ _bucket_stage3_terms（core 两条路：强主干≥0.66 或 结构型≥0.62/ptc≥0.55/cross≥0.94）→ _debug_print_stage3_bucket_details → top_k
...
**Stage3：词过滤与权重（去重聚合 + 唯一主分 + family 角色约束 + top_k）**

| 项 | 说明 |
|----|------|
| **作用** | 对 raw_candidates **按 tid 去重聚合**后：① **全局共识**（无 cluster）；② **轻硬过滤** should_drop_term（仅对非 primary-like）；③ **身份闸门** passes_identity_gate；④ **topic 闸门**：**仅对 support-like 严格**，primary-like 跳过（_is_primary_like），软惩罚交给 score_term_record 的 path_topic_consistency；⑤ **唯一主分** score_term_record（primary-like 的 path_topic_consistency 保底 0.45）；⑥ **family 角色约束**；⑦ **risky/bucket**；⑧ 按 final_score 排序 top_k；⑨ **select_terms_for_paper_recall**。日志：`[Stage3 topic_gate] bypass primary-like` / `[Stage3 topic_gate] drop support ... reason=topic_consistency`。 |
| **思路** | **彻底移除 cluster 依赖**；Stage3 先去重聚合再排序；family 内 primary 可见、support 只补充不夺权；risky/bucket 基于弱证据、topic 偏移、family 边缘。 |
| **逻辑流程** | ① _merge_stage3_duplicates；② stage2_rank 写入；③ _compute_stage3_global_consensus（无 cluster）；④ _build_family_buckets；⑤ 每 term _compute_family_centrality；⑥ should_drop_term → passes_identity_gate → 若 primary_like 则跳过 topic gate，否则 passes_topic_consistency；⑦ quality_score 仅 debug；⑧ score_term_record 唯一主分（含 backbone_boost、object_like_penalty、bonus_term_penalty）；⑨ 排序 → _apply_family_role_constraints → 再排序；⑩ _collect_risky_reasons、_bucket_stage3_terms（core/support/risky）；⑪ select_terms_for_paper_recall；⑫ 写 map、tag_purity_debug。 |
| **输入参数（名字与含义）** | **raw_candidates**：Stage2 输出；**query_vector**；**anchor_vids**：可选。 |
| **输出参数（名字与含义）** | **score_map**、**term_map**、**idf_map**、**term_role_map**、**term_source_map**、**parent_anchor_map**、**parent_primary_map**、**paper_terms**；**tag_purity_debug** 含 tid、term、anchor_count、evidence_count、family_centrality、path_topic_consistency、generic_penalty、cross_anchor_factor、**backbone_boost**、**object_like_penalty**、**bonus_term_penalty**、retrieval_role、stage3_explain。 |
| **主要公式** | **身份闸门**：primary/similar_to≥0.62，其余≥0.68。**topic 闸门**：primary-like 默认通过；support-like 仅当 topic_fit<0.05 且 outside>0.95 时拒绝。**唯一主分**（score_term_record）：base=0.36·semantic+0.18·context+0.20·subfield_fit+0.14·topic_fit+0.12·multi_source；path_topic_consistency 含 primary_bonus/similar_bonus/anchor_bonus，**primary-like 保底 0.45**；gate=0.75+0.15×path_topic+0.10×generic_penalty；**backbone_boost**（primary/多锚点/高 cross_anchor，上限 1.08）、**object_like_penalty**（arm/manipulator/hand 轻压，≥0.92）、**bonus_term_penalty**（RL/q-learning/medical robotics 轻压，≥0.90）；final=base×gate×cross_anchor_factor×backbone_boost×object_like_penalty×bonus_term_penalty。**bucket**：**core 两条路**——路 A 强主干直通（paper_primary 且 final≥0.66），路 B 结构型主干（paper_primary 且 final≥0.62、ptc≥0.55、cross≥0.94）；support 为 final≥0.56 且无 high_drift_risk；其余 risky。**保留策略**：按 final_score 降序取前 STAGE3_TOP_K（默认 20）。 |

**Stage3 主分**：**compose_term_final_score** 已退出 Stage3 主链（deprecated），唯一主分由 **score_term_record** 承担；final = base_score×gate×cross_anchor_factor×**backbone_boost**×**object_like_penalty**×**bonus_term_penalty**（backbone 轻推主轴、object 轻压 arm/manipulator/hand、bonus 轻压 RL/q-learning/medical robotics）。
...
| **5. Stage3** | 全局重排与风险分桶 | `[Stage3]` 分区头；`[Stage3 Final Score Breakdown]` … \| final；`[Stage3 Buckets]` core_terms / support_terms / risky_terms；**`[Stage3 Bucket Details]`** term \| bucket \| final \| ptc \| cross \| reasons；`[Stage3 Risky Term Reasons]` term \| reasons \| final（LEVEL≥3） |
...
- **Stage3**：幸存词打 **family_key** 与 **retrieval_role**；**select_terms_for_paper_recall** 按 family 分桶，每 family 保 1 个 primary + 最多 1 个 support（support 只补充不夺权）；上限 **PAPER_RECALL_MAX_TERMS**（默认 12）。  
...
- **Stage3 全部滤空或几乎全过**：优先看 passes_identity_gate；**topic 闸门只拦 support-like**，primary-like 跳过；当前按 final_score 排序保留 top_k（STAGE3_TOP_K）；可开 STAGE3_DETAIL_DEBUG 看每词 base_score、path_topic_consistency、**backbone_boost**、**object_like_penalty**、**bonus_term_penalty**、reject_reason；**Stage3 Buckets** 重点看 core 是否含 motion control/robot control，medical robotics 是否落 support/risky 尾部。
