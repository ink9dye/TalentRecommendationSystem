...
| **核心中间产物** | **Stage1**：Stage1Result（**含 jd_profile**）、anchor_skills（含 local_context、phrase_context、**anchor_source/anchor_source_weight**）、_jd_cleaned_terms、job_ids、regex_str。**Stage2A**：primary_landings（含 domain_fit、**landing_score**、**subfield_fit**、**topic_fit** 等；有 jd_profile 时做层级 fit 与硬门槛）。**Stage2B**：raw_candidates（term_role、domain_fit、parent_anchor、parent_primary、**subfield_fit**、**topic_fit**、**landing_score**、**cluster_id**、**main_subfield_match**；**`run_stage2`→`_expanded_to_raw_candidates` 顶层透传** **`primary_bucket`、`can_expand_from_2a`、`fallback_primary`、`admission_reason`、`reject_reason`、`survive_primary`、`stage2b_seed_tier`、`mainline_candidate`、`primary_reason`**，供 Stage3 聚合与统一/paper 门消费）。**Stage3**：**按 tid 去重聚合**、**classify_stage3_entry_groups**、**check_stage3_admission**、**第一段** score_term_record×**identity_factor**×**risk_penalty**×**role_factor**、**assign_stage3_bucket**（观测）、**`stage3_build_score_map`（统一连续分）**、**select_terms_for_paper_recall**（按统一分+family+floor）、_collect_risky_reasons/_bucket_stage3_terms；score_map、term_map、idf_map、term_role_map、term_source_map、parent_anchor_map、parent_primary_map、tag_purity_debug（含 **stage3_entry_group**、**identity_factor**、**risk_penalty**、**stage3_bucket**、anchor_count、evidence_count、family_centrality、path_topic_consistency、generic_penalty、cross_anchor_factor、retrieval_role 等）；label_path 据此构建 term_confidence_map、term_uniqueness_map。**Stage4**：author_papers_list。**Stage5**：paper_map、contribution、作者聚合分、最终 author_id 列表。 |
...
| **Stage3** | raw_candidates（含 term_role、role_in_anchor、source_types、retain_mode、**primary_bucket / fallback_primary / mainline_candidate** 等）, query_vector, anchor_vids | score_map, term_map, idf_map, **paper_terms** | 去重聚合 → classify_stage3_entry_groups → check_stage3_admission；**第一段**：score_term_record × identity × risk × **role_factor**；**`_assign_stage3_bucket`**（仅观测标签 core/support/risky）；**`stage3_build_score_map`**：**统一连续分**取代 bucket 乘子链 → **`_apply_family_role_constraints`**；**`select_terms_for_paper_recall`**：**全候选按 final_score 排序** + **family 去重** + **`dynamic_floor`** + topN；仍硬挡 **单锚 conditioned_only**、**fallback_primary**；**`[Stage3 unified score breakdown]`**、**`[Stage3 paper cutoff]`**、**`[Stage3 support contamination]`** |
...
| **Stage5** | author_papers_list, score_map, term_map, debug_1（含 term_role_map、term_confidence_map、**term_family_keys**、**term_paper_meta**、**term_retrieval_roles**） | author_id 列表、last_debug_info（含 author_evidence_by_term_role） | **paper_scoring.compute_contribution**：累加 rank_score 时对 **同 wid 上 `hit.role=paper_support`** 做**附着衰减**（有 `paper_primary` 同现→辅证因子；仅 support→强衰减；再乘 **mainline_hits** 浅开关）；护栏 5；accumulate_author_scores；CoverageBonus、FamilyBalancePenalty；**`[Stage5 support dominance audit]`**；time_weight、recency；AUTHOR_BEST_PAPER_MIN_RATIO |
...
- **Stage3 全局复审层（非全杀门）**：去重聚合 → classify_stage3_entry_groups → check_stage3_admission → **第一段分** = score_term_record × identity_factor × risk_penalty × **role_factor**（同左 Role 因子）；**assign_stage3_bucket** 仍为 core/support/risky，**仅观测**；**第二段** **`stage3_build_score_map`**：改为 **统一连续分**（seed / anchor_identity / jd_align / family_centrality / cross_anchor / mainline / expand − drift / generic / poly / object），再 **`_apply_family_role_constraints`**；**select_terms_for_paper_recall**：按 **统一 final_score** 全局排序 + **family_key** 去重 + **`PAPER_RECALL_DYNAMIC_FLOOR_*`**，**不再** core/support/risky 三岔与 **`_support_grounded_enough`**；**risky 可凭分入 paper**（若过线且未撞 family/floor）；上限 **PAPER_RECALL_MAX_TERMS**（默认 12）。
...
| **`[paper_term_selection gate]`** | **`_print_paper_term_selection_audit`**（`STAGE3_PAPER_GATE_DEBUG`，**默认 False 降噪**） | 逐项 paper 门历史字段；**当前 paper 准入以 `[Stage3 paper cutoff]` 为主**（统一分 + family + `dynamic_floor`） |
...
| **`[Stage3 paper cutoff]`** | **`select_terms_for_paper_recall` 末尾**（`STAGE3_PAPER_CUTOFF_AUDIT`） | 全表排名：`dynamic_floor`、`family_key`、`selected`、`cutoff_reason`（`below_dynamic_floor` / `family_dup_or_below_cut` / `top_n_unified_score` 等） |
| **`[Stage3 support contamination]`** | **`select_terms_for_paper_recall` 末尾**（`STAGE3_SUPPORT_CONTAMINATION_AUDIT`） | 已入 paper 且 **stage3_bucket=support**、结构单薄（单锚低 ml 或 **paper_grounding_proxy** 过低）的摘要，便于 Stage4 前预警 |
...
- **`select_terms_for_paper_recall`**：候选集合上 **`dynamic_floor = max(PAPER_RECALL_DYNAMIC_FLOOR_ABS, top_eligible_score × PAPER_RECALL_DYNAMIC_FLOOR_REL)`**；按 **`final_score` 降序**，**`family_key` 去重**，取满 **`PAPER_RECALL_MAX_TERMS`**；**仍硬挡**：**单锚 conditioned_only**、**fallback_primary**。**`retrieval_role`**：bucket==core → `paper_primary`，否则 **`paper_support`**（仅软权重，不再由桶决定入场）。
...
- **3）Stage3 第二段 + paper 选词（现行）**
  ...
  - **`select_terms_for_paper_recall`**：**统一分全局排序** + **`family_key` 去重** + **`dynamic_floor`**；单锚 conditioned_only、**fallback_primary** 仍不进；**risky 可入 paper**（由分数与截断决定，不再「risky 桶一律不进」）。
...
- **Stage3 全局复审层**：classify_stage3_entry_groups → check_stage3_admission；**第一段** = score_term_record × identity_factor × risk_penalty × **role_factor**（系数同前）；**`stage3_build_score_map`** 用 **统一连续分** 覆盖 **`final_score`**（**已移除** bucket 乘子链），再 **`_apply_family_role_constraints`**；**select_terms_for_paper_recall** 按 **统一分** + **family** + **dynamic_floor**，**risky 可入 paper**。日志含 **[Stage3 unified score breakdown]**、**[Stage3 paper cutoff]** 等。
...
| 项 | 说明 |
|----|------|
| **作用** | 对 raw_candidates **按 tid 去重聚合**后：① classify_stage3_entry_groups；② check_stage3_admission；③ should_drop_term；④ topic 闸门；⑤ **第一段**：score_term_record × identity × risk × **role_factor**；⑥ **assign_stage3_bucket**（**观测用**）；⑦ **stage3_build_score_map**：**统一连续分** + **`_apply_family_role_constraints`**；⑧ **select_terms_for_paper_recall**（统一分排序 + family + dynamic_floor）；⑨ 写 map、tag_purity_debug。**目标**：减少「规则乘子 + 桶裁判」导致的支线词入 paper；强证据词靠连续分项胜出。 |
...
| **函数 2：paper** | **conditioned_only** ∧ **单锚** → **`conditioned_only_single_anchor_block`**。**is_fallback_primary** → **`fallback_primary_block`**。其余 **一律按统一 `final_score` 排序**，**`family_key` 去重**，**`dynamic_floor` 截断**，**topN**；**`retrieval_role`**：core→`paper_primary`，否则 **`paper_support`**。 |
...
| **调试审计（可选）** | **`STAGE3_AUDIT_DEBUG`**：**`[Stage3 final adjust audit]`**（pipeline pre vs 统一 post；risk/cross 列deprecated=1.0）、**`[Stage3 cross-anchor audit]`**、**`[Stage3 support/risky concise]`**、**`[Stage3->Paper bridge]`**。**`STAGE3_UNIFIED_SCORE_DEBUG`**：**`[Stage3 unified score breakdown]`**。**`STAGE3_PAPER_CUTOFF_AUDIT`** / **`STAGE3_SUPPORT_CONTAMINATION_AUDIT`**：paper 排名与可疑 support 摘要。**`STAGE3_PAPER_GATE_DEBUG`**：逐项 gate（默认 **False**）。**`DEBUG_LABEL_PATH`**：**`[Stage3 rerank summary]`**（统一分后 Top12）。 |
...
- **审计（hierarchy bonus 分布）**：`[Stage4 hierarchy bonus distribution]`  
  **candidate 篇数**；**hierarchy_consensus** 与 **hierarchy_bonus** 的 **min / p25 / p50 / p75 / max**；**top boosted**（按 bonus 降序）、**top penalized**（按 bonus 升序）。用于判断是 consensus 太挤还是映射仍偏软。
...
- **Stage3**：幸存词打 **stage3_bucket**（**观测**）与 **retrieval_role**（paper_primary/paper_support，由 bucket 软映射）；**select_terms_for_paper_recall** 按 **统一 final_score** 排序 + **family 去重** + **dynamic_floor**，上限 **PAPER_RECALL_MAX_TERMS**（默认 12）；**risky 可入选**（凭分与截断）。  
...
| **PAPER_RECALL_MAX_TERMS**（stage3_term_filtering） | 12 | 参与论文检索的最大词数（**统一分**排序 + family 去重 + dynamic floor 截断） |
