...
| `check_primary_admission` | 准入从「hierarchy/path 弱 ⇒ reject」改为**三档**：① 大领域强冲突（三层 overlap 全 0 且 semantic/jd 不足）⇒ reject；② 主干语义保护（semantic≥0.82、jd_align≥0.80）⇒ **weak_retain、suppress_seed=False**（可做 seed）；③ effective_topic/effective_path 达标 ⇒ **normal**；④ hierarchy 弱但 semantic≥0.78、jd_align≥0.76 ⇒ **weak_retain、suppress_seed=True**（保留但不扩散）；⑤ 否则 reject。 |
| `compute_primary_score` | hierarchy 改为**强加分项**（0.2×effective_subfield + 0.3×effective_topic + 0.3×effective_path + 0.2×specificity），base 为 0.35×semantic + 0.25×jd_align + 0.15×identity + 0.10×cross + 0.15×neighbor；**weak_retain** 仅轻降权 ×0.85，不判死。 |
